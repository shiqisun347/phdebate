...
@pytest.mark.asyncio
async def test_paused_room_remains_scheduled_for_disconnect_pause(register_user) -> None:
    """A paused room must retain an offline participant and stay recoverable.

    Redis presence expiry records the disconnect, while the match engine owns
    the 60-second safety pause. The scheduler therefore has to keep paused
    rooms in its lifecycle set even though it does not advance their stage.
    """
    ...
    with SessionLocal() as db:
        ...
        assert old_owner.occupant_type == "human"
        assert room.owner_id == old_owner.user_id
        assert db.scalar(
...
async def test_five_room_mixed_format_long_soak_preserves_authority_and_recovers(
    ...
    disconnect recovery, engine-restart recovery and out-of-order judge
    ...
    with SessionLocal() as db:
        for code in retry_judge_codes:
            ...
            assert room.status == "judging" and scorecard.status == "running" and scorecard.task_id
    for code in retry_judge_codes:
    ...
    with SessionLocal() as db:
        ...
        for scenario in scenarios:
            ...
            if scenario["code"] in disconnected_codes:
                ...
                assert any(item.event_type == "participant.disconnect_timeout" for item in events)
