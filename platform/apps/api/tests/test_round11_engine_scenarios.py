...
@pytest.mark.asyncio
async def test_public_lobby_websocket_sends_redacted_snapshot_without_raising_http_response(register_user) -> None:
    """A shared lobby link is useful, but never exposes student identities."""
    ...
    assert socket.sent_texts and '"real_name":"房主"' in socket.sent_texts[0]
    assert "round11_hidden_websocket" not in socket.sent_texts[0]
