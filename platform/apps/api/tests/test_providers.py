...
def test_lighttts_prompt_text_map_is_normalized_and_validated() -> None:
    ...
    assert configured.moss_tts_realtime_first_pcm_timeout_seconds == 2.5
    ...
    assert Settings(moss_tts_prompt_files={"debate_voice_1": " voice-1.wav "}).moss_tts_prompt_files == {"debate_voice_1": "voice-1.wav"}
    with pytest.raises(ValueError, match="音色 ID"):
    ...
    with pytest.raises(ValueError, match="只允许 1 个活动会话"):
        Settings(
            ...
            moss_tts_prompt_files={f"debate_voice_{index}": f"debate_voice_{index}.wav" for index in range(1, 9)},
        )
...
async def test_agent_provider_accepts_json_idempotency_replay_when_stream_was_requested(monkeypatch) -> None:
    """A completed task replay may be JSON even though the request asks for SSE."""
    ...
    async def handler(request: httpx.Request) -> httpx.Response:
        ...
        return httpx.Response(
            ...
            json={
                "task_id": "completed-prefetch-task",
                "content": "这是已经完成并缓存的辩手发言。",
                "status": "completed",
            },
        )
...
class IncrementalBiStreamWebSocket:
    ...
    async def recv(self) -> bytes | str | None:
        if not self.audio_sent:
            while not any(isinstance(item, str) and "tts_text" in json.loads(item) for item in self.sent):
                await asyncio.sleep(0)
...
async def test_moss_ws_prepare_reserves_ready_endpoint_before_accepting_text(
    tmp_path: Path,
    monkeypatch,
) -> None:
    configure_ws_moss(monkeypatch, tmp_path, endpoints=["https://moss-prepare.internal:8443/api"])
    ...
    session = provider.open_incremental_session(
        room_code="moss-prepare-room",
        speech_id="moss-prepare-speech",
    ...
    await asyncio.sleep(0)
    turn_websocket = next(websocket for websocket in connections if any(payload["type"] == "start" for payload in websocket.sent))
    assert not preparing.done(), "prepare must wait for the upstream ready contract"
    assert [payload["type"] for payload in turn_websocket.sent] == ["start"]

    allow_ready.set()
    await asyncio.wait_for(preparing, timeout=1)
    assert [payload["type"] for payload in turn_websocket.sent] == ["start"]

    await session.push_text("准备完成后才提交正文。")
    await session.finish()
    assert [payload["type"] for payload in turn_websocket.sent] == [
        "start",
        "text_delta",
        "final",
    ]
...
async def test_moss_ws_second_prepare_waits_for_capacity_and_abort_releases_first(
    tmp_path: Path,
    monkeypatch,
) -> None:
    configure_ws_moss(monkeypatch, tmp_path, endpoints=["https://moss-capacity.internal:8443/api"])
    connections: list[FakePersistentMossWebSocket] = []
    ...
    first = provider.open_incremental_session(
        room_code="moss-capacity-a",
        speech_id="speech-a",
        voice="debate_voice_1",
        on_stream_event=lambda _event: asyncio.sleep(0),
    )
    second = provider.open_incremental_session(
        room_code="moss-capacity-b",
        speech_id="speech-b",
        voice="debate_voice_1",
        on_stream_event=lambda _event: asyncio.sleep(0),
    )

    await asyncio.wait_for(first.prepare(), timeout=1)
    second_preparing = asyncio.create_task(second.prepare())
    await asyncio.sleep(0.05)
    assert not second_preparing.done(), "the second room must queue before its Agent stream starts"
    assert sum(payload["type"] == "start" for websocket in connections for payload in websocket.sent) == 1

    await asyncio.wait_for(first.abort(reason="match_paused"), timeout=1)
    await asyncio.wait_for(second_preparing, timeout=1)
    assert sum(payload["type"] == "start" for websocket in connections for payload in websocket.sent) == 2
    await asyncio.wait_for(second.abort(reason="test_cleanup"), timeout=1)
    assert provider._endpoint_semaphores[0][1].locked() is False


async def test_moss_ws_queued_room_receives_fresh_active_job_deadline(
    tmp_path: Path,
    monkeypatch,
) -> None:
    configure_ws_moss(monkeypatch, tmp_path, endpoints=["https://moss-deadline.internal:8443/api"])
    monkeypatch.setattr(settings, "moss_tts_realtime_queue_timeout_seconds", 1.0)
    monkeypatch.setattr(settings, "moss_tts_realtime_job_timeout_seconds", 0.25)
    ...
    async def script(websocket: FakePersistentMossWebSocket, payload: dict[str, object]) -> None:
        if payload["type"] == "start":
            ...
            async def finish_after_one_room_turn() -> None:
            ...
            asyncio.create_task(finish_after_one_room_turn())
    ...
    provider = MossTTSRealtimeProvider(httpx.MockTransport(moss_ready_handler), ws_connect=connect)
    first = provider.open_incremental_session(
        room_code="moss-deadline-a",
        speech_id="speech-a",
        voice="debate_voice_1",
        on_stream_event=lambda _event: asyncio.sleep(0),
    )
    second = provider.open_incremental_session(
        room_code="moss-deadline-b",
        speech_id="speech-b",
        voice="debate_voice_1",
        on_stream_event=lambda _event: asyncio.sleep(0),
    )
    ...
    await first.prepare()
    await first.push_text("第一场先使用唯一的语音容量。")
    first_finish = asyncio.create_task(first.finish())
    second_prepare = asyncio.create_task(second.prepare())
    await asyncio.sleep(0.1)
    assert not second_prepare.done()

    await asyncio.wait_for(first_finish, timeout=1)
    await asyncio.wait_for(second_prepare, timeout=1)
    await second.push_text("第二场排队结束后仍应获得完整生成时间。")
    assert await asyncio.wait_for(second.finish(), timeout=1) == "/media/moss-deadline-b/speech-b.wav"
...
async def test_moss_ws_uses_one_persistent_connection_for_start_deltas_pcm_and_release(
    ...
    turn_connections = [item for item in connections if any(payload["type"] == "start" for payload in item[2].sent)]
...
async def test_moss_ws_integrates_with_real_gateway_asgi_and_websockets(
    ...
    server = uvicorn.Server(uvicorn.Config(application, host="127.0.0.1", port=port, log_level="error", lifespan="on"))
...
async def test_moss_ws_first_pcm_timeout_aborts_same_connection_and_returns_scheduler_slot(
    tmp_path: Path,
    monkeypatch,
) -> None:
...
async def test_moss_ws_rejects_out_of_order_ack_and_returns_scheduler_slot(tmp_path: Path, monkeypatch) -> None:
...
async def test_moss_ws_lost_delta_and_release_ack_does_not_wedge_following_speech(
    tmp_path: Path,
    monkeypatch,
) -> None:
    configure_ws_moss(monkeypatch, tmp_path, endpoints=["http://moss-recover.internal:8083"])
    monkeypatch.setattr(settings, "moss_tts_realtime_ack_timeout_seconds", 0.02)
    monkeypatch.setattr(settings, "moss_tts_realtime_close_timeout_seconds", 0.02)
    connections = 0

    async def failing_script(websocket: FakePersistentMossWebSocket, payload: dict[str, object]) -> None:
        if payload["type"] == "start":
            websocket.session_id = str(payload["session_id"])
            websocket.feed_json(
                {
        ...
        # Deliberately lose both the text_delta ACK and abort release frame.

    def connect(_url: str, **_kwargs):
        nonlocal connections
        connections += 1
        script = failing_script if connections == 1 else normal_moss_ws_script
        return FakePersistentMossConnection(FakePersistentMossWebSocket(script))
    ...
    provider = MossTTSRealtimeProvider(httpx.MockTransport(moss_ready_handler), ws_connect=connect)
    failed = provider.open_incremental_session(
        room_code="moss-recover-room",
        speech_id="speech-failed",
        voice="debate_voice_1",
        on_stream_event=on_stream_event,
    )
    await failed.push_text("第一次发言模拟丢失确认。")
    with pytest.raises(ProviderError) as raised:
        await asyncio.wait_for(failed.finish(), timeout=0.5)
    assert raised.value.code == "moss_tts_ws_ack_timeout"
    assert provider._endpoint_semaphores[0][1].locked() is False

    recovered = provider.open_incremental_session(
        room_code="moss-recover-room",
        speech_id="speech-recovered",
        voice="debate_voice_1",
        on_stream_event=on_stream_event,
    )
    await recovered.push_text("恢复正常。")
    url = await asyncio.wait_for(recovered.finish(), timeout=1)

    assert url == "/media/moss-recover-room/speech-recovered.wav"
    assert connections >= 2
    assert provider._endpoint_semaphores[0][1].locked() is False


async def test_moss_ws_refuses_gateway_that_still_has_an_active_upstream_turn(
    tmp_path: Path,
    monkeypatch,
) -> None:
    configure_ws_moss(monkeypatch, tmp_path, endpoints=["http://moss-busy.internal:8083"])
    connect_calls = 0

    async def busy_handler(request: httpx.Request) -> httpx.Response:
        if request.url.path.endswith("/health/ready"):
            return moss_ready_response(active=1)
        return httpx.Response(404)
    ...
    provider = MossTTSRealtimeProvider(httpx.MockTransport(busy_handler), ws_connect=connect)
    session = provider.open_incremental_session(
        room_code="moss-busy-room",
        speech_id="speech-busy",
        voice="debate_voice_1",
        on_stream_event=on_stream_event,
    )
    with pytest.raises(ProviderError) as raised:
        await asyncio.wait_for(
            session.push_text("网关还在释放旧任务时不能创建第二个 turn。"),
            timeout=0.5,
        )

    assert raised.value.code == "moss_tts_not_ready"
...
async def test_lighttts_whole_job_deadline_includes_retry_budget_and_removes_stale_audio(tmp_path: Path, monkeypatch) -> None:
    prompt = tmp_path / "debate_voice_1.wav"
    prompt.write_bytes(b"prompt")
    media = tmp_path / "media"
...
async def test_external_task_cancel_drains_gpu_before_releasing_global_lease(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(providers_service, "lighttts_admission_gate", Gate())
    provider = LightTTSProvider(httpx.MockTransport(handler))
    waiter = asyncio.create_task(provider.synthesize("外部取消必须先安全排空。", room_code="external-cancel", speech_id="speech"))
    await asyncio.wait_for(request_started.wait(), timeout=1)
    waiter.cancel()
