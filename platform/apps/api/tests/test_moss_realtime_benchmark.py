...
def test_continuous_playback_distinguishes_network_gaps_from_audible_underruns() -> None:
    ...
    # A 960ms PCM block arriving every 800ms has a large raw network interval,
    # but the delivery is still 160ms ahead of playout and must not be called a
    # gap or a stall.
    assert benchmark.delivery_excess_gaps_ms(
        [0.0, 0.8, 1.6],
        [0.96, 0.96, 0.96],
    ) == [0.0, 0.0]
    assert benchmark.delivery_excess_gaps_ms(
...
async def test_websocket_benchmark_uses_one_incremental_session_and_demuxes_pcm(
    ...
    assert result["delivery_excess_gap_p99_ms"] >= 0
