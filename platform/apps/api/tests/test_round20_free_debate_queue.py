...
@pytest.mark.asyncio
async def test_free_debate_waits_for_real_human_speech_and_freezes_between_turns(register_user) -> None:
    owner = register_user("round56_free_clock_owner")
    opponent = register_user("round56_free_clock_opponent")
    code = create_training_room(owner, "自由辩论真实发言计时")["code"]
    ...
    initial = owner.get(f"/api/rooms/{code}").json()["room"]
    assert initial["remaining_seconds"] == 120
    assert initial["turn_remaining_seconds"] == 30
    assert initial["current_stage"]["awaiting_human_start"] is True
    with SessionLocal() as db:
        room = load_room(db, code)
        assert room.stage_deadline_at is None
        assert stage(room).get("turn_started_at") is None

    # Repeated engine ticks and elapsed wall-clock time must not manufacture a
    # silent timeout before the participant presses the speaking button.
    ...
    headers = csrf(owner) | {"X-Control-Lease": "round56-free-clock"}
    owner.post(f"/api/rooms/{code}/control-lease", headers=headers, json={}).raise_for_status()
    ...
    with SessionLocal() as db:
        ...
        current = stage(room)
        assert current and current.get("turn_started_at")
        room.stage_deadline_at = now() + timedelta(seconds=117)
        db.commit()

    finished = owner.post(
        f"/api/rooms/{code}/speech/finish",
        headers=headers,
        json={"speech_id": started.json()["speech_id"], "content": "我方先明确核心判断，并给出第一项完整论据。"},
    )
    ...
    with SessionLocal() as db:
        ...
        current = stage(room)
        assert room.stage_deadline_at is None
        assert current and current.get("intermission_deadline_at")
        assert current.get("turn_started_at") is None
        assert current.get("free_stage_remaining_seconds") in {116, 117}
...
def test_concurrent_requests_are_ordered_once_and_only_selected_human_can_speak(client, register_user) -> None:
    ...
    with SessionLocal() as db:
        ...
        requests = list(
            db.scalars(
                select(FreeTurnRequest).where(FreeTurnRequest.room_id == room.id).order_by(FreeTurnRequest.requested_at, FreeTurnRequest.id)
            ).all()
        )
...
def test_all_human_free_turn_does_not_claim_a_nonexistent_ai_fallback(register_user) -> None:
    owner = register_user("round20_all_human_owner")
    participant = register_user("round20_all_human_participant")
    code = create_training_room(owner, "Round20 双真人无人举手提示")["code"]
    _claim(participant, code, "neg_1")
    speech_id = _start_free_room(owner, code, [participant])
    with SessionLocal() as db:
        room = load_room(db, code, lock=True)
        db.get(Speech, speech_id).status = "completed"
        current = dict(stage(room))
        current["side"] = "neg"
        current["force_ai_fallback"] = True
        snapshot = list(room.template_snapshot)
        snapshot[0] = current
        room.template_snapshot = snapshot
        db.commit()

    projection = participant.get(f"/api/rooms/{code}")
    assert projection.status_code == 200
    assert projection.json()["room"]["speak_reason"] == "本轮无人申请，系统将自动交换发言方"
...
async def test_no_request_resolves_to_bounded_ai_fallback_after_restart(register_user, monkeypatch) -> None:
    ...
    with SessionLocal() as db:
        ...
        assert stage(room)["ai_preparing"] is True
        assert stage(room)["preparing_stage_remaining_seconds"] > 0
        assert stage(room)["preparing_turn_remaining_seconds"] == 30
    # The application window is over, but provider work has not begun on this
    # fresh engine process yet.  The authoritative projection must already
    # identify AI preparation and retain a non-zero clock instead of exposing
    # a hidden 00:00 transition.
    projected = owner.get(f"/api/rooms/{code}").json()["room"]
    assert projected["current_stage"]["ai_preparing"] is True
    assert projected["remaining_seconds"] > 0
    assert projected["turn_remaining_seconds"] == 30
...
async def test_ai_intent_and_candidate_run_concurrently_and_negative_discards(
    ...
    with SessionLocal() as db:
        ...
        assert "ai_preparing" not in current
        assert "preparing_stage_remaining_seconds" not in current
        assert "preparing_turn_remaining_seconds" not in current
...
async def test_ai_speculation_starts_during_three_second_window_and_is_reused_once(
    ...
    with SessionLocal() as db:
        ...
        assert current["preparing_turn_remaining_seconds"] == 30
