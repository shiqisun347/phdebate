...
class SharedPresenceRedis:
    ...
    async def eval(self, script: str, key_count: int, *values):
        ...
        if script in {
            realtime_service._ASR_STREAM_CLAIM_SCRIPT,
            realtime_service._LIVEKIT_PUBLISHER_CLAIM_SCRIPT,
        }:
            owner, lease_ms = args
            current = self.values.get(keys[0])
            now_ms = realtime_service.time.time() * 1000
            if current and current[1] > now_ms:
                if (
                    script == realtime_service._LIVEKIT_PUBLISHER_CLAIM_SCRIPT
                    and current[0] == str(owner)
                ):
                    self.values[keys[0]] = (str(owner), now_ms + float(lease_ms))
                    return 1
                return 0
            self.values[keys[0]] = (str(owner), now_ms + float(lease_ms))
            return 1
        if script == realtime_service._ASR_STREAM_REFRESH_SCRIPT:
            owner, lease_ms = args
            current = self.values.get(keys[0])
            now_ms = realtime_service.time.time() * 1000
            if current and current[1] <= now_ms:
                ...
                current = None
            if not current or current[0] != str(owner):
                return 0
            self.values[keys[0]] = (str(owner), now_ms + float(lease_ms))
            return 1
        if script == realtime_service._ASR_STREAM_RELEASE_SCRIPT:
            owner = str(args[0])
            current = self.values.get(keys[0])
...
async def test_asr_stream_lease_is_atomic_across_api_workers_and_owner_bound(monkeypatch) -> None:
    shared = SharedPresenceRedis()
    monkeypatch.setattr(realtime_service.redis, "from_url", lambda *_args, **_kwargs: shared)
    first_worker = RoomHub()
    second_worker = RoomHub()

    assert await first_worker.asr_stream_claim("speech-1", owner="socket-a") is True
    assert await second_worker.asr_stream_claim("speech-1", owner="socket-b") is False
    assert await second_worker.asr_stream_refresh("speech-1", owner="socket-b") is False
    assert await second_worker.asr_stream_release("speech-1", owner="socket-b") is False
    assert await first_worker.asr_stream_refresh("speech-1", owner="socket-a") is True
    assert await first_worker.asr_stream_release("speech-1", owner="socket-a") is True
    assert await second_worker.asr_stream_claim("speech-1", owner="socket-b") is True

    await first_worker.close()
    await second_worker.close()


async def test_asr_stream_lease_expires_after_crashed_worker(monkeypatch) -> None:
    shared = SharedPresenceRedis()
    clock = {"seconds": 1_000.0}
    monkeypatch.setattr(realtime_service.redis, "from_url", lambda *_args, **_kwargs: shared)
    monkeypatch.setattr(realtime_service.time, "time", lambda: clock["seconds"])
    ...
    assert await first_worker.asr_stream_claim("speech-2", owner="socket-a", lease_seconds=2) is True
    clock["seconds"] += 1
    assert await second_worker.asr_stream_claim("speech-2", owner="socket-b", lease_seconds=2) is False
    clock["seconds"] += 2
    assert await second_worker.asr_stream_claim("speech-2", owner="socket-b", lease_seconds=2) is True

    await first_worker.close()
    await second_worker.close()


async def test_livekit_publisher_lease_is_atomic_owner_bound_and_reclaimable(monkeypatch) -> None:
    shared = SharedPresenceRedis()
    clock = {"seconds": 1_000.0}
    monkeypatch.setattr(realtime_service.redis, "from_url", lambda *_args, **_kwargs: shared)
    monkeypatch.setattr(realtime_service.time, "time", lambda: clock["seconds"])
    ...
    assert await first_worker.livekit_publisher_claim("123456", owner="worker-a", lease_seconds=2) is True
    assert await first_worker.livekit_publisher_claim("123456", owner="worker-a", lease_seconds=2) is True
    assert await second_worker.livekit_publisher_claim("123456", owner="worker-b", lease_seconds=2) is False
    assert await second_worker.livekit_publisher_release("123456", owner="worker-b") is False
    assert await first_worker.livekit_publisher_refresh("123456", owner="worker-a", lease_seconds=2) is True
    clock["seconds"] += 3
    assert await second_worker.livekit_publisher_claim("123456", owner="worker-b", lease_seconds=2) is True
    assert await first_worker.livekit_publisher_release("123456", owner="worker-a") is False
    assert await second_worker.livekit_publisher_release("123456", owner="worker-b") is True
