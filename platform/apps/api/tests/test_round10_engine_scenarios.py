...
def test_anonymous_users_receive_redacted_waiting_room_but_logged_in_join_still_works(
    client,
    register_user,
) -> None:
    ...
    for public_lobby in (client.get(f"/api/rooms/{code}"), client.get(f"/api/rooms/{code}/public")):
        assert public_lobby.status_code == 200
        public_room = public_lobby.json()["room"]
        assert public_room["owner"]["real_name"] == "房主"
        assert owner_name not in {seat["display_name"] for seat in public_room["seats"]}
        assert all(not seat["is_ready"] and not seat["connected"] for seat in public_room["seats"])
    with client.websocket_connect(f"/ws/rooms/{code}") as socket:
        waiting = socket.receive_json()["room"]
        assert waiting["owner"]["real_name"] == "房主"
        assert owner_name not in {seat["display_name"] for seat in waiting["seats"]}
    assert client.post(f"/api/rooms/{code}/rtc-token").status_code in {403, 503}
...
@pytest.mark.asyncio
async def test_disabled_active_speaker_is_interrupted_without_ai_substitution(client, register_user) -> None:
    ...
    code = create_training_room(owner, "停用进行中真人后安全暂停")["code"]
    ...
    with SessionLocal() as db:
        ...
        assert seat.occupant_type == "human" and seat.connected is False
        ...
        assert event and event.payload["reason"] == "participant_disconnected"
        assert db.scalar(
...
def test_terminated_match_rejects_mutations_and_archive_remains_stable(client, register_user) -> None:
    ...
    assert all(response.status_code in {403, 409, 410} for response in rejected)
