...
def test_production_start_preserves_lobby_when_moss_is_not_warmed(register_user, monkeypatch) -> None:
    ...
    async def cold_snapshot() -> dict:
        return {
            ...
            "idle_ws_ready_endpoints": 0,
            "endpoints": [{"index": 0, "ok": True, "idle_ws_ready": False, "code": "moss_tts_ws_prewarm_failed"}],
        }
    ...
    monkeypatch.setattr(moss_tts_realtime, "prewarm", cold_snapshot)
    ...
    with SessionLocal() as db:
        ...
        open_seats = db.scalars(select(RoomSeat).where(RoomSeat.room_id == room.id, RoomSeat.occupant_type == "open")).all()
        assert open_seats
...
def test_production_start_accepts_one_warmed_moss_endpoint(register_user, monkeypatch) -> None:
    ...
    monkeypatch.setattr(settings, "realtime_voice_backend", "moss_realtime")
    monkeypatch.setattr(settings, "moss_tts_realtime_enabled", True)
    monkeypatch.setattr(moss_tts_realtime, "prewarm", warm_snapshot)
    ...
    monkeypatch.setattr(moss_tts_realtime, "prewarm", cold_after_commit)
...
def test_production_retry_keeps_failure_intact_until_moss_is_warmed(register_user, monkeypatch) -> None:
    ...
    async def cold_snapshot() -> dict:
        return {
            ...
            "required_endpoints": 1,
            "idle_ws_ready_endpoints": 0,
            "endpoints": [{"index": 0, "ok": True, "idle_ws_ready": False}],
        }
    ...
    async def warm_snapshot() -> dict:
        return {
            ...
            "required_endpoints": 1,
            "idle_ws_ready_endpoints": 1,
            "endpoints": [{"index": 0, "ok": True, "model_warmed": True, "active": 0, "pending": 0, "idle_ws_ready": True}],
        }
