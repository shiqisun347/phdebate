...
class SharedPublisherLeaseAuthority:
    def __init__(self) -> None:
    ...
    async def livekit_publisher_claim(
        self, room_code: str, *, owner: str, lease_seconds: float
    ) -> bool | None:
        del lease_seconds
        if not self.available:
            return None
        async with self.lock:
            if room_code in self.owners:
                return self.owners[room_code] == owner
            self.owners[room_code] = owner
            return True

    async def livekit_publisher_refresh(
        self, room_code: str, *, owner: str, lease_seconds: float
    ) -> bool | None:
        del lease_seconds
        ...
        if not self.available:
            return None
        async with self.lock:
            return self.owners.get(room_code) == owner

    async def livekit_publisher_release(self, room_code: str, *, owner: str) -> bool:
        self.release_calls.append((room_code, owner))
        async with self.lock:
            if self.owners.get(room_code) != owner:
                return False
            self.owners.pop(room_code, None)
            return True
...
@pytest.mark.asyncio
async def test_moss_start_buffer_holds_configured_lead_then_plays_without_early_silence(monkeypatch) -> None:
    FakeLiveKitAdapter.instances.clear()
...
@pytest.mark.asyncio
async def test_distributed_lease_allows_only_one_livekit_publisher_across_workers(monkeypatch) -> None:
    FakeLiveKitAdapter.instances.clear()
    monkeypatch.setattr(settings, "livekit_audio_source_queue_ms", 40)
    authority = SharedPublisherLeaseAuthority()
    first = LiveKitAudioRegistry(
        adapter_factory=FakeLiveKitAdapter,
        lease_authority=authority,
        owner_id="worker-a",
    )
    second = LiveKitAudioRegistry(
        adapter_factory=FakeLiveKitAdapter,
        lease_authority=authority,
        owner_id="worker-b",
    )
    ...
    results = await asyncio.gather(
        first.ensure_room("lease-room"),
        second.ensure_room("lease-room"),
        return_exceptions=True,
    )

    assert sum(not isinstance(result, BaseException) for result in results) == 1
    assert sum(isinstance(result, LiveKitPublisherLeaseConflict) for result in results) == 1
    assert len(FakeLiveKitAdapter.instances) == 1
    assert len(authority.owners) == 1
...
@pytest.mark.asyncio
async def test_publisher_lease_loss_revokes_track_and_crash_expiry_can_be_reclaimed(monkeypatch) -> None:
    FakeLiveKitAdapter.instances.clear()
    monkeypatch.setattr(settings, "livekit_audio_source_queue_ms", 40)
    monkeypatch.setattr(livekit_audio_module, "LIVEKIT_PUBLISHER_REFRESH_SECONDS", 0.01)
    authority = SharedPublisherLeaseAuthority()
    first = LiveKitAudioRegistry(
        adapter_factory=FakeLiveKitAdapter,
        lease_authority=authority,
    ...
    await first.ensure_room("takeover-room")
    first_adapter = FakeLiveKitAdapter.instances[0]
    # Models Redis expiring worker A's lease after a crash/stall and worker B
    # claiming it. The stale worker must fail closed on its next heartbeat.
    authority.owners["takeover-room"] = "worker-b"
    await asyncio.wait_for(
        _wait_until(lambda: first_adapter.disconnected),
        timeout=0.2,
    )
    authority.owners.pop("takeover-room")
    publisher = await second.ensure_room("takeover-room")

    assert publisher.track_sid == "TR_takeover-room"
    assert len(FakeLiveKitAdapter.instances) == 2
    await first.close()
    await second.close()


@pytest.mark.asyncio
async def test_same_owner_rebuilds_immediately_after_transient_redis_failure(monkeypatch) -> None:
    FakeLiveKitAdapter.instances.clear()
    monkeypatch.setattr(settings, "livekit_audio_source_queue_ms", 40)
    monkeypatch.setattr(livekit_audio_module, "LIVEKIT_PUBLISHER_REFRESH_SECONDS", 0.01)
    authority = SharedPublisherLeaseAuthority()
    registry = LiveKitAudioRegistry(
        adapter_factory=FakeLiveKitAdapter,
        lease_authority=authority,
        owner_id="worker-a",
    )

    await registry.ensure_room("redis-recovery-room")
    first_adapter = FakeLiveKitAdapter.instances[0]
    authority.available = False
    await asyncio.wait_for(_wait_until(lambda: first_adapter.disconnected), timeout=0.2)
    assert authority.owners == {"redis-recovery-room": "worker-a"}

    authority.available = True
    rebuilt = await registry.ensure_room("redis-recovery-room")
    assert rebuilt.track_sid == "TR_redis-recovery-room"
    assert len(FakeLiveKitAdapter.instances) == 2
    assert authority.owners == {"redis-recovery-room": "worker-a"}
    await registry.close()
...
@pytest.mark.asyncio
async def test_media_write_is_fenced_after_another_worker_takes_lease(monkeypatch) -> None:
    FakeLiveKitAdapter.instances.clear()
    monkeypatch.setattr(settings, "livekit_audio_source_queue_ms", 40)
    monkeypatch.setattr(livekit_audio_module, "LIVEKIT_PUBLISHER_FENCE_CACHE_SECONDS", 0)
    authority = SharedPublisherLeaseAuthority()
    registry = LiveKitAudioRegistry(
        adapter_factory=FakeLiveKitAdapter,
        lease_authority=authority,
    ...
    generation = uuid.uuid4().hex
    await registry.start_generation("fenced-room", "speech", generation, 24_000)

    authority.owners["fenced-room"] = "worker-b"
    with pytest.raises(LiveKitPublisherLeaseConflict, match="ownership lost"):
        await registry.write_pcm("fenced-room", generation, pcm([8_000] * 960))

    assert FakeLiveKitAdapter.instances[0].disconnected is True
    assert "fenced-room" not in registry._publishers
    await registry.close()


@pytest.mark.asyncio
async def test_pause_clears_generation_but_terminal_close_releases_publisher_lease(monkeypatch) -> None:
    FakeLiveKitAdapter.instances.clear()
    monkeypatch.setattr(settings, "livekit_audio_source_queue_ms", 40)
    authority = SharedPublisherLeaseAuthority()
    registry = LiveKitAudioRegistry(
    ...
    publisher = await registry.ensure_room("pause-terminal-room")
    await registry.start_generation("pause-terminal-room", "speech", generation, 24_000)
    assert await registry.abort_generation("pause-terminal-room", generation) is True
    assert authority.owners == {"pause-terminal-room": "worker-a"}
    assert publisher.active_generation is None

    await registry.close_room("pause-terminal-room")
    assert authority.owners == {}
    assert FakeLiveKitAdapter.instances[0].disconnected is True
    assert authority.release_calls == [("pause-terminal-room", "worker-a")]
...
@pytest.mark.asyncio
async def test_paused_room_keeps_the_same_publisher_track_for_resume(monkeypatch) -> None:
    FakeLiveKitAdapter.instances.clear()
    monkeypatch.setattr(settings, "livekit_audio_source_queue_ms", 40)
    registry = LiveKitAudioRegistry(adapter_factory=FakeLiveKitAdapter)

    running_publisher = await registry.ensure_room("pause-resume-room")
    running_track_sid = running_publisher.track_sid
    await registry.close_inactive_rooms({"pause-resume-room"})
    resumed_publisher = await registry.ensure_room("pause-resume-room")

    assert "paused" in match_engine_module.MatchEngine._LIVEKIT_ROOM_STATUSES
    assert resumed_publisher is running_publisher
    assert resumed_publisher.track_sid == running_track_sid
    assert len(FakeLiveKitAdapter.instances) == 1
    assert FakeLiveKitAdapter.instances[0].disconnected is False
    await registry.close()
...
def test_moss_livekit_start_buffer_configuration_is_frame_aligned_and_fits_app_queue() -> None:
    configured = Settings()
    assert configured.moss_tts_livekit_start_buffer_ms == 800
...
async def test_engine_owns_prewarm_and_terminal_room_reconciliation(monkeypatch) -> None:
    async def ensure_room(room_code: str):
        if room_code == "paused-room":
            raise LiveKitPublisherLeaseConflict("owned by worker-b")
        return SimpleNamespace(room_code=room_code)

    ensure_room_mock = AsyncMock(side_effect=ensure_room)
    close_inactive = AsyncMock()
    monkeypatch.setattr(settings, "webrtc_audio_enabled", True)
    monkeypatch.setattr(settings, "webrtc_audio_backend", "livekit")
    monkeypatch.setattr(match_engine_module.livekit_audio_registry, "ensure_room", ensure_room_mock)
    monkeypatch.setattr(match_engine_module.livekit_audio_registry, "close_inactive_rooms", close_inactive)

    local_codes = await match_engine_module.MatchEngine()._reconcile_livekit_publishers(
        {"active-room", "paused-room"}
    )

    assert {call.args[0] for call in ensure_room_mock.await_args_list} == {"active-room", "paused-room"}
    close_inactive.assert_awaited_once_with({"active-room", "paused-room"})
    assert local_codes == {"active-room"}
