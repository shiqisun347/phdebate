...
async def test_ai_caption_writer_publishes_clauses_without_mutating_transcript(
    ...
    await writer.submit("第一句已经完整。")
    await writer.feed("第二句仍在")
    ...
    assert [item["timing_basis"] for item in published] == ["estimated_playback", "agent_text"]
...
def test_stable_wav_caption_projection_is_timed_idempotent_and_participant_only(
    register_user,
) -> None:
    owner = register_user("round20_stable_caption_owner")
    outsider = register_user("round20_stable_caption_outsider")
    code = create_training_room(owner, "稳定 WAV 逐句字幕")['code']
    content = "第一句说明核心观点。第二句补充关键证据，而且不能提前整篇出现。最后一句作出结论。"
    with SessionLocal() as db:
        room = load_room(db, code, lock=True)
        room.status = "running"
        match = Match(room_id=room.id, competition_id=room.competition_id, status="running")
        ...
        speech = Speech(
            ...
            content=content,
            duration_seconds=12,
            playback_started_at=room.created_at,
        )
        db.add(speech)
        db.flush()
        created = sync_audio_timed_agent_captions(db, room, speech, content, 12)
        db.commit()
        speech_id = speech.id
    ...
    with SessionLocal() as db:
        ...
        assert items[0].presentation_offset_ms == 0
        assert [item.presentation_offset_ms for item in items] == sorted(item.presentation_offset_ms for item in items)
        assert items[-1].presentation_offset_ms < 12_000
        assert all(item.timing_basis == "audio_duration" and item.source == "agent" for item in items)

        # A retry of the same immutable speech replaces the projection rather
        # than leaving late clauses that could reappear after an interruption.
        sync_audio_timed_agent_captions(db, room, speech, "重试后只保留这一句。", 3)
        db.commit()
        refreshed = list(
            db.scalars(
                select(CaptionSegment)
        ...
        assert [item.text for item in refreshed] == ["重试后只保留这一句。"]
        assert refreshed[0].presentation_offset_ms == 0

    participant_room = owner.get(f"/api/rooms/{code}").json()["room"]
    spectator_room = outsider.get(f"/api/rooms/{code}").json()["room"]
    anonymous_room = owner.get(f"/api/rooms/{code}/public").json()["room"]
    assert [item["text"] for item in participant_room["caption_segments"]] == ["重试后只保留这一句。"]
    for projected in (spectator_room, anonymous_room):
        assert projected["can_view_transcript"] is False
        assert projected["caption_segments"] == []
