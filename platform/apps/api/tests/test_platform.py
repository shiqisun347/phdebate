...
def test_legacy_cookie_session_is_upgraded_without_logging_the_user_out(register_user) -> None:
    user = register_user("cookie_upgrade")
    session_token = user.cookies.get("jixia_session")
    csrf_token = user.cookies.get("jixia_csrf")
    assert session_token and csrf_token
    user.cookies.delete("jixia_session")
    user.cookies.delete("jixia_csrf")
    ...
    upgraded = user.get("/api/auth/session-state")
    assert upgraded.status_code == 200
    assert upgraded.json()["user"]["account"].startswith("cookie_upgrade")
    assert user.cookies.get("jixia_session") == session_token
    assert user.cookies.get("jixia_csrf") == csrf_token
    assert user.cookies.get("jixia_v2_session") is None
    assert user.cookies.get("jixia_v2_csrf") is None

    room = create_training_room(user, "唯一版本 Cookie 平滑迁移")
...
def test_admin_data_quality_separates_production_coverage_from_qa_data(client: TestClient, register_user) -> None:
    ...
    with admin:
        ...
        refreshed_sample = next(
            item for item in refreshed["attention"]["samples"] if item["speech_id"] == broken_speech_id
        )
