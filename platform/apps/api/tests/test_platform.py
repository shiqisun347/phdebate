...
from app.services.transient_audio import cleanup_transient_match_audio
...
def asr_authentication(speech_id: str, *, lease: str = "test-lease", stage_key: str | None = None) -> dict:
...
def asr_ready(speech_id: str, *, stage_key: str | None = None) -> dict:
    if stage_key is None:
        with SessionLocal() as db:
            speech = db.get(Speech, speech_id)
            assert speech is not None
...
def test_system_allows_only_five_simultaneously_open_rooms_and_releases_capacity(register_user, monkeypatch) -> None:
    with SessionLocal() as db:
    ...
    released = owners[released_index].post(f"/api/rooms/{released_code}/cancel", headers=csrf(owners[released_index]), json={})
    ...
    owners[blocked_index].post(f"/api/rooms/{replacement['code']}/cancel", headers=csrf(owners[blocked_index]), json={})
...
def test_public_catalog_and_registration(client: TestClient, register_user) -> None:
    ...
    assert anonymous_session.json() == {"user": None, "active_room": None}
    ...
    assert session_state.json()["active_room"] is None

    active_room = create_training_room(user, "会话返回当前比赛")
    active_session_state = user.get("/api/auth/session-state")
    assert active_session_state.status_code == 200
    assert active_session_state.json()["active_room"] == {
        "code": active_room["code"],
        "topic": "会话返回当前比赛",
        "status": "lobby",
        "seat_key": "aff_1",
    }
...
def test_retired_versioned_cookie_is_not_accepted(register_user) -> None:
    user = register_user("retired_cookie")
    session_token = user.cookies.get("jixia_session")
    csrf_token = user.cookies.get("jixia_csrf")
    assert session_token and csrf_token
    user.cookies.clear()
    user.cookies.set("jixia_v2_session", session_token, domain="testserver.local", path="/")
    user.cookies.set("jixia_v2_csrf", csrf_token, domain="testserver.local", path="/")

    rejected = user.get("/api/auth/session-state")
    assert rejected.status_code == 200
    assert rejected.json()["user"] is None
    assert user.cookies.get("jixia_session") is None
    assert user.cookies.get("jixia_csrf") is None
...
def test_public_catalog_aggregates_live_room_counts_and_room_metadata(client: TestClient, register_user) -> None:
    ...
    with SessionLocal() as db:
        rooms = list(
            db.scalars(
                select(Room).where(Room.code.in_([first_room["code"], second_room["code"], third_room["code"], fourth_room["code"]]))
            ).all()
        )
        assert len(rooms) == 4
...
def test_room_projection_labels_completed_caption_as_previous_speech(client: TestClient, register_user) -> None:
    ...
    assert speech["speaker"] == "上一段发言 · 正方一辩"
...
def test_runtime_exposes_only_competition_scoped_product_domains(client: TestClient, register_user) -> None:
    ...
    retired_audio_route = f"/api/rooms/{room['code']}/speech/retired/audio"
    assert user.post(retired_audio_route, headers=csrf(user)).status_code == 404
    assert not any(
        path.endswith("/speech/{speech_id}/audio")
...
@pytest.mark.asyncio
async def test_retired_speech_audio_compatibility_call_fails_closed_without_writing(tmp_path: Path, monkeypatch) -> None:
    monkeypatch.setattr(settings, "media_root", str(tmp_path / "audio"))

    with pytest.raises(HTTPException) as rejected:
        await rooms_api.upload_speech_audio("123456", "speech-id", object())

    assert rejected.value.status_code == 410
    assert "只保存文字记录" in str(rejected.value.detail)
...
def test_readiness_checks_schema_redis_engine_backup_and_storage(client: TestClient, monkeypatch, tmp_path) -> None:
    ...
    monkeypatch.setenv("AGENT_BACKUP_STATUS_FILE", str(tmp_path / "agent-backup-status.json"))
    monkeypatch.setenv("HEALTH_DISK_FAILURE_FREE_PERCENT", "2")
    monkeypatch.setattr(system_health_service, "expected_schema_revision", lambda: "test-head")
    monkeypatch.setattr(
        system_health_service,
        "release_provenance",
        lambda: {"ok": True, "release": "test", "source_commit": "a" * 40, "source_tree": "b" * 40},
    )
    ...
    (tmp_path / "agent-backup-status.json").write_text(
        json.dumps({"last_success_at": now().isoformat(), "bytes": 67890, "state": "succeeded"}),
        encoding="utf-8",
    )
    with SessionLocal() as db:
    ...
    assert payload["checks"]["agent_backup"]["ok"] is True
        ...
        json.dumps(
            {
                ...
                "state": "failed",
                "error_code": "backup_command_failed",
                "bytes": 67890,
            }
        ),
        encoding="utf-8",
    )
    recent_failure = client.get("/api/health/ready")
    assert recent_failure.status_code == 200
    agent_backup = recent_failure.json()["checks"]["agent_backup"]
    assert agent_backup["ok"] is True
    assert agent_backup["warning"] is True
    assert agent_backup["status"] == "failed"
...
async def test_same_event_loop_conflicting_claims_release_failed_transactions(client: TestClient, register_user) -> None:
    ...
    headers = {"X-CSRF-Token": cookies["jixia_csrf"]}
...
def test_participant_cannot_abandon_started_match_for_ai_and_keeps_human_assignment(client: TestClient, register_user) -> None:
    ...
    assert abandoned.status_code == 410, abandoned.text
    assert replayed.status_code == 410, replayed.text
    ...
    with SessionLocal() as db:
        ...
        assert not db.scalar(
            select(MatchEvent.id).where(
                ...
                MatchEvent.event_type.in_(("seat.abandoned", "seat.ai_substituted")),
            )
        )
...
def test_ai_takeover_endpoint_remains_retired_even_while_human_speech_is_active(client: TestClient, register_user) -> None:
    ...
    assert "取消 AI 自动接管" in blocked.json()["detail"]
    with SessionLocal() as db:
        room = load_room(db, code)
        seat = next(item for item in room.seats if item.seat_key == "neg_1")
        active = db.scalar(
...
@pytest.mark.asyncio
async def test_anonymous_spectator_releases_database_session_before_redis_reservation(register_user, monkeypatch) -> None:
...
async def test_disconnect_during_asr_ready_releases_stream_without_server_error(register_user) -> None:
    ...
    class DisconnectingAsrWebSocket:
        ...
        async def receive_json(self) -> dict:
            return asr_authentication(speech_id)
...
def test_asr_websocket_rejects_stale_device_lease(client: TestClient, register_user) -> None:
    ...
    with pytest.raises(WebSocketDisconnect) as rejected:
        with owner.websocket_connect(f"/ws/rooms/{code}/asr") as socket:
            socket.send_json(asr_authentication(speech_id, lease="stale-device"))
            socket.receive_json()
    assert rejected.value.code == 4409
...
@pytest.mark.parametrize(
    ("field", "value"),
    [("speech_id", "speech-from-another-turn"), ("stage_key", "stage-from-another-turn")],
)
def test_asr_websocket_rejects_cross_turn_identity_before_upstream_connect(
    client: TestClient,
    register_user,
    monkeypatch,
    field: str,
    ...
    def unexpected_connect(*_args, **_kwargs):
        raise AssertionError("cross-turn ASR identity must be rejected before upstream connect")

    monkeypatch.setattr(realtime_api.websockets, "connect", unexpected_connect)
    owner = register_user(f"asr_cross_turn_{field}")
    code, speech_id = prepare_human_speech(owner)
    authentication = asr_authentication(speech_id)
...
def test_asr_websocket_accepts_current_device_lease(client: TestClient, register_user, monkeypatch) -> None:
    class FakeUpstream:
        def __init__(self) -> None:
    ...
    owner = register_user("asr_current_lease_v2")
    code, speech_id = prepare_human_speech(owner)
    with SessionLocal() as db:
        room = load_room(db, code)
        match = db.scalar(select(Match).where(Match.room_id == room.id))
    ...
    with owner.websocket_connect(f"/ws/rooms/{code}/asr") as socket:
        ...
        # ``ready`` is a real upstream readiness barrier, not merely the
        # browser-side socket opening.  The adapter handshake must have been
        # sent before the browser is allowed to flush its 20 ms PCM preroll.
        assert upstream.sent[:2] == ["START", "LANGUAGE:zh-CN"]
        assert ready == asr_ready(speech_id)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("missing_identity", {"speech_id": None}),
        ("bool_version", {"protocol_version": True}),
...
def test_asr_websocket_rejects_unsupported_wire_protocol_before_claiming_stream(
    ...
    with pytest.raises(WebSocketDisconnect) as rejected:
        with owner.websocket_connect(f"/ws/rooms/{code}/asr") as socket:
            socket.send_json(asr_authentication(speech_id) | metadata)
            assert socket.receive_json() == {
                "type": "error",
                "code": "unsupported_asr_protocol",
                "message": "语音识别协议不兼容，请刷新页面或使用文字补录。",
            }
            socket.receive_json()
...
def test_asr_allows_only_one_live_stream_per_speech(client: TestClient, register_user, monkeypatch) -> None:
    ...
    with owner.websocket_connect(f"/ws/rooms/{code}/asr") as first:
        first.send_json(asr_authentication(speech_id))
        assert first.receive_json() == asr_ready(speech_id)
        with pytest.raises(WebSocketDisconnect) as rejected:
            with owner.websocket_connect(f"/ws/rooms/{code}/asr") as second:
                second.send_json(asr_authentication(speech_id))
                second.receive_json()
        assert rejected.value.code == 4409


def test_asr_rejects_stream_already_claimed_by_another_api_worker(
    client: TestClient,
    register_user,
    monkeypatch,
) -> None:
    async def already_claimed(*_args, **_kwargs) -> bool:
        return False

    def unexpected_connect(*_args, **_kwargs):
        raise AssertionError("a duplicate cross-worker ASR stream must not reach FunASR")

    monkeypatch.setattr(realtime_api.room_hub, "asr_stream_claim", already_claimed)
    monkeypatch.setattr(realtime_api.websockets, "connect", unexpected_connect)
    owner = register_user("asr_cross_worker_stream")
    code, speech_id = prepare_human_speech(owner)
    ...
    with pytest.raises(WebSocketDisconnect) as rejected:
        with owner.websocket_connect(f"/ws/rooms/{code}/asr") as socket:
            ...
            assert socket.receive_json() == {
                "type": "error",
                "message": "该发言已在另一设备进行实时识别，请关闭重复页面。",
            }
            socket.receive_json()
...
def test_asr_empty_final_rejects_immediately_instead_of_waiting_for_timeout(
    client: TestClient,
    register_user,
    monkeypatch,
) -> None:
    class EmptyFinalUpstream:
        def __init__(self) -> None:
            self.messages: asyncio.Queue[str] | None = None
        ...
        async def send(self, payload: bytes | str) -> None:
            if payload == "STOP":
                assert self.messages is not None
                await self.messages.put(json.dumps({"is_final": True, "text": ""}))
    ...
    monkeypatch.setattr(realtime_api.websockets, "connect", lambda *args, **kwargs: EmptyFinalUpstream())
    owner = register_user("asr_empty_final")
    code, speech_id = prepare_human_speech(owner)
    voiced_pcm = ((4000).to_bytes(2, "little", signed=True) + (-4000).to_bytes(2, "little", signed=True)) * 4096
...
def test_asr_bridge_forwards_20ms_pcm_while_streaming_partial_results(
    client: TestClient,
    register_user,
    monkeypatch,
) -> None:
    class DuplexUpstream:
        def __init__(self) -> None:
            self.messages: asyncio.Queue[str] | None = None
            self.sent: list[bytes | str] = []
            self.audio_chunks = 0
        ...
        async def send(self, payload: bytes | str) -> None:
            ...
            if isinstance(payload, bytes):
                self.audio_chunks += 1
                if self.audio_chunks == 25:
                    assert self.messages is not None
                    await self.messages.put(json.dumps({"partial": "正在低延迟识别", "is_final": False}, ensure_ascii=False))
            elif payload == "STOP":
                assert self.messages is not None
                await self.messages.put(json.dumps({"text": "正在低延迟识别。", "is_final": True}, ensure_ascii=False))
    ...
    upstream = DuplexUpstream()
    monkeypatch.setattr(realtime_api.websockets, "connect", lambda *args, **kwargs: upstream)
    owner = register_user("asr_true_duplex_20ms")
    code, speech_id = prepare_human_speech(owner)
    # 16 kHz mono PCM16: one 20 ms browser frame is exactly 320 samples / 640 bytes.
    frame = ((4000).to_bytes(2, "little", signed=True) + (-4000).to_bytes(2, "little", signed=True)) * 160
    ...
    with owner.websocket_connect(f"/ws/rooms/{code}/asr") as socket:
        ...
        # A partial arrives before STOP while the same context remains open.
        socket.send_json({"type": "finish"})
        final = socket.receive_json()
        assert final["type"] == "asr" and final["is_final"] is True

    audio_chunks = [item for item in upstream.sent if isinstance(item, bytes)]
    assert len(audio_chunks) == 25
    assert all(len(item) == 640 for item in audio_chunks)


def test_asr_bridge_rejects_misaligned_pcm_before_forwarding(client: TestClient, register_user, monkeypatch) -> None:
    class BlockingUpstream:
        sent: list[bytes | str] = []
    ...
    monkeypatch.setattr(realtime_api.websockets, "connect", lambda *_args, **_kwargs: upstream)
    owner = register_user("asr_odd_pcm")
    code, speech_id = prepare_human_speech(owner)
    with pytest.raises(WebSocketDisconnect) as rejected:
        with owner.websocket_connect(f"/ws/rooms/{code}/asr") as socket:
            socket.send_json(asr_authentication(speech_id))
            ...
            socket.receive_json()
    assert rejected.value.code == 4400
    assert [item for item in upstream.sent if isinstance(item, bytes)] == []
...
def test_asr_failure_does_not_expose_internal_service_address(client: TestClient, register_user, monkeypatch) -> None:
    ...
    with owner.websocket_connect(f"/ws/rooms/{code}/asr") as socket:
        socket.send_json(asr_authentication(speech_id))
        error = socket.receive_json()
        assert error == {"type": "error", "message": "语音识别服务暂时不可用，请结束发言后使用文字补录。"}
...
def test_startup_presence_reset_marks_stale_humans_offline(register_user) -> None:
    ...
    with SessionLocal() as db:
        ...
        assert connected_before >= 1
        assert reset_connected_presence(db) == connected_before
        db.commit()
    with SessionLocal() as db:
...
async def test_distributed_presence_expiry_persists_disconnect_exactly_once(register_user, monkeypatch) -> None:
    ...
    with SessionLocal() as db:
        ...
        assert (now() - disconnected_at).total_seconds() >= 59.5
        ...
        assert disconnected.payload["confirmed_absence_seconds"] == min(
            realtime_service.PRESENCE_LEASE_SECONDS,
            60,
        )
        assert db.scalar(
            select(func.count(MatchEvent.id)).where(
...
def test_participant_ping_refreshes_presence_lease(client: TestClient, register_user, monkeypatch) -> None:
    owner = register_user("participant_ping_presence")
    room_data = create_training_room(owner, "辩手心跳续租测试")
    refreshed = AsyncMock(return_value=True)
    monkeypatch.setattr(realtime_service.room_hub, "presence_refresh", refreshed)
    monkeypatch.setattr(realtime_service.room_hub, "publish", AsyncMock())

    with owner.websocket_connect(f"/ws/rooms/{room_data['code']}") as socket:
        assert socket.receive_json()["type"] == "snapshot"
        # Let the sender consume its initial synchronization before isolating
        # the explicit browser heartbeat below.
        time.sleep(0.05)
    ...
    assert refreshed.await_args.args[:3] == (
        room_data["code"],
        room_data["my_seat"],
...
async def test_presence_expiry_releases_lobby_seat_and_pauses_running_human(register_user) -> None:
    ...
    running = create_training_room(owner2, "比赛断线后安全暂停测试")
...
@pytest.mark.asyncio
async def test_expired_stage_pauses_before_a_disconnected_human_can_be_replaced(register_user, monkeypatch) -> None:
    ...
    with SessionLocal() as db:
        ...
        assert seat.occupant_type == "human"
        assert room.status == "paused"
        assert room.current_stage_index == 0 and room.current_stage_index < len(room.template_snapshot)
        event_types = list(db.scalars(select(MatchEvent.event_type).where(MatchEvent.room_id == room.id).order_by(MatchEvent.seq)).all())
...
@pytest.mark.asyncio
async def test_presence_expiry_interrupts_abandoned_human_speech_and_pauses_without_ai_takeover(register_user, monkeypatch) -> None:
    ...
    with SessionLocal() as db:
        ...
        assert interrupted and interrupted.payload["reason"] == "participant_disconnected"
        assert not db.scalar(
            select(MatchEvent.id).where(
                MatchEvent.room_id == room.id,
                MatchEvent.event_type == "seat.ai_substituted",
...
def test_admin_room_inventory_uses_compact_bounded_queries(client: TestClient, register_user) -> None:
    ...
    with admin:
        ...
        try:
            ...
            event.remove(engine, "before_cursor_execute", track_selects)
        assert response.status_code == 200
        inventory_statements = [statement for statement in statements if "user_sessions" not in statement and "from users" not in statement]
        ...
        seat_query = next(statement for statement in inventory_statements if "room_seats.room_id in" in statement)
...
async def test_admin_audio_cue_is_reused_by_new_rooms_and_can_fall_back_to_lighttts(client: TestClient, register_user, monkeypatch) -> None:
    ...
    with admin:
        ...
        with SessionLocal() as db:
            ...
            assert not (settings.media_path / code / f"{asset.id}.wav").exists()
...
async def test_host_cue_locks_target_stage_without_changing_its_index(client: TestClient, register_user) -> None:
    ...
    with SessionLocal() as db:
        ...
        assert room_service_service.remaining_seconds(room) == 30
...
def test_admin_data_quality_separates_production_coverage_from_qa_data(client: TestClient, register_user) -> None:
    ...
    with SessionLocal.begin() as db:
        rooms = {
            item.code: item for item in db.scalars(select(Room).where(Room.code.in_([healthy["code"], broken["code"], qa["code"]]))).all()
        }
        matches = {
            item.room_id: item for item in db.scalars(select(Match).where(Match.room_id.in_([item.id for item in rooms.values()]))).all()
        }
    ...
    with admin:
        ...
        assert payload["speeches"]["transcript_coverage_percent"] == round(
            100 * (baseline["speeches"]["human_with_transcript"] + 1) / (baseline["speeches"]["human_completed"] + 2),
            1,
        )
        assert payload["speeches"]["audio_coverage_percent"] == round(
            100 * (baseline["speeches"]["human_with_audio"] + 1) / (baseline["speeches"]["human_completed"] + 2),
            1,
        )
        assert payload["attention"]["published_without_scorecard"] == baseline["attention"]["published_without_scorecard"] + 1
        assert payload["attention"]["published_without_speeches"] == baseline["attention"]["published_without_speeches"]
        ...
        assert payload["attention"]["unreviewed_sample_issues"] >= 3
        assert all(item["room_code"] != qa["code"] for item in payload["attention"]["samples"])

        disposition = admin.patch(
            f"/api/admin/data-quality/speeches/{broken_speech_id}/issues/missing_audio",
            headers=csrf(admin),
            json={"status": "needs_recollection", "note": "联系参赛者安排补采录音"},
        )
        assert disposition.status_code == 200, disposition.text
        assert disposition.json()["disposition"]["status"] == "needs_recollection"
        assert disposition.json()["disposition"]["note"] == "联系参赛者安排补采录音"

        refreshed = admin.get("/api/admin/data-quality").json()
        refreshed_sample = next(item for item in refreshed["attention"]["samples"] if item["speech_id"] == broken_speech_id)
        assert refreshed_sample["dispositions"]["missing_audio"]["status"] == "needs_recollection"
        assert refreshed["attention"]["unreviewed_sample_issues"] == payload["attention"]["unreviewed_sample_issues"] - 1

        revised = admin.patch(
            f"/api/admin/data-quality/speeches/{broken_speech_id}/issues/missing_audio",
            headers=csrf(admin),
            json={"status": "unrecoverable", "note": "原设备文件损坏，已完成原因核验"},
        )
        assert revised.status_code == 200, revised.text
        assert revised.json()["disposition"]["status"] == "unrecoverable"
        assert (
            admin.patch(
                f"/api/admin/data-quality/speeches/{broken_speech_id}/issues/not_a_real_issue",
                headers=csrf(admin),
                json={"status": "verified", "note": "无效问题代码"},
            ).status_code
            == 422
        )

    forbidden = broken_owner.patch(
        f"/api/admin/data-quality/speeches/{broken_speech_id}/issues/missing_audio",
        headers=csrf(broken_owner),
        json={"status": "verified", "note": "普通用户不能处置"},
    )
    assert forbidden.status_code == 403
...
def test_active_shared_host_cue_is_publicly_streamable(client: TestClient) -> None:
    cue_dir = settings.media_path / "_cues"
    cue_dir.mkdir(parents=True, exist_ok=True)
    filename = "shared-female-host-cue.wav"
    target = cue_dir / filename
    target.write_bytes(_wav_bytes(0.2))
    with SessionLocal() as db:
        db.add(
            AudioCue(
                key="shared_host_route_test",
                name="统一女声主持提示",
                text="现在进入下一环节。",
    ...
    response = client.get(f"/media/_cues/{filename}")
    assert response.status_code == 200
    assert response.headers["content-type"].startswith(("audio/wav", "audio/x-wav"))
    assert response.headers["cache-control"] == "public, max-age=86400, immutable"
    assert client.get("/media/_cues/not-referenced.wav").status_code == 404
...
def test_removed_seat_takeover_routes_and_projection_never_reappear(register_user) -> None:
    owner = register_user("retired_takeover_routes")
    code = create_training_room(owner, "真人断线仅暂停比赛")["code"]
    room = owner.get(f"/api/rooms/{code}").json()["room"]
    assert "seat_restore_requests" not in room
    assert "can_admin_restore" not in room

    assert owner.post(f"/api/rooms/{code}/seat-restore-requests", headers=csrf(owner), json={}).status_code == 404
    assert (
        owner.post(
            f"/api/rooms/{code}/seat-restore-requests/missing/approve",
            headers=csrf(owner),
            json={"reason": "retired"},
        ).status_code
        == 404
    ...
    assert (
        owner.post(
            f"/api/admin/rooms/{code}/seats/aff_1/restore",
            headers=csrf(owner),
            json={},
        ).status_code
        == 404
...
def test_expired_control_session_does_not_block_same_users_new_device(client: TestClient, register_user) -> None:
    """A stale browser session must not strand a human seat forever."""

    owner = register_user("expired_lease_owner")
    owner_identity = owner.get("/api/auth/session").json()["user"]
    second_device = TestClient(client.app)
    second_device.__enter__()
    try:
        ...
        code = create_training_room(owner, "过期设备控制租约应自动释放")["code"]
        owner.post(f"/api/rooms/{code}/ready", headers=csrf(owner), json={"ready": True})
        owner.post(f"/api/rooms/{code}/start", headers=csrf(owner), json={})
        with SessionLocal() as db:
            room = load_room(db, code, lock=True)
        ...
        acquired = owner.post(
            f"/api/rooms/{code}/control-lease",
            headers=csrf(owner) | {"X-Control-Lease": "stale-device"},
        )
        assert acquired.status_code == 200
        with SessionLocal.begin() as db:
            room = load_room(db, code, lock=True)
            seat = next(item for item in room.seats if item.user_id == owner_identity["id"])
            stale_session = db.get(UserSession, seat.control_session_id)
            assert stale_session is not None
            stale_session.expires_at = now() - timedelta(seconds=1)

        reclaimed = second_device.post(
            f"/api/rooms/{code}/control-lease",
            headers=csrf(second_device) | {"X-Control-Lease": "new-device"},
            json={"force": False},
        )
        assert reclaimed.status_code == 200, reclaimed.text
        assert reclaimed.json()["replayed"] is False
    finally:
        second_device.__exit__(None, None, None)
...
def test_same_browser_lease_rebinds_after_login_session_rotation(client: TestClient, register_user) -> None:
    owner = register_user("same_device_rotated_session")
    identity = owner.get("/api/auth/session").json()["user"]
    code = create_training_room(owner, "同一设备登录状态轮换后保持控制")["code"]

    first = owner.post(
        f"/api/rooms/{code}/control-lease",
        headers=csrf(owner) | {"X-Control-Lease": "stable-browser-device"},
        json={},
    )
    assert first.status_code == 200
    first_seq = first.json()["seq"]

    rotated_session = TestClient(client.app)
    with rotated_session:
        assert rotated_session.post(
            "/api/auth/login",
            json={"account": identity["account"], "password": "Password-1234"},
        ).status_code == 200
        recovered = rotated_session.post(
            f"/api/rooms/{code}/control-lease",
            headers=csrf(rotated_session) | {"X-Control-Lease": "stable-browser-device"},
            json={},
        )
        assert recovered.status_code == 200, recovered.text
        assert recovered.json()["replayed"] is True
        assert recovered.json()["same_session_recovery"] is False
        assert recovered.json()["same_device_recovery"] is True
        assert recovered.json()["seq"] == first_seq

        # Lobby writes now follow the rebound session instead of falsely
        # reporting that the same browser is another device.
        ready = rotated_session.post(
            f"/api/rooms/{code}/ready",
            headers=csrf(rotated_session),
            json={"ready": True},
        )
    ...
    with SessionLocal() as db:
        ...
        assert [event.event_type for event in control_events] == ["seat.control_acquired"]
...
def test_pause_resume_preserves_frozen_free_ai_preparation_clock(client: TestClient, register_user) -> None:
    ...
    assert paused_room["turn_remaining_seconds"] == 30
    ...
    assert resumed_room["turn_remaining_seconds"] in {29, 30}
...
def test_safe_pause_closes_active_human_speech(client: TestClient, register_user) -> None:
    owner = register_user("safe_pause_owner")
    code, speech_id = prepare_human_speech(owner)
    paused = owner.post(
        f"/api/rooms/{code}/control/safe-pause",
        headers=csrf(owner),
        json={"reason": "真人设备异常，紧急暂停"},
    )
    assert paused.status_code == 200, paused.text
    with SessionLocal() as db:
        assert db.get(Speech, speech_id).status == "interrupted"
        assert load_room(db, code).status == "paused"
...
def test_control_operation_is_idempotent(client: TestClient, register_user) -> None:
    ...
    with SessionLocal() as db:
        ...
        assert len(events) == 1
        assert db.get(Speech, speech_id).status == "completed"
...
def test_anonymous_realtime_event_hides_captions_and_keeps_flush_identity_only() -> None:
    ...
    caption_segment = room_service_service.anonymous_realtime_event(
        {
            "type": "caption.segment",
            "seq": 42,
            "text": "观众不应收到的逐句字幕",
            "speech_id": "internal-speech-id",
            "segment_id": "internal-caption-id",
            "start_ms": 1200,
            "is_final": True,
        }
    )
    assert caption_segment == {"type": "caption.segment", "seq": 42}
...
async def test_text_only_ai_wav_is_private_spool_deleted_after_livekit_playout(
    client: TestClient,
    register_user,
    monkeypatch,
) -> None:
    owner = register_user("ai_livekit_text_only")
    room_data = create_training_room(owner, "AI 临时 WAV 不进入比赛档案")
    code = room_data["code"]
    owner.post(f"/api/rooms/{code}/ready", headers=csrf(owner), json={"ready": True})
    owner.post(f"/api/rooms/{code}/start", headers=csrf(owner), json={})
    with SessionLocal() as db:
    ...
    generation = uuid.uuid4().hex
    spool_seen_during_playout = False

    async def generated_content(*_args, **_kwargs) -> str:
        return "这段语音只通过房间内唯一的实时音轨播放，赛后只留下文字。"

    async def one_shot_tts(*_args, room_code: str, speech_id: str, on_stream_event, **_kwargs) -> str:
        assert on_stream_event is None
        target = settings.media_path / room_code / f"{speech_id}.wav"
        target.parent.mkdir(parents=True, exist_ok=True)
        target.write_bytes(_wav_bytes(0.4))
        return f"/media/{room_code}/{speech_id}.wav"

    async def publish_wav(*, room_code: str, speech_id: str, should_cancel, on_stream_event) -> str:
        nonlocal spool_seen_during_playout
        spool = settings.media_path / room_code / f"{speech_id}.wav"
        spool_seen_during_playout = spool.is_file()
        assert should_cancel() is False
        await on_stream_event(
            {
                "type": "audio.stream.started",
                ...
                "sample_width": 2,
                "transport": "livekit",
                "track_sid": "TR_text_only",
                "server_first_capture_at": now().isoformat(),
            }
        )
        # Returning represents LiveKit wait_for_playout completion.
        return generation

    monkeypatch.setattr(settings, "match_audio_archive_enabled", False)
    monkeypatch.setattr(settings, "webrtc_audio_enabled", True)
    monkeypatch.setattr(settings, "webrtc_audio_backend", "livekit")
    monkeypatch.setattr(settings, "lighttts_streaming_enabled", False)
    monkeypatch.setattr(settings, "realtime_voice_pipeline_enabled", False)
    ...
    monkeypatch.setattr(lighttts, "synthesize", one_shot_tts)
    monkeypatch.setattr(lighttts, "publish_wav_to_livekit", publish_wav)
    monkeypatch.setattr(match_engine, "_schedule_next_agent_prefetch", lambda *_args: None)

    await match_engine.process_room(code)

    assert spool_seen_during_playout is True
    with SessionLocal() as db:
        room = load_room(db, code)
        speech = db.scalar(select(Speech).where(Speech.room_id == room.id, Speech.stage_key == "neg_1_case"))
        ready = db.scalar(
        ...
        assert speech and speech.content
        assert speech.audio_url == ""
        assert ready and ready.payload["audio_url"] == ""
        assert rtc and rtc.payload["audio_url"] == ""
...
async def test_moss_formal_speech_waits_for_complete_wav_then_publishes_stable_1_1x_audio(
    ...
    monkeypatch.setattr(
        match_engine,
        "_schedule_next_agent_prefetch",
        lambda _room_code, _stage_index: order.append("next-prefetch"),
    )
...
async def test_moss_completed_wav_is_available_during_livekit_playout_for_rtc_recovery(
    client: TestClient,
    register_user,
    monkeypatch,
) -> None:
    owner = register_user("moss_livekit_midplay_fallback")
    room_data = create_training_room(owner, "RTC 中途失败时使用同一稳定 WAV 接力测试")
    code = room_data["code"]
    owner.post(f"/api/rooms/{code}/ready", headers=csrf(owner), json={"ready": True})
    owner.post(f"/api/rooms/{code}/start", headers=csrf(owner), json={})
    with SessionLocal() as db:
    ...
    generation = uuid.uuid4().hex
    rtc_started = asyncio.Event()
    playout_finished = asyncio.Event()

    async def generated_content(*_args, **_kwargs) -> str:
        return "浏览器丢失实时音轨时，必须从同一个完整音频按真实起播时间继续。"
    ...
    async def publish_wav(*, room_code: str, speech_id: str, should_cancel, on_stream_event) -> str:
        # The immutable fallback must already be committed before LiveKit
        # publication starts, but it must not pretend to be audible yet.
        with SessionLocal() as db:
            target_room = load_room(db, room_code)
            prepared = db.get(Speech, speech_id)
            assert target_room.current_stage_index == 2
            assert prepared and prepared.status == "synthesizing"
            assert prepared.audio_url == f"/media/{room_code}/{speech_id}.wav"
            assert prepared.duration_seconds == pytest.approx(2.0, abs=0.02)
            assert prepared.playback_started_at is None
            prepared_captions = list(
                db.scalars(select(CaptionSegment).where(CaptionSegment.speech_id == speech_id).order_by(CaptionSegment.ordinal))
            )
            assert prepared_captions
            assert prepared_captions[0].presentation_offset_ms == 0
            assert all(item.timing_basis == "audio_duration" for item in prepared_captions)
            assert db.scalar(
                select(MatchEvent).where(
                    MatchEvent.room_id == target_room.id,
                    MatchEvent.event_type == "speech.audio.prepared",
    ...
    monkeypatch.setattr(settings, "moss_tts_realtime_enabled", True)
    monkeypatch.setattr(settings, "moss_tts_stable_playback_enabled", True)
    monkeypatch.setattr(settings, "moss_tts_playback_speed", 1.0)
    monkeypatch.setattr(settings, "webrtc_audio_enabled", True)
    monkeypatch.setattr(settings, "webrtc_audio_backend", "livekit")
    monkeypatch.setattr(debate_agent, "generate", generated_content)
    monkeypatch.setattr(moss_tts_realtime, "synthesize", complete_moss_wav)
    ...
    processing = asyncio.create_task(match_engine.process_room(code))
    await asyncio.wait_for(rtc_started.wait(), timeout=2)
    ...
    with SessionLocal() as db:
        ...
        assert target_room.current_stage_index == 2
        assert speech and speech.status == "playing"
        assert speech.audio_url == f"/media/{code}/{speech.id}.wav"
        assert speech.duration_seconds == pytest.approx(2.0, abs=0.02)
        assert speech.stream_generation == generation
        assert speech.stream_sample_rate == 24_000
        assert speech.playback_started_at is not None
        assert speech.playback_ends_at == speech.playback_started_at + timedelta(seconds=speech.duration_seconds)
        rtc_event = db.scalar(
            select(MatchEvent).where(
                MatchEvent.room_id == target_room.id,
                MatchEvent.event_type == "audio.rtc.started",
        ...
        assert rtc_event
        assert rtc_event.payload["audio_url"] == speech.audio_url
        assert rtc_event.payload["duration_seconds"] == speech.duration_seconds
        assert datetime.fromisoformat(rtc_event.payload["playback_started_at"]).replace(tzinfo=None) == (
            speech.playback_started_at.replace(tzinfo=None)
        )

        # Simulate a delayed engine tick after the nominal file endpoint. The
        # active generation proves LiveKit playout has not returned yet, so the
        # authoritative stage must remain fixed.
        target_room.stage_deadline_at = now() - timedelta(seconds=1)
        speech.playback_ends_at = now() - timedelta(seconds=1)
        db.commit()

    public_snapshot = client.get(f"/api/rooms/{code}/public")
    assert public_snapshot.status_code == 200
    public_room = public_snapshot.json()["room"]
    public_active = public_room["active_speech"]
    assert public_active["id"] == speech.id
    assert public_active["status"] == "playing"
    assert public_active["stream_generation"] == generation
    public_fallback = next(item for item in public_room["speeches"] if item["id"] == speech.id)
    assert public_fallback["audio_url"] == speech.audio_url
    assert public_fallback["duration_seconds"] == speech.duration_seconds
    assert public_fallback["playback_started_at"] is not None
    public_rtc = next(item for item in public_room["recent_events"] if item["type"] == "audio.rtc.started")
    assert public_rtc["payload"]["generation"] == generation
    assert public_rtc["payload"]["audio_url"] == speech.audio_url
    assert public_rtc["payload"]["duration_seconds"] == speech.duration_seconds
    assert datetime.fromisoformat(public_rtc["payload"]["playback_started_at"]).replace(tzinfo=None) == (
        datetime.fromisoformat(public_fallback["playback_started_at"]).replace(tzinfo=None)
    )

    await match_engine.process_room(code)
    with SessionLocal() as db:
        target_room = load_room(db, code)
        still_playing = db.get(Speech, speech.id)
        assert target_room.current_stage_index == 2
        assert still_playing and still_playing.status == "playing"
        assert still_playing.stream_generation == generation

    # Releasing the fake publisher represents the SDK's successful
    # wait_for_playout. Only after that confirmation may the engine unlock the
    # generation and let the next tick complete/advance the speech.
    playout_finished.set()
    await asyncio.wait_for(processing, timeout=2)
    with SessionLocal() as db:
        target_room = load_room(db, code, lock=True)
        played = db.get(Speech, speech.id)
        assert target_room.current_stage_index == 2
        assert played and played.status == "playing"
        assert played.stream_generation == ""
        target_room.stage_deadline_at = now() - timedelta(seconds=1)
        played.playback_ends_at = now() - timedelta(seconds=1)
        db.commit()

    await match_engine.process_room(code)
    with SessionLocal() as db:
        target_room = load_room(db, code)
        completed = db.get(Speech, speech.id)
        assert target_room.current_stage_index == 3
...
async def test_realtime_voice_pipeline_starts_tts_from_agent_delta_before_final(
    ...
    async def generated_stream(*_args, **_kwargs):
        yield {"type": "delta", "delta": "我方先提出第一点，核心标准是长期公共利益。"}
        await release_final.wait()
        yield {"type": "delta", "delta": "再补充第二点。"}
        yield {
            "type": "final",
            "content": "我方先提出第一点，核心标准是长期公共利益。再补充第二点。",
        }
    ...
    monkeypatch.setattr(settings, "realtime_voice_backend", "moss_realtime")
    monkeypatch.setattr(settings, "moss_tts_realtime_enabled", True)
    monkeypatch.setattr(settings, "moss_tts_realtime_url", "http://moss.test:8890")
    monkeypatch.setattr(settings, "moss_tts_stable_playback_enabled", False)
    monkeypatch.setattr(debate_agent, "generate_stream", generated_stream)
    monkeypatch.setattr(moss_tts_realtime, "open_incremental_session", open_session)
    ...
    with SessionLocal() as db:
        ...
        assert speech and speech.status == "synthesizing"
        assert speech.stream_generation == generation
        assert "".join(chunks) == "我方先提出第一点，核心标准是长期公共利益。"
        assert speech.content == ""
        rtc_started = db.scalar(
            select(MatchEvent).where(
                MatchEvent.room_id == target_room.id,
    ...
    with SessionLocal() as db:
        ...
        speech = db.scalar(select(Speech).where(Speech.room_id == target_room.id, Speech.stage_key == "neg_1_case"))
        assert speech and speech.status == "playing"
        # ``finish`` is the authoritative LiveKit playout-drained boundary for
        # the native incremental session.  Leaving this generation active would
        # make every later engine tick return without advancing the stage.
        assert speech.stream_generation == ""
        assert speech.stream_sample_rate == 0
        assert speech.content == "我方先提出第一点，核心标准是长期公共利益。再补充第二点。"
        assert speech.audio_url.endswith(f"/{speech.id}.wav")
        assert "".join(chunks) == "我方先提出第一点，核心标准是长期公共利益。再补充第二点。"
    ...
    await match_engine.process_room(code)
    with SessionLocal() as db:
        advanced_room = load_room(db, code)
        completed_speech = db.scalar(select(Speech).where(Speech.room_id == advanced_room.id, Speech.stage_key == "neg_1_case"))
        assert advanced_room.current_stage_index == 3
        assert completed_speech and completed_speech.status == "completed"
...
async def test_owner_resets_stuck_ai_playout_and_reuses_fixed_transcript(client: TestClient, register_user, monkeypatch) -> None:
    owner = register_user("reset_stuck_ai_playout")
    room_data = create_training_room(owner, "卡住的 AI 发言应由房主显式重置")
    code = room_data["code"]
    owner.post(f"/api/rooms/{code}/ready", headers=csrf(owner), json={"ready": True})
    owner.post(f"/api/rooms/{code}/start", headers=csrf(owner), json={})
    fixed_text = "这是一段已经固定、重置后必须原样重新合成的辩论发言。"
    original_deadline = now() + timedelta(seconds=2)
    ...
    with SessionLocal() as db:
        ...
        room.status = "running"
        room.current_stage_index = 0
        room.stage_started_at = now() - timedelta(seconds=28)
        room.stage_deadline_at = original_deadline
        seat = next(item for item in room.seats if item.seat_key == "neg_1")
        ...
        speech = Speech(
            ...
            stream_generation="a" * 32,
            stream_sample_rate=24_000,
            playback_started_at=now() - timedelta(seconds=5),
    ...
    reset = owner.post(
        f"/api/rooms/{code}/control/reset-speech",
        headers=csrf(owner) | {"X-Idempotency-Key": "reset-stuck-ai-once"},
        json={"reason": "声音完全卡住"},
    )
    assert reset.status_code == 200, reset.text
    assert reset.json()["room"]["status"] == "running"
    assert reset.json()["room"]["current_stage"]["key"] == "ai_case"

    with SessionLocal() as db:
        room = load_room(db, code)
        speech = db.get(Speech, speech_id)
        assert speech and speech.status == "reset_requested"
        assert speech.stream_generation == "" and speech.audio_url == ""
        assert room.stage_deadline_at
        restored_deadline = room.stage_deadline_at
        if restored_deadline.tzinfo is None:
            restored_deadline = restored_deadline.replace(tzinfo=original_deadline.tzinfo)
        assert restored_deadline > original_deadline
        assert match_engine._tts_reuse_source(db, room, room.template_snapshot[0]) == speech
        assert db.scalar(
            select(MatchEvent.id).where(
                MatchEvent.room_id == room.id,
                MatchEvent.event_type == "speech.reset_requested",
                MatchEvent.payload["reason"].as_string() == "owner_reset",
            )
        )
        assert db.scalar(
            select(MatchEvent.id).where(
                MatchEvent.room_id == room.id,
                MatchEvent.event_type == "control.reset-speech",
            )
        )

    resynthesized: list[str] = []

    class ResetRealtimeSession:
        async def push_text(self, text: str) -> None:
            resynthesized.append(text)

        async def finish(self) -> str:
            return "/media/mock/reset-recovered.wav"

        async def abort(self, reason: str = "") -> None:
            return None

    async def forbidden_agent(*_args, **_kwargs):
        raise AssertionError("fixed reset transcript must not call the Agent again")
        yield
    ...
    monkeypatch.setattr(settings, "lighttts_streaming_enabled", True)
    monkeypatch.setattr(settings, "lighttts_bistream_enabled", True)
    monkeypatch.setattr(lighttts, "open_incremental_session", lambda **_kwargs: ResetRealtimeSession())
    monkeypatch.setattr(debate_agent, "generate_stream", forbidden_agent)
    await match_engine.process_room(code)
    ...
    with SessionLocal() as db:
        ...
        assert "".join(resynthesized) == fixed_text
        assert [item.status for item in attempts] == ["reset_retried", "completed"]
        assert room.current_stage_index == 1

    result = owner.get(f"/api/rooms/{code}/result")
    assert result.status_code == 200, result.text
    assert result.json()["speech_pagination"]["total"] == 1
    assert [(item["id"], item["status"]) for item in result.json()["speeches"]] == [(attempts[1].id, "completed")]
...
async def test_engine_restart_preserves_completed_wav_and_revokes_only_orphaned_livekit_generation(
    client: TestClient,
    register_user,
) -> None:
    owner = register_user("engine_restart_livekit_fallback")
    room_data = create_training_room(owner, "引擎重启后稳定 WAV 继续播放测试")
    code = room_data["code"]
    owner.post(f"/api/rooms/{code}/ready", headers=csrf(owner), json={"ready": True})
    owner.post(f"/api/rooms/{code}/start", headers=csrf(owner), json={})
    generation = uuid.uuid4().hex
    ...
    with SessionLocal() as db:
        ...
        room.stage_deadline_at = started_at + timedelta(seconds=8)
        match = db.scalar(select(Match).where(Match.room_id == room.id))
        speech = Speech(
            match_id=match.id,
            room_id=room.id,
            seat_key="neg_1",
            stage_key=room.template_snapshot[2]["key"],
            speaker_type="ai",
            content="重启后浏览器仍应从同一个稳定音频继续。",
            audio_url=f"/media/{code}/restart-stable.wav",
            duration_seconds=8,
            playback_started_at=started_at,
            playback_ends_at=started_at + timedelta(seconds=8),
            stream_generation=generation,
            stream_sample_rate=24_000,
            status="playing",
        )
        ...
        db.commit()
        speech_id = speech.id
    target = settings.media_path / code / "restart-stable.wav"
    target.parent.mkdir(parents=True, exist_ok=True)
    target.write_bytes(_wav_bytes(8.0))

    recovered = recover_inflight_engine_tasks()
    assert recovered["speeches"] >= 1 and recovered["judges"] == 0
    ...
    with SessionLocal() as db:
        ...
        interrupted = db.scalar(
            select(MatchEvent).where(
                MatchEvent.room_id == room.id,
                MatchEvent.event_type == "audio.rtc.interrupt",
                MatchEvent.payload["speech_id"].as_string() == speech_id,
            )
        )
        assert speech and speech.status == "playing"
        assert speech.stream_generation == "" and speech.stream_sample_rate == 0
        assert speech.audio_url.endswith("/restart-stable.wav")
        assert speech.playback_started_at == started_at.replace(tzinfo=None)
        assert interrupted and interrupted.payload["generation"] == generation
        assert interrupted.payload["reason"] == "engine_restart"
...
async def test_pause_resume_invalidates_inflight_judge_result(client: TestClient, register_user, monkeypatch) -> None:
    ...
    with SessionLocal() as db:
        ...
        assert room.status == "judging" and match.status == "running" and match.winner is None
...
@pytest.mark.skip(reason="retired speech-audio archive surface")
...
def test_human_finish_preserves_chinese_punctuation_through_result_projection(client: TestClient, register_user) -> None:
    owner = register_user("chinese_punctuation")
    code, speech_id = prepare_human_speech(owner)
    content = "人工智能带来效率，也要求责任；我们应当谨慎、坚定地使用它！"
    ...
    result_speech = next(item for item in owner.get(f"/api/rooms/{code}/result").json()["speeches"] if item["id"] == speech_id)
    assert result_speech["content"] == content
    with SessionLocal() as db:
        speech = db.get(Speech, speech_id)
        segments = list(db.scalars(select(TranscriptSegment).where(TranscriptSegment.speech_id == speech_id)).all())
        event = db.scalar(
            select(MatchEvent).where(
                MatchEvent.room_id == speech.room_id,
                MatchEvent.event_type == "speech.completed",
            )
        )
        assert speech.content == content
        assert [item.text for item in segments] == [content]
        assert event and event.payload["content"] == content
...
@pytest.mark.skip(reason="retired speech-audio archive surface")
def test_text_only_policy_rejects_recording_before_multipart_parse_but_keeps_transcript(
    client: TestClient,
    register_user,
    monkeypatch,
) -> None:
    owner = register_user("text_only_upload")
    code, speech_id = prepare_human_speech(owner)
    finalized = owner.post(
        f"/api/rooms/{code}/speech/finish",
        headers=csrf(owner) | {"X-Control-Lease": "test-lease"},
        json={"speech_id": speech_id, "content": "只保存这段经过确认的文字记录。"},
    )
    assert finalized.status_code == 200

    async def multipart_must_not_run(*_args, **_kwargs):
        raise AssertionError("disabled recording route must reject before multipart parsing")

    monkeypatch.setattr(settings, "match_audio_archive_enabled", False)
    monkeypatch.setattr(MultiPartParser, "parse", multipart_must_not_run)
    rejected = owner.post(
        f"/api/rooms/{code}/speech/{speech_id}/audio",
        headers=csrf(owner),
        files={"audio": ("recording.wav", _wav_bytes(), "audio/wav")},
    )
    assert rejected.status_code == 410
    assert rejected.json()["detail"] == "比赛音频归档已关闭；系统只保存文字记录。"
    projected = owner.get(f"/api/rooms/{code}").json()["room"]
    assert projected["match_audio_archive_enabled"] is False
    assert owner.get("/api/health").json()["match_audio_archive_enabled"] is False

    with SessionLocal() as db:
        speech = db.get(Speech, speech_id)
        assert speech and speech.status == "completed"
        assert speech.content == "只保存这段经过确认的文字记录。"
        assert speech.audio_url == ""


def test_restart_cleanup_removes_spools_but_preserves_global_cues_and_legacy_references(
    client: TestClient,
    register_user,
    monkeypatch,
    tmp_path: Path,
) -> None:
    owner = register_user("text_only_cleanup")
    room = create_training_room(owner, "临时音频清理策略")
    code = room["code"]
    assert owner.post(f"/api/rooms/{code}/ready", headers=csrf(owner), json={"ready": True}).status_code == 200
    assert owner.post(f"/api/rooms/{code}/start", headers=csrf(owner), json={}).status_code == 200
    ...
    cue = tmp_path / "_cues" / "female-opening.wav"
    cue.parent.mkdir(parents=True)
    cue.write_bytes(_wav_bytes())
    room_dir = tmp_path / code
    room_dir.mkdir(parents=True)
    legacy = room_dir / "legacy.wav"
    orphan = room_dir / "orphan.wav"
    source = room_dir / "speech.source.wav"
    partial = room_dir / ".speech.generation.wav.part"
    for path in (legacy, orphan, source, partial):
        path.write_bytes(_wav_bytes())

    with SessionLocal() as db:
        stored_room = load_room(db, code)
        match = db.scalar(select(Match).where(Match.room_id == stored_room.id))
        speech = Speech(
            match_id=match.id,
            room_id=stored_room.id,
            seat_key="aff_1",
            stage_key="legacy",
            speaker_type="human",
            content="迁移前已有的录音引用只读保留。",
            status="completed",
            audio_url=f"/media/{code}/legacy.wav",
        )
        db.add(speech)
        db.commit()
        speech_id = speech.id
        result = cleanup_transient_match_audio(db)
    ...
    with SessionLocal() as db:
        speech = db.get(Speech, speech_id)
        assert speech and speech.audio_url == f"/media/{code}/legacy.wav"
        db.delete(speech)
        db.commit()
...
async def test_audio_upload_releases_database_transaction_while_streaming(client: TestClient, register_user) -> None:
    ...
    with SessionLocal() as db:
        ...
        audio = SlowAudio()
        result = await rooms_api.upload_speech_audio(code, speech_id, audio, user, db)  # type: ignore[arg-type]
...
def test_terminal_participant_can_create_idempotent_same_topic_rematch(client: TestClient, register_user) -> None:
    ...
    with SessionLocal() as db:
        ...
        created_event = db.scalar(select(MatchEvent).where(MatchEvent.room_id == rematch.id, MatchEvent.event_type == "room.created"))
...
def test_personal_history_and_result_events_are_paginated(client: TestClient, register_user) -> None:
    ...
    # Failed/interrupted synthesis attempts remain auditable events but must
    # never appear as completed debate speeches in the participant result.
    assert result_first.json()["speeches"] == []
