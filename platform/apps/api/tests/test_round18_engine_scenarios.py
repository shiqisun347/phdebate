...
@pytest.mark.asyncio
async def test_disconnect_grace_pauses_and_real_websocket_reconnect_allows_owner_resume(register_user) -> None:
    """A student keeps the seat and a real reconnect unlocks explicit resume."""

    owner = register_user("round18_restore_owner")
    returning = register_user("round18_restore_participant")
    code = create_training_room(owner, "Round18 断线暂停与真人恢复")["code"]
