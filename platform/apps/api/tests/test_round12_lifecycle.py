...
async def test_round12_complete_mixed_human_agent_lifecycle_is_recoverable_and_room_isolated(
    ...
    async def generated(payload, **_kwargs):
        ...
        return (
            f"房间{payload['room_code']}围绕《{payload['debate_topic']}》在{payload['current_stage']}给出独立、完整且可审计的论证。"
        )
    ...
    # installs an AI substitute. Its first Agent attempt fails and pauses only
    # this room, while a different room advances normally.
    with SessionLocal() as db:
    ...
    with SessionLocal() as db:
        ...
        substitute = next(item for item in ranked.seats if item.seat_key == "neg_1")
        assert ranked.status == "paused" and "暂时不可用" in ranked.failure_reason
        assert substitute.occupant_type == "ai_substitute" and substitute.user_id is not None
        assert skipped.status == "judging" and skipped.current_stage_index == 1

    retried = owner.post(
        f"/api/rooms/{ranked_code}/control/retry",
        headers=csrf(owner) | {"X-Idempotency-Key": "round12-agent-retry-once"},
        json={"reason": "Agent 健康检查恢复"},
    )
    ...
    with SessionLocal() as db:
        ...
        attempts = list(
            db.scalars(
                select(Speech).where(Speech.room_id == room.id, Speech.stage_key == "neg_case").order_by(Speech.created_at)
            ).all()
        )
        assert [item.status for item in attempts] == ["failed_retried", "completed"]
        assert attempts[-1].seat_key == "neg_1" and ranked_topic in attempts[-1].content
    ...
    # The disconnected participant returns, requests restoration, and is
    # approved by the room owner before taking the negative free-debate turn.
    with SessionLocal() as db:
        room = load_room(db, ranked_code, lock=True)
        returning_seat = next(item for item in room.seats if item.seat_key == "neg_1")
        returning_seat.connected = True
        db.commit()
    requested = returning_debater.post(
        f"/api/rooms/{ranked_code}/seat-restore-requests",
        headers=csrf(returning_debater) | {"X-Idempotency-Key": "round12-restore-once"},
        json={},
    )
    assert requested.status_code == 200, requested.text
    restore_id = requested.json()["request"]["id"]
    approved = owner.post(
        f"/api/rooms/{ranked_code}/seat-restore-requests/{restore_id}/approve",
        headers=csrf(owner),
        json={"reason": "原辩手已重新连接并完成设备检查"},
    )
    ...
    with SessionLocal() as db:
        ...
        for room, expected_topic in (
            ...
            assert all(
                expected_topic in speech.content
