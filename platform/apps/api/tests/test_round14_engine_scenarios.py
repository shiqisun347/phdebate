...
def test_four_rooms_race_for_seats_and_start_idempotently_without_cross_room_state(register_user) -> None:
    ...
    with SessionLocal() as db:
        ...
        scenario_codes = {item[3] for item in scenarios}
        for index, (_owner, _first, _second, code) in enumerate(scenarios):
            ...
            other_scenarios = db.scalars(
                select(Room).where(Room.code.in_(scenario_codes - {code}))
            ).all()
            assert room.topic == f"Round14 四房并发隔离辩题 {index}"
            assert all(other.topic != room.topic for other in other_scenarios)
