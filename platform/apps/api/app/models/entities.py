...
class SeatRestoreRequest(Base, TimestampMixin):
    """Retired audit rows from the removed seat-takeover workflow.

    No application service creates, updates, or projects these rows. The table
    remains mapped only so historical database snapshots stay inspectable.
    """
...
class VoiceTelemetry(Base):
    """Small, text-only timing record for one synthesized speech.

    Audio samples and transcript text deliberately do not belong here.  The
    record exists to locate latency and playout failures across the Agent,
    TTS, LiveKit and browser boundaries without retaining another media copy.
    """

    __tablename__ = "voice_telemetry"

    id: Mapped[str] = mapped_column(String(36), primary_key=True, default=uuid4)
    room_id: Mapped[str] = mapped_column(ForeignKey("rooms.id", ondelete="CASCADE"), index=True)
    match_id: Mapped[str] = mapped_column(ForeignKey("matches.id", ondelete="CASCADE"), index=True)
    speech_id: Mapped[str] = mapped_column(
        ForeignKey("speeches.id", ondelete="CASCADE"), unique=True, index=True
    )
    generation: Mapped[str] = mapped_column(String(64), default="")
    phases: Mapped[dict[str, Any]] = mapped_column(JSON, default=dict)
    network_summary: Mapped[dict[str, Any]] = mapped_column(JSON, default=dict)
    browser_report_count: Mapped[int] = mapped_column(Integer, default=0)
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), default=utcnow)
    updated_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), default=utcnow, onupdate=utcnow)
