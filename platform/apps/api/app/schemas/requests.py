...
class VoiceTelemetryReport(BaseModel):
    speech_id: str = Field(min_length=1, max_length=36)
    generation: str = Field(min_length=1, max_length=64)
    event: Literal["browser_first_audible", "browser_network_sample"]
    metrics: dict[str, float | int] = Field(default_factory=dict, max_length=20)
...
class RoomMatchConsistencyRepairRequest(BaseModel):
    expected_room_status: Literal["cancelled", "terminated", "completed", "review_required"]
    expected_match_status: str = Field(min_length=1, max_length=32)
    reason: str = Field(min_length=2, max_length=300)

    @field_validator("reason")
    @classmethod
    def normalize_repair_reason(cls, value: str) -> str:
        normalized = " ".join(value.strip().split())
        if len(normalized) < 2:
            raise ValueError("补偿原因至少需要 2 个字符。")
        return normalized
...
class AutomationStageRequest(BaseModel):
    ...
    turn_duration: int | None = Field(default=None, ge=5, le=30)
...
class SpeechDataIssueDispositionPatch(BaseModel):
    status: Literal["needs_recollection", "unrecoverable", "verified"]
    ...
    def normalize_note(cls, value: str) -> str:
        normalized = " ".join(value.strip().split())
        if len(normalized) < 2:
            raise ValueError("处置说明至少需要 2 个字符。")
        return normalized
