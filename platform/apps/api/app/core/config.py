...
class Settings(BaseSettings):
    ...
    app_name: str = "Jixia Debate"
    app_env: str = "development"
    app_secret: str = "development-only-secret-change-me"
    database_url: str = "sqlite:///./storage/phdebate.db"
    ...
    moss_tts_realtime_speech_instruction: str = Field(
        default=("请用自信、坚定、有说服力的中文辩论语气，语速比自然朗读快约百分之十，重音清晰，停顿简短，句尾有力，但不要喊叫或夸张。"),
        min_length=10,
        max_length=500,
    )
    ...
    # The browser budget is three seconds from first Agent text to audible
    # playout.  LiveKit keeps an ~800 ms anti-underrun start buffer, so the
    # server still targets first PCM well below 2.2 s.  A 1.2 s hard cutoff was
    # lower than the measured warm RTX 3090 first chunk (~1.5 s) and killed
    # otherwise healthy turns before they could become audible.
    moss_tts_realtime_first_pcm_timeout_seconds: float = Field(default=2.5, ge=0.2, le=3)
    ...
    # Waiting behind another room is admission time, not synthesis time. Five
    # rooms sharing one endpoint may legitimately queue several 60-90 second
    # turns; each admitted turn still receives a fresh job budget below.
    moss_tts_realtime_queue_timeout_seconds: float = Field(default=600, ge=5, le=1800)
    moss_tts_realtime_job_timeout_seconds: float = Field(default=240, ge=5, le=300)
    # Formal matches use the native text-in/audio-out session and publish only
    # one continuous LiveKit track.  A complete-WAV preparation path is kept
    # only for old test fixtures and must never be the production default: a
    # stalled stream is reset by room control instead of switching players.
    moss_tts_stable_playback_enabled: bool = False
    ...
    # synthetic voices while remaining inexpensive for competition spectators.
    ...
    moss_tts_livekit_start_buffer_ms: int = Field(default=800, ge=120, le=2_400)
    ...
    # System announcements are authored once by an administrator and reused by
    ...
    # Match speech audio is transport data, not a durable competition record.
    # Keep this off by default: human/AI speech persists as text while fixed
    # administrator-authored system cues remain reusable media assets.  The
    # switch exists only so a legacy installation can continue serving its
    # already-established recording workflow during migration.
    match_audio_archive_enabled: bool = False
    ...
    # capacity slot.
    stale_paused_after_minutes: int = Field(default=120, ge=15, le=10080)
    ...
    def validate_lighttts_gate_deadlines(self) -> Settings:
        ...
        if self.moss_tts_realtime_enabled:
            endpoints = {item.strip().rstrip("/") for item in [*self.moss_tts_realtime_urls, self.moss_tts_realtime_url] if item.strip()}
            if not endpoints:
            ...
            if any(Path(prompt).name != prompt or not prompt.lower().endswith(".wav") for prompt in self.moss_tts_prompt_files.values()):
