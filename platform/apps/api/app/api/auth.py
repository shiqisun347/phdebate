...
def _upgrade_legacy_cookies(request: Request, response: Response) -> None:
    if request.cookies.get(SESSION_COOKIE):
        return
    legacy_session = request.cookies.get(LEGACY_SESSION_COOKIE)
    legacy_csrf = request.cookies.get(LEGACY_CSRF_COOKIE)
    if not legacy_session or not legacy_csrf:
        return
    cookie_common = dict(max_age=settings.session_days * 86400, secure=settings.cookie_secure, samesite="lax", path="/")
    response.set_cookie(SESSION_COOKIE, legacy_session, httponly=True, **cookie_common)
    response.set_cookie(CSRF_COOKIE, legacy_csrf, httponly=False, **cookie_common)
    response.delete_cookie(LEGACY_SESSION_COOKIE, path="/")
    response.delete_cookie(LEGACY_CSRF_COOKIE, path="/")
...
def session(
    ...
    _upgrade_legacy_cookies(request, response)
...
def session_state(
    ...
    if user:
        _upgrade_legacy_cookies(request, response)
    return {"user": _serialized_user(db, user) if user else None}
