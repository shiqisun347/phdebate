...
ASR_MAX_PCM_MESSAGE_BYTES = 64 * 1024
...
def _asr_protocol_supported(authentication: dict) -> bool:
    if set(authentication) - ASR_AUTHENTICATION_FIELDS:
        return False
    if ASR_PROTOCOL_FIELDS.intersection(authentication) != ASR_PROTOCOL_FIELDS:
        return False
    return (
        type(authentication.get("protocol_version")) is int
        and authentication["protocol_version"] == ASR_PROTOCOL_V2["protocol_version"]
        and authentication.get("encoding") == ASR_PROTOCOL_V2["encoding"]
        and type(authentication.get("channels")) is int
        and authentication["channels"] == ASR_PROTOCOL_V2["channels"]
        and type(authentication.get("sample_rate")) is int
        and authentication["sample_rate"] == ASR_PROTOCOL_V2["sample_rate"]
        and isinstance(authentication.get("speech_id"), str)
        and bool(authentication["speech_id"].strip())
        and len(authentication["speech_id"]) <= 64
        and isinstance(authentication.get("stage_key"), str)
        and bool(authentication["stage_key"].strip())
        and len(authentication["stage_key"]) <= 128
    )
...
def persist_asr_final(
    ...
    *,
    ...
    with SessionLocal() as db:
        # Serialize a final ASR write with room-engine interruption of the same
        # speech.  Without a row lock, the bridge could read ``speaking``, the
        # 60-second disconnect path could commit ``interrupted``, and the ASR
        # transaction could then append text after the authoritative pause.
        speech = db.scalar(select(Speech).where(Speech.id == speech_id).with_for_update())
...
def _asr_stream_active(
    ...
    *,
    ...
    with SessionLocal() as db:
        ...
        current_stage = None
        if room and 0 <= room.current_stage_index < len(room.template_snapshot or []):
            current_stage = room.template_snapshot[room.current_stage_index]
        return bool(
            session
            and session.user_id == user.id
            and as_utc(session.expires_at) > datetime.now(timezone.utc)
            ...
            and current_stage
            and speech.stage_key == current_stage.get("key")
        )
...
async def room_websocket(websocket: WebSocket, code: str) -> None:
    ...
    presence_connection_id = spectator_connection_id(code, spectator_ticket) if not user and spectator_ticket else uuid.uuid4().hex
    ...
    async def synchronize_presence() -> list[tuple[str, int]]:
        ...
        if joined_presence:
            ...
            if await room_hub.presence_leave(code, old_seat_key, old_user_id, connection_id=presence_connection_id):
                with SessionLocal() as db:
    ...
    async def sender() -> None:
        ...
        async for message in prepared_messages():
            ...
            if joined_as_spectator and not await room_hub.spectator_refresh(code, connection_id=presence_connection_id):
                await close_socket(4429)
                return
    ...
    async def receiver() -> None:
        while True:
            ...
            if data.get("type") == "ping":
                ...
                if joined_presence:
                    seat_key, user_id = joined_presence
                    refreshed = await room_hub.presence_refresh(
                        code,
                        seat_key,
                        user_id,
                        connection_id=presence_connection_id,
                    ...
                    if not refreshed:
                        # Redis or the process-local lease registry may have
                        # restarted while the authenticated socket stayed
                        # alive. Re-register this same connection instead of
                        # allowing a healthy participant to be reaped as
                        # offline during a quiet stage.
                        await room_hub.presence_join(
                            code,
                            seat_key,
                            user_id,
...
async def asr_websocket(websocket: WebSocket, code: str) -> None:
    ...
    with SessionLocal() as db:
        ...
        if not speech:
            ...
            return
        speech_id = speech.id
        speech_stage_key = speech.stage_key
        room_id = room.id
    ...
    if authentication.get("type") != "authenticate" or not control_lease or len(control_lease) > 64:
        await close_socket(4409)
        return
    if not _asr_protocol_supported(authentication):
        ...
        return
    if (
        str(authentication.get("speech_id") or "").strip() != speech_id
        or str(authentication.get("stage_key") or "").strip() != speech_stage_key
    ...
    if not _claim_asr_stream(speech_id):
        await close_socket(4409)
        return
    distributed_owner = uuid.uuid4().hex
    distributed_claimed = False
    distributed_claim = await room_hub.asr_stream_claim(speech_id, owner=distributed_owner)
    if distributed_claim is False or (distributed_claim is None and settings.app_env != "test"):
            ...
            await send_json(
                {
                    "type": "error",
                    "message": (
                        "该发言已在另一设备进行实时识别，请关闭重复页面。"
                        if distributed_claim is False
                        else "实时字幕调度暂时不可用，请结束发言后核对并补充文字。"
                    ),
                }
            )
        except WebSocketDisconnect:
            pass
        await close_socket(4409 if distributed_claim is False else 1013)
        return
    distributed_claimed = distributed_claim is True
    ...
    try:
        # Do not advertise readiness until the upstream ASR WebSocket has
        # completed its handshake.  Previously ``ready`` was sent before the
        # connection was opened, which made the browser flush its preroll into
        # a socket that could still fail and produced a misleading "live"
        # subtitle state during reconnects.  The bridge remains genuinely
        # duplex after this point: browser PCM is forwarded while upstream
        # partial/final messages are read concurrently below.
        async with websockets.connect(asr_endpoint, max_size=8 * 1024 * 1024) as upstream:
            ...
            await send_json(
                {
                    "type": "ready",
                    "speech_id": speech_id,
                    "stage_key": speech_stage_key,
                    "protocol_version": ASR_PROTOCOL_V2["protocol_version"],
                }
            )
                ...
                while True:
                    ...
                    if current_time - last_validation >= 1:
                        ...
                        if distributed_claimed:
                            refreshed = await room_hub.asr_stream_refresh(
                                speech_id,
                                owner=distributed_owner,
                            )
                    ...
                    if message.get("bytes") is not None:
                        ...
                        if (
                            ...
                            or not audio_bytes
                            or len(audio_bytes) > ASR_MAX_PCM_MESSAGE_BYTES
                        ...
                        if control == {"type": "finish"}:
                            if distributed_claimed:
                                refreshed = await room_hub.asr_stream_refresh(
                                    speech_id,
                                    owner=distributed_owner,
                                    lease_seconds=final_wait_seconds + 5,
                                )
                                if refreshed is False:
                                    await close_socket(4409)
                                    return
            ...
            async def asr_to_browser() -> None:
                ...
                async for raw in upstream:
                    ...
                    # A valid 2-pass ASR session can finish without any text
                    # (silence, clipped speech, or a provider-side VAD reject).
                    # Treat that as an authoritative final immediately.  The
                        ...
                        await send_json(
                            {
                                "type": "asr_rejected",
                                "room_code": code,
                                "reason": "silence" if not voice_activity.has_voice else "empty",
                                "seat_key": seat_key,
                                "speech_id": speech_id,
                            }
                        )
                    ...
                    if text:
                        ...
                        if is_final:
                            ...
                            if not persisted:
                                # The authoritative speech may have been
                                # interrupted between the validation above and
                                # this final write (for example by the 60-second
                                # disconnect pause). Never publish a final
                                # subtitle that the match record rejected.
                                await send_json(
                                    {
                                        "type": "asr_rejected",
                                        "room_code": code,
                                        "reason": "speech_inactive",
                                        "seat_key": seat_key,
                                        "speech_id": speech_id,
                                    }
                                )
        ...
        try:
            await send_json({"type": "error", "message": "语音识别服务暂时不可用，请结束发言后使用文字补录。"})
        except WebSocketDisconnect:
            logger.debug("ASR error could not be delivered to the browser", exc_info=True)
    finally:
        if distributed_claimed:
            await room_hub.asr_stream_release(speech_id, owner=distributed_owner)
        _release_asr_stream(speech_id)
        await close_socket()
