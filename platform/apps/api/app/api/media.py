...
from app.models.entities import AudioAsset, AudioCue, Match, Speech, User
from app.services.room_service import can_view_room, load_room

router = APIRouter(tags=["media"])


@router.get("/media/_cues/{filename}")
def preset_cue_media(filename: str, db: Session = Depends(get_db)) -> FileResponse:
    """Serve only active administrator-authored host cues.

    ``/media/{code}/{filename}`` treats its first segment as a room code.  The
    shared ``_cues`` namespace therefore needs an explicit route before the
    generic room handler; otherwise every valid preset is misreported as a
    missing room and browsers surface a misleading decode error.
    """

    if Path(filename).name != filename or filename.endswith(".part"):
        raise HTTPException(status_code=404, detail="音频文件不存在。")
    storage_key = f"/media/_cues/{filename}"
    referenced = db.scalar(
        select(AudioCue.id).where(
            AudioCue.audio_url == storage_key,
            AudioCue.is_active.is_(True),
        )
    )
    if not referenced:
        raise HTTPException(status_code=404, detail="音频文件不存在。")
    cue_directory = (settings.media_path / "_cues").resolve()
    target = cue_directory / filename
    if target.is_symlink() or target.resolve().parent != cue_directory or not target.is_file():
        raise HTTPException(status_code=404, detail="音频文件不存在。")
