...
FREE_TURN_DEFAULT_SECONDS = 30
FREE_TURN_MAX_SECONDS = 30
ANONYMOUS_SPECTATOR_STATUSES = frozenset(
    {"lobby", "preparing", "running", "paused", "judging", "completed", "review_required", "terminated"}
)
...
def free_turn_duration(current: dict[str, Any] | None) -> int:
    """Return the product-wide bounded duration for one free-debate turn."""

    raw = (current or {}).get("turn_duration", FREE_TURN_DEFAULT_SECONDS)
    try:
        duration = int(raw)
    except (TypeError, ValueError):
        duration = FREE_TURN_DEFAULT_SECONDS
    return max(1, min(FREE_TURN_MAX_SECONDS, duration))
...
def remaining_seconds(room: Room) -> int | None:
    ...
    if current and current.get("host_announcement_pending"):
        ...
        if deadline:
            ...
            return 0 if remaining <= 0 else max(1, math.ceil(remaining))
    ...
    if current and current.get("kind") == "free" and not room.stage_deadline_at:
        frozen = current.get("free_stage_remaining_seconds")
        if frozen is not None:
            return max(0, int(frozen))
    if current and current.get("awaiting_human_start"):
        frozen = current.get("free_stage_remaining_seconds")
        if frozen is None:
            frozen = current.get("human_speech_duration_seconds")
        if frozen is None:
            frozen = current.get("duration", 30)
        return max(0, int(frozen))
    ...
    if deadline.tzinfo is None:
        deadline = deadline.replace(tzinfo=timezone.utc)
    # ``int`` truncates a positive fractional second to zero.  The engine
    # uses zero as the authoritative expiry signal, so a deadline 200 ms in
    # the future used to advance the stage almost a full second early (and
    # the UI displayed 00:00 while the stage was still live).  Keep zero
    # exclusive to an actually reached deadline and round a live countdown
    # up so the last visible second is not lost.
    remaining = (deadline - now()).total_seconds()
    return 0 if remaining <= 0 else max(1, math.ceil(remaining))
...
def free_turn_deadline(room: Room, current: dict[str, Any] | None = None) -> datetime | None:
    ...
    deadline = started_at + timedelta(seconds=free_turn_duration(current))
...
def free_turn_remaining_seconds(room: Room, current: dict[str, Any] | None = None) -> int | None:
    ...
    if room.status == "paused" and current and current.get("kind") == "free":
        ...
        return max(0, min(free_turn_duration(current), int(paused))) if paused is not None else None
    if current and current.get("kind") == "free" and current.get("awaiting_human_start"):
        turn_duration = free_turn_duration(current)
        resumed_turn = current.get("human_turn_remaining_seconds")
        if resumed_turn is not None:
            return max(0, min(turn_duration, int(resumed_turn)))
        return turn_duration
    if current and current.get("kind") == "free" and current.get("ai_preparing"):
        frozen = current.get("preparing_turn_remaining_seconds")
        return max(0, min(free_turn_duration(current), int(frozen))) if frozen is not None else None
    if current and current.get("kind") == "free" and not current.get("turn_started_at"):
        return free_turn_duration(current)
...
def transfer_room_owner(
    ...
    *,
    ...
    online room controller. Keeping ownership attached to an unavailable
    participant can leave every remaining human unable to pause, retry, or
    terminate the match. This helper keeps the ownership mutation and its
    append-only audit event inseparable; it never changes a participant seat
    into an AI seat.
    """
...
def can_view_room(db: Session, room: Room, user: User | None) -> bool:
    ...
    if room.visibility == "public":
        ...
        # Public waiting rooms expose only a redacted projection (the
        # serializer removes names/readiness), so a shared watch link can show
        # a useful waiting state without enumerating students.
...
def speaking_permission(
    ...
    *,
    ...
    if current.get("host_announcement_pending"):
        return False, "系统正在播放阶段提示"
    if remaining_seconds(room) == 0:
        return False, "当前环节已经结束，正在切换下一环节"
    stage_deadline = room.stage_deadline_at
    if stage_deadline:
        if stage_deadline.tzinfo is None:
            stage_deadline = stage_deadline.replace(tzinfo=timezone.utc)
        if stage_deadline <= now():
            return False, "当前环节已经结束，正在切换下一环节"
    if current.get("kind") == "free":
        turn_deadline = free_turn_deadline(room, current)
        if turn_deadline and turn_deadline <= now():
            return False, "本轮发言时间已经结束，正在切换下一位辩手"
    ...
    if current.get("kind") == "free":
        ...
        if current.get("force_ai_fallback"):
            has_ai_fallback = any(item.side == current.get("side") and item.occupant_type == "ai" for item in room.seats)
            return (
                False,
                "本轮无人申请，系统正在安排 AI 发言" if has_ai_fallback else "本轮无人申请，系统将自动交换发言方",
            )
...
def participant_disconnect_pause_context(db: Session, room: Room) -> tuple[bool, bool]:
    """Return the authoritative recovery mode after a 60-second disconnect.

    A provider failure can be appended after ``participant.disconnect_timeout``
    while the old engine task is unwinding.  Looking only at the newest event
    (the previous implementation did that in both the control API and the
    serializer) then made a disconnect pause look like an ordinary provider
    pause: ``resume`` was rejected, but ``retry`` could restart the stage while
    the human was still offline. Only a successful ``resume``/``retry``
    supersedes a timeout boundary. Reconnection merely makes that explicit
    owner/admin action eligible; it never resumes the match automatically.

    Returns ``(has_unresolved_timeout, can_resume_without_retry)``.  The second
    value is false when the timeout happened while another service failure was
    already active, or when a provider failure was recorded afterwards; those
    cases require the regular retry path once every human is back online.
    """
    ...
    # A successful recovery command is room-wide and supersedes every timeout
    # before it. Reconnection intentionally does *not* clear this context: the
    # room stays paused until its owner or a system administrator explicitly
    # chooses resume/retry after checking that every human is back online.
    latest_recovery_seq = max(
        (event.seq for event in relevant if event.event_type in {"control.resume", "control.retry"}),
        default=0,
    )
    timeout_events = [
        event for event in relevant if event.event_type == "participant.disconnect_timeout" and event.seq > latest_recovery_seq
    ]
    if not timeout_events:
        return False, False

    # A provider failure after the timeout changes the action from ``resume``
    # to ``retry`` but never removes the online-participant requirement.
    first_timeout_seq = min(event.seq for event in timeout_events)
    service_failed_after = any(event.event_type == "provider.failed" and event.seq > first_timeout_seq for event in relevant)
    can_resume = (
        all(not bool((event.payload if isinstance(event.payload, dict) else {}).get("prior_requires_retry")) for event in timeout_events)
        and not service_failed_after
    )
    return True, can_resume
...
def serialize_room(db: Session, room: Room, user: User | None = None, *, public: bool = False) -> dict[str, Any]:
    ...
    anonymous_lobby = public and user is None and room.status == "lobby"
    ...
    if room.status in {"preparing", "running", "paused", "judging"}:
        ...
        for item in room.seats:
            if item.occupant_type != "human" or not item.user_id or item.connected or not item.disconnected_at:
                continue
            ...
            remaining = max(0, math.ceil(60 - (snapshot_time - disconnected_at).total_seconds()))
            disconnect_pending.append(
                {
    ...
    if disconnect_pending:
        if public or not (viewer_can_control or viewer_is_participant):
            disconnect_grace = {
                "will_pause": room.status != "paused",
                "pending_count": len(disconnect_pending),
                "minimum_remaining_seconds": min(item["remaining_seconds"] for item in disconnect_pending),
            }
        else:
            disconnect_grace = {
                "will_pause": room.status != "paused",
    ...
    if room.status == "paused":
        pause_event = next(
            (item for item in events if item.event_type in {"control.pause", "engine.quarantined", "provider.failed"}),
            None,
        )
        paused_at = pause_event.created_at if pause_event else room.updated_at
        if paused_at.tzinfo is None:
            paused_at = paused_at.replace(tzinfo=timezone.utc)
        paused_duration_seconds = max(0, int((now() - paused_at).total_seconds()))
        connected_humans = sum(1 for item in room.seats if item.occupant_type == "human" and item.connected)
        is_stale = bool(connected_humans == 0 and paused_duration_seconds >= settings.stale_paused_after_minutes * 60)
        pause_health = {
    ...
    return {
        ...
        "owner": (
            {"id": room.owner.id, "real_name": room.owner.real_name}
            if not public
            else {"real_name": "房主" if anonymous_lobby else room.owner.real_name}
        ),
        "seats": [
            {
                "seat_key": item.seat_key,
                ...
                "display_name": (
                    seat_label(item.seat_key)
                    if anonymous_lobby and item.occupant_type == "open"
                    else "已占用"
                    if anonymous_lobby
                    else item.display_name
                ),
                "is_ready": False if anonymous_lobby else item.is_ready,
                "connected": False if anonymous_lobby else item.connected,
                "is_me": False if anonymous_lobby else bool(user and item.user_id == user.id),
                "is_owner": False if anonymous_lobby else bool(item.user_id and item.user_id == room.owner_id),
            }
        ...
        "caption_segments": [
            {
                ...
                "text": item.text,
                "start_ms": (item.presentation_offset_ms if isinstance(item, CaptionSegment) else item.start_ms),
                "end_ms": (item.presentation_offset_ms if isinstance(item, CaptionSegment) else item.end_ms),
        ...
        "can_speak": allowed,
        "speak_reason": reason,
        "can_control": viewer_can_control,
        ...
        "speeches": [
            {
                ...
                # Formal matches use one LiveKit track. Legacy file metadata
                # remains in the database for migration but is not projected
                # while text-only mode is active.
...
def public_event_payload(event_type: str, payload: dict[str, Any]) -> dict[str, Any]:
    ...
    if (
        event_type.startswith(("stage.", "control.", "speech.", "audio.rtc.")) or event_type in {"room.cancelled", "judge.interrupted"}
...
def anonymous_event_payload(event_type: str, payload: dict[str, Any]) -> dict[str, Any]:
    ...
    if (event_type.startswith(("stage.", "control.")) or event_type == "room.cancelled") and isinstance(payload.get("reason"), str):
        result["reason"] = payload["reason"]
    return result
...
def seat_label(key: str) -> str:
    try:
        ...
        display_position = number_words.get(position, position)
        return f"{'正方' if side == 'aff' else '反方'}{display_position}辩"
    except ValueError:
        return key
