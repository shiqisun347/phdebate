...
LIVEKIT_PUBLISHER_LEASE_SECONDS = 30
LIVEKIT_PUBLISHER_REFRESH_SECONDS = 10
LIVEKIT_PUBLISHER_FENCE_CACHE_SECONDS = 1


class LiveKitPublisherLeaseConflict(RuntimeError):
    """Another API worker owns the room's single LiveKit audio publisher."""


class LiveKitPublisherLeaseUnavailable(RuntimeError):
    """The distributed publisher authority is unavailable."""
...
class LiveKitAudioRegistry:
    def __init__(
        ...
        *,
        ...
        # Production uses the shared RoomHub Redis client. Unit registries with
        # a fake adapter remain deliberately local unless a lease authority is
        # injected, which keeps media tests independent from external Redis.
        if lease_authority is None and adapter_factory is None:
            from app.services.realtime import room_hub

            lease_authority = room_hub
        ...
        self._lease_verified_at: dict[str, float] = {}
    ...
    async def ensure_room(self, room_code: str) -> LiveKitRoomAudioPublisher:
        ...
        async with lock:
            ...
            if publisher is None:
                await self._claim_lease(room_code)
                ...
                try:
                    ...
                    await self._release_lease(room_code)
                    raise
                if self._lease_authority is not None:
                    self._lease_tasks[room_code] = asyncio.create_task(
                        self._refresh_lease(room_code, publisher),
                        name=f"livekit-publisher-lease-{room_code}",
                    )
            return publisher

    async def _claim_lease(self, room_code: str) -> None:
        if self._lease_authority is None:
            return
        claimed = await self._lease_authority.livekit_publisher_claim(
            room_code,
            owner=self._owner_id,
            lease_seconds=LIVEKIT_PUBLISHER_LEASE_SECONDS,
        )
        if claimed is None:
            raise LiveKitPublisherLeaseUnavailable(
                f"LiveKit publisher authority unavailable for room {room_code}"
            )
        if not claimed:
            raise LiveKitPublisherLeaseConflict(
                f"LiveKit publisher already owned for room {room_code}"
            )
        self._owned_leases.add(room_code)
        self._lease_verified_at[room_code] = asyncio.get_running_loop().time()

    async def _release_lease(self, room_code: str) -> None:
        if self._lease_authority is None or room_code not in self._owned_leases:
            return
        self._owned_leases.discard(room_code)
        self._lease_verified_at.pop(room_code, None)
        released = await self._lease_authority.livekit_publisher_release(
            room_code,
            owner=self._owner_id,
        )
        if released is None:
            logger.warning("LiveKit publisher lease release unavailable room=%s", room_code)

    async def _refresh_lease(
        self,
        room_code: str,
        publisher: LiveKitRoomAudioPublisher,
    ) -> None:
        try:
            while True:
                await asyncio.sleep(LIVEKIT_PUBLISHER_REFRESH_SECONDS)
                refreshed = await self._lease_authority.livekit_publisher_refresh(
                    room_code,
                    owner=self._owner_id,
                    lease_seconds=LIVEKIT_PUBLISHER_LEASE_SECONDS,
                )
                if refreshed:
                    self._lease_verified_at[room_code] = asyncio.get_running_loop().time()
                    continue
                # Fail closed before another worker can publish the same fixed
                # identity. A definite ownership loss or unavailable Redis
                # authority must revoke this local track and active PCM queue.
                logger.error(
                    "LiveKit publisher lease lost room=%s authority_available=%s",
                    room_code,
                    refreshed is not None,
                )
                with self._registry_lock:
                    if self._publishers.get(room_code) is publisher:
                        self._publishers.pop(room_code, None)
                self._owned_leases.discard(room_code)
                self._lease_verified_at.pop(room_code, None)
            ...
            if self._lease_tasks.get(room_code) is asyncio.current_task():
                self._lease_tasks.pop(room_code, None)

    async def _verify_lease_fence(self, room_code: str) -> None:
        """Fence stale workers before they mutate the room's media stream."""

        if self._lease_authority is None:
            return
        last_verified = self._lease_verified_at.get(room_code, 0.0)
        loop = asyncio.get_running_loop()
        if loop.time() - last_verified < LIVEKIT_PUBLISHER_FENCE_CACHE_SECONDS:
            return
        refreshed = await self._lease_authority.livekit_publisher_refresh(
            room_code,
            owner=self._owner_id,
        ...
        if refreshed:
            self._lease_verified_at[room_code] = loop.time()
            return
        ...
        raise LiveKitPublisherLeaseConflict(
            f"LiveKit publisher ownership lost for room {room_code}"
        )
    ...
    async def start_generation(
        ...
        *,
        ...
        await self._verify_lease_fence(room_code)
    ...
    async def close_room(self, room_code: str) -> None:
        ...
        async with lock:
            ...
            lease_task = self._lease_tasks.pop(room_code, None)
            if lease_task and lease_task is not asyncio.current_task():
                lease_task.cancel()
    ...
    async def close(self) -> None:
        ...
        async with lock:
            ...
            for room_code in tuple(self._owned_leases):
                await self._release_lease(room_code)
            self._lease_verified_at.clear()
            self._loop = None
            self._lock = None
