...
ASR_STREAM_KEY_PREFIX = "jixia:asr:streams:"
ASR_STREAM_LEASE_SECONDS = 10
LIVEKIT_PUBLISHER_KEY_PREFIX = "jixia:livekit:publishers:"
...
# LiveKit permits only one participant with the fixed ``agent-audio:<room>``
# identity.  These owner-bound leases prevent a second API worker from
# connecting with that identity and forcing LiveKit to evict the room's
# already-playing publisher.
_LIVEKIT_PUBLISHER_CLAIM_SCRIPT = """
...
return 0
"""
_LIVEKIT_PUBLISHER_REFRESH_SCRIPT = _ASR_STREAM_REFRESH_SCRIPT
_LIVEKIT_PUBLISHER_RELEASE_SCRIPT = _ASR_STREAM_RELEASE_SCRIPT
...
class RoomHub:
    ...
    @staticmethod
    def _asr_stream_key(speech_id: str) -> str:
        # Speech ids are internal UUIDs today, but hashing keeps a malformed or
        # migrated identifier from creating an unbounded Redis key.
        digest = hashlib.sha256(speech_id.encode()).hexdigest()
        return f"{ASR_STREAM_KEY_PREFIX}{digest}"

    async def asr_stream_claim(
        self,
        speech_id: str,
        *,
        owner: str,
        lease_seconds: float = ASR_STREAM_LEASE_SECONDS,
    ) -> bool | None:
        """Claim one authoritative ASR bridge across every API worker.

        ``None`` means the distributed authority was unavailable. Callers may
        use a process-local fallback in tests, but production should fail the
        live-caption connection closed and keep the speech available for
        manual transcript review rather than risk two finals racing each other.
        """

        lease_ms = max(1_000, round(lease_seconds * 1_000))
        result = await self._presence_eval(
            _ASR_STREAM_CLAIM_SCRIPT,
            [self._asr_stream_key(speech_id)],
            [owner, lease_ms],
        )
        return None if result is None else bool(result)

    async def asr_stream_refresh(
        self,
        speech_id: str,
        *,
        owner: str,
    ...
    async def asr_stream_release(self, speech_id: str, *, owner: str) -> bool | None:
        result = await self._presence_eval(
            _ASR_STREAM_RELEASE_SCRIPT,
            [self._asr_stream_key(speech_id)],
            [owner],
    ...
    @staticmethod
    def _livekit_publisher_key(room_code: str) -> str:
        digest = hashlib.sha256(room_code.encode()).hexdigest()
        return f"{LIVEKIT_PUBLISHER_KEY_PREFIX}{digest}"
    ...
    async def livekit_publisher_claim(
        ...
        *,
        owner: str,
        lease_seconds: float = LIVEKIT_PUBLISHER_LEASE_SECONDS,
    ) -> bool | None:
        lease_ms = max(1_000, round(lease_seconds * 1_000))
        result = await self._presence_eval(
            _LIVEKIT_PUBLISHER_CLAIM_SCRIPT,
            [self._livekit_publisher_key(room_code)],
            [owner, lease_ms],
        )
        return None if result is None else bool(result)
    ...
    async def livekit_publisher_refresh(
        ...
        *,
        ...
        lease_ms = max(1_000, round(lease_seconds * 1_000))
        result = await self._presence_eval(
            _LIVEKIT_PUBLISHER_REFRESH_SCRIPT,
            [self._livekit_publisher_key(room_code)],
            [owner, lease_ms],
        )
        return None if result is None else bool(result)

    async def livekit_publisher_release(self, room_code: str, *, owner: str) -> bool | None:
        result = await self._presence_eval(
            _LIVEKIT_PUBLISHER_RELEASE_SCRIPT,
            [self._livekit_publisher_key(room_code)],
            [owner],
        )
        return None if result is None else bool(result)
