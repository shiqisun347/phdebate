...
def normalize_transcript(text: str) -> str:
    """Normalize layout noise without rewriting the speaker's punctuation.

    NFKC is useful for comparing and validating compatibility characters, but
    it also turns Chinese full-width punctuation such as ``，`` into ASCII
    punctuation.  A transcript is user-authored evidence, so the persisted
    value must retain that distinction.  NFC still gives us stable Unicode
    composition while preserving the punctuation the speaker submitted.
    """
...
def transcript_rejection_reason(
    ...
    *,
    ...
    # Compatibility normalization belongs only to validation.  This keeps
    # full-width letters/digits eligible as speech content without mutating
    # the transcript later stored and rendered to the participant.
    validation_text = unicodedata.normalize("NFKC", normalized)
    if not _VISIBLE_CONTENT.search(validation_text):
    ...
    visible = [character for character in validation_text if not character.isspace()]
