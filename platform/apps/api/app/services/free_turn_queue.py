...
from app.services.room_service import append_event, now, stage
...
def begin_intermission(db: Session, room: Room, current: dict) -> dict:
    ...
    if room.stage_deadline_at:
        deadline = min(deadline, _aware(room.stage_deadline_at))
...
def resolve_intermission(db: Session, room: Room, current: dict) -> tuple[dict, FreeTurnRequest | None]:
    ...
    for item in pending:
        ...
        if item is winner:
            ...
            item.status = "expired"
            item.resolution_reason = "another_request_selected" if winner else "window_closed"
    updated = dict(current)
    for key in (
        "intermission_side",
    ...
    updated["turn_started_at"] = resolved_at.isoformat()
    if winner:
        ...
        has_ai = any(item.side == side and item.occupant_type in {"ai", "ai_substitute"} for item in room.seats)
        if not has_ai:
            turn_duration = max(1, int(updated.get("turn_duration", 45)))
            updated["turn_started_at"] = (resolved_at - timedelta(seconds=max(0, turn_duration - INTERMISSION_SECONDS))).isoformat()
    snapshot = list(room.template_snapshot)
    snapshot[room.current_stage_index] = updated
    room.template_snapshot = snapshot
    append_event(
        ...
        {"side": side, "turn_seq": target_turn, "seat_key": winner.seat_key if winner else None, "fallback": winner is None},
    )
