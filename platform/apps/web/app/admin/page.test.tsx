...
describe("admin operations", () => {
  ...
  it("renders topic, agent and versioned automation controls", async () => {
    ...
    fireEvent.change(screen.getByLabelText("审核理由"), { target: { value: "已对照识别分段和上下文确认" } });
    ...
    await waitFor(() => expect(fetchMock).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        method: "POST",
        body: JSON.stringify({ expected_updated_at: new Date(0).toISOString(), reason: "已对照识别分段和上下文确认" }),
      }),
    ));
