  ...
  return (
    <div className="page-shell">
      ...
      <section style={{ paddingTop: 28 }}>
        ...
        {tab === "intro" && <div className="dashboard-grid"><article className="panel"><div className="panel-title"><h2>关于赛事</h2><ShieldCheck color="#8c6cff" /></div><p className="hero-copy" style={{ fontSize: 15 }}>{item.description}</p><h3>比赛方式</h3><p className="muted">登录用户创建房间并认领人类席位，其他玩家通过六位房间号加入。房主开始比赛后，所有空席由 AI 自动填充，系统按预设流程完成播报与赛程推进。</p></article><aside className="panel"><div className="panel-title"><h3>可选辩题</h3></div><div className="history-list">{item.topics?.length ? item.topics.map((topic,index) => <div className="history-row" key={topic.id}><span className="badge">{String(index+1).padStart(2,'0')}</span><span>{topic.title}</span></div>) : <div className="empty">{item.allow_custom_topic ? "训练赛支持自定义辩题" : "管理员尚未添加可用辩题"}</div>}</div></aside></div>}
        {tab === "ranking" && <div className="panel"><div className="panel-title"><h2>赛季排行榜</h2><Trophy color="#ffcc6d" /></div><div className="table-wrap">{data.leaderboard.length > 0 && <table className="ranking-table"><caption className="sr-only">当前赛事的赛季个人排行榜</caption><thead><tr><th>排名</th><th>辩手</th><th>赛季积分</th><th>战绩</th><th>平均评分</th><th>有效场次</th></tr></thead><tbody>{data.leaderboard.map((row) => <tr key={row.user_id}><td data-label="排名">#{row.rank}</td><td data-label="辩手"><strong>{row.real_name}</strong></td><td data-label="赛季积分"><strong>{row.points}</strong></td><td data-label="战绩">{row.wins} 胜 · {row.draws} 平 · {row.losses} 负</td><td data-label="平均评分">{row.average_score}</td><td data-label="有效场次">{row.matches}</td></tr>)}</tbody></table>}{!data.leaderboard.length && <div className="empty empty-guidance"><Trophy size={30} style={{ margin: "0 auto 10px" }} /><strong>这个赛事还没有有效比赛</strong><span>完成计分比赛后，积分、战绩与平均评分会显示在这里。</span></div>}</div></div>}
        ...
        {tab === "rules" && <article className="panel"><div className="panel-title"><h2>赛事规则</h2><ShieldCheck /></div><p className="hero-copy" style={{ whiteSpace: "pre-wrap", fontSize: 15 }}>{item.rules}</p><h3>通用约定</h3><ul className="muted" style={{ lineHeight: 2 }}><li>系统按固定阶段自动计时和切换。</li><li>只有当前轮次指定的人类辩手可以开启发言。</li><li>真人断线后席位和身份保持不变；60 秒超时后整场比赛自动暂停，辩手返回后由房主确认继续。</li><li>所有房间合计最多允许 5 位观众同时在线；辩手、房主和管理员不占用观战名额。</li><li>禁止人身攻击、冒用身份和绕过客户端权限。</li></ul></article>}
        </div>
      </section>
