...
export default function ResultPage() {
  ...
  // A completed result is an immutable REST document until the user asks to
  // refresh it. Keeping a spectator WebSocket open here wastes one of the five
  // global audience slots and produces a misleading “立即重连” prompt after
  // the match has already ended. Only the pre-result live record needs room
  // events so it can follow the active match.
  const resultRealtimeEnabled = Boolean(
    data && !["completed", "review_required", "terminated"].includes(data.match.status),
  );
  const { room: liveRoom, error: connectionError, reconnect } = useRoom(code, {
    enabled: resultRealtimeEnabled,
  });
  ...
  const load = useCallback(async (preserveLoadedSpeeches = false) => {
    ...
    try {
      ...
      if (sequence === loadSequence.current) {
        loadedSpeechIds.current = new Set(next.speeches.map((speech) => speech.id));
        setData(next);
      }
    } catch (err) {
  ...
  async function refreshResult() {
    setRefreshing(true);
    try {
      // The user can refresh in the same task that just appended an older
      // page, before React has run the effect that mirrors speech ids into the
      // ref. Capture the rendered collection synchronously so a refresh never
      // drops already visible history pages.
      if (data) loadedSpeechIds.current = new Set(data.speeches.map((speech) => speech.id));
      await load(true);
      reconnect();
    } finally {
      setRefreshing(false);
    ...
    try {
      ...
      setData((current) =>
        current?.speech_pagination.next_cursor === cursor
          ? (() => {
              const speeches = mergeSpeeches(older.speeches, current.speeches);
              loadedSpeechIds.current = new Set(speeches.map((speech) => speech.id));
              return {
                ...current,
                speeches,
      ...
      : data.room.status === "paused"
        ...
        : data.room.status === "judging"
          ...
          : ["running", "preparing", "lobby"].includes(data.room.status)
            ...
            : canViewTranscript
              ? "赛果已经确认，发言文字、评分和积分变化均可在下方回溯。"
              : "赛果已经确认，评分和比赛时间线可在下方回溯。";
  ...
  return (
    <div className="page-shell">
      <section className={`result-hero panel ${resultTone}`}>
        ...
        {data.match.winner === "draw" &&
          data.scorecard?.status === "approved" &&
          data.scorecard.affirmative_score !== data.scorecard.negative_score && (
            <p className="result-score-note">
              双方分数接近，但裁判未认定差距足以形成明确胜负，因此本场判为平局；详细依据见下方裁判理由。
            </p>
          )}
      ...
      {canViewTranscript ? (
        <section className="panel" style={{ marginTop: 18 }}>
          ...
          <div className="transcript-list" role={data.speeches.length ? "list" : undefined} aria-label={data.speeches.length ? "完整辩论文字记录" : undefined}>
            ...
            {!data.speeches.length && (
              <div className="empty">本场没有已完成的发言文字记录</div>
            )}
          </div>
          {data.speech_pagination.has_more && (
            <div
        ...
        <div className="notice-box" role="status" style={{ marginTop: 18 }}>
          为保护参赛者数据，观众不能查看赛中字幕和赛后发言文字。
        </div>
      )}
