...
describe("watch result redirect", () => {
  ...
  it("shows a redacted waiting state before a public match starts", () => {
    mocks.room = { id: "room", code: "123456", status: "lobby", seq: 1, topic: "赛前公开观战测试" } as unknown as Room;
    render(<WatchPage />);
    expect(screen.getByRole("heading", { name: "比赛尚未开始" })).toBeInTheDocument();
    expect(screen.getByText("赛前公开观战测试")).toBeInTheDocument();
    expect(screen.queryByText("观战舞台")).not.toBeInTheDocument();
  });
