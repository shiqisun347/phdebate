...
describe("API client", () => {
  afterEach(() => {
    document.cookie = "jixia_csrf=; Max-Age=0; path=/";
  ...
  it("attaches the CSRF cookie to state-changing requests", async () => {
    document.cookie = "jixia_csrf=token%2Bvalue; path=/";
  ...
  it("does not describe a failed read-only request as an uncertain submission", async () => {
    vi.stubGlobal("fetch", vi.fn().mockRejectedValue(new TypeError("Failed to fetch")));
    const error = await apiFetch("/api/competitions").catch((value) => value);
    expect(error).toBeInstanceOf(ApiRequestError);
    expect(error).toMatchObject({
      status: 0,
      detail: { code: "network_unavailable" },
      message: "网络连接失败，暂时无法加载内容。请检查网络后重试。",
    });
  });
