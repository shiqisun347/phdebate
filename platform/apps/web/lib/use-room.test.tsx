...
describe("useRoom", () => {
  ...
  it("does not consume a room WebSocket or fetch a snapshot while realtime is disabled", async () => {
    const fetchMock = vi.fn().mockResolvedValue(roomResponse(1));
    vi.stubGlobal("fetch", fetchMock);
    vi.stubGlobal("WebSocket", FakeWebSocket);
    const { result, rerender } = renderHook(
      ({ enabled }) => useRoom("123456", { enabled }),
  ...
  it("shows a terminal message instead of retrying when all 5 global spectator slots are occupied", () => {
    vi.useFakeTimers();
    ...
    act(() => {
      FakeWebSocket.instances[0].open();
      FakeWebSocket.instances[0].emitClose(4429);
      vi.advanceTimersByTime(20_000);
    });
    expect(FakeWebSocket.instances).toHaveLength(1);
  ...
  it("accepts equal-sequence caption projections that do not create match events", () => {
    vi.stubGlobal("fetch", vi.fn(() => new Promise<Response>(() => undefined)));
    vi.stubGlobal("WebSocket", FakeWebSocket);
    const { result } = renderHook(() => useRoom("123456"));
    const socket = FakeWebSocket.instances[0];
    const initial = snapshot(4) as Room;
    const activeSpeech = { id: "speech-ai", seat_key: "aff_1", speaker_type: "ai", status: "synthesizing" };

    act(() => {
      socket.open();
      socket.message({ type: "snapshot", room: { ...initial, active_speech: activeSpeech, caption_segments: [] } });
      socket.message({
        type: "snapshot",
        room: {
          ...initial,
        ...
        event: { type: "audio.rtc.started", room_code: "123456" },
      });
    });

    expect(result.current.room?.caption_segments).toHaveLength(1);
    expect(result.current.room?.caption_segments[0].text).toContain("增强人的判断能力");
  });

  it("clears an ephemeral ASR partial before accepting a reconnect snapshot", () => {
    vi.useFakeTimers();
    vi.stubGlobal("fetch", vi.fn(() => new Promise<Response>(() => undefined)));
    vi.stubGlobal("WebSocket", FakeWebSocket);
    const { result } = renderHook(() => useRoom("123456"));
    ...
    expect(result.current.liveEvent?.text).toBe("断线前尚未确认的旧字幕");
    ...
    act(() => second.message({ type: "snapshot", room: snapshot(8) }));
    expect(result.current.room?.seq).toBe(8);
    expect(result.current.liveEvent).toBeNull();
  });
