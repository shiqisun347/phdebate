...
describe("AuthForm", () => {
  ...
  it("catches a password confirmation mismatch locally without a failed registration request", async () => {
    const fetchMock = vi.fn();
    vi.stubGlobal("fetch", fetchMock);
    render(<AuthForm mode="register" />);

    fireEvent.change(screen.getByLabelText("密码"), { target: { value: "Password-1234" } });
    fireEvent.change(screen.getByLabelText("确认密码"), { target: { value: "Password-5678" } });

    expect(screen.getByLabelText("确认密码")).toHaveAttribute("aria-invalid", "true");
    expect(screen.getByRole("alert")).toHaveTextContent("两次输入的密码不一致");
    expect(screen.getByRole("button", { name: "注册并进入赛场" })).toBeDisabled();
