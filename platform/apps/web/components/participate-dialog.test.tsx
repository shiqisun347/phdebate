...
const sessionState = vi.hoisted(() => ({ user: { id: "user" } as { id: string } | null, activeRoom: null as { code: string; topic: string; status: string; seat_key: string } | null, loading: false }));
...
describe("participation dialog", () => {
  ...
  it("makes the current match the primary action instead of allowing a doomed create attempt", async () => {
    sessionState.activeRoom = { code: "769024", topic: "当前进行中的辩题", status: "running", seat_key: "aff_1" };
    vi.stubGlobal("fetch", vi.fn().mockResolvedValue(response({ competition: training })));
    render(<ParticipateDialog competition={training} onClose={vi.fn()} />);

    expect(await screen.findByText("你正在房间 #769024 参赛")).toBeInTheDocument();
    expect(screen.getByRole("button", { name: /创建比赛\s+先返回房间 #769024/ })).toBeDisabled();
    expect(screen.getByRole("link", { name: "返回当前比赛" })).toHaveAttribute("href", "/rooms/769024/debate");
