...
export default function MediaModule({ media, archives, dataQuality, saving, onRefreshMedia, onCleanupMedia, onRefreshArchives, onRefreshDataQuality, onUpdateDataIssue, onRepairArchives, onCleanupArchives }: Props) {
  ...
  const qualityAttention = dataQuality
    ...
    : 0;
  const visibleSamples = (dataQuality?.attention.samples || []).flatMap((item) => {
    const issues = item.issues.filter((issue) => issue !== "missing_audio");
    return issues.length ? [{ ...item, issues }] : [];
  });
  const unreviewedVisibleIssueCount = visibleSamples.reduce(
    (total, item) => total + item.issues.filter((issue) => !item.dispositions[issue]).length,
    0,
  );
  ...
  return (
    <>
      <div className="panel-title">
        <div>
          <h2>比赛数据质量</h2>
          <p className="admin-section-copy">仅统计正式比赛，检查赛果、发言文字和必要分段是否可用于复盘与分析。比赛音频不保存，也不参与完整性判断。</p>
        </div>
      ...
      {dataQuality ? (
        <>
          <div className="stats-grid">
            ...
            <div className="stat-card"><span className="muted">文字待核验</span><strong>{dataQuality.attention.human_missing_transcript}</strong><small>缺少可用发言文字</small></div>
          ...
          {unreviewedVisibleIssueCount > 0 && (
            <div className="notice-box" role="status">
              当前抽样的异常中还有 {unreviewedVisibleIssueCount} 项未记录处置意见。数据异常不会被自动隐藏，必须明确标记为待补采、不可恢复或已人工核验。
            </div>
          )}
          {qualityAttention ? (
            <div className="error-box" role="status">
              <TriangleAlert size={17} />
              已发布无有效裁判 {dataQuality.attention.published_without_scorecard} 场；无发言 {dataQuality.attention.published_without_speeches} 场；真人缺逐字稿 {dataQuality.attention.human_missing_transcript} 段；缺分段记录 {dataQuality.attention.human_missing_segments} 段。
            </div>
          ) : (
            <div className="success-box" role="status"><ShieldCheck size={17} />正式比赛的赛果、发言文字和必要分段均通过完整性检查。</div>
          )}
      ...
      <div className="panel-title" style={{ marginTop: 36 }}><div><h2>系统预设媒体与历史兼容文件</h2><p className="admin-section-copy">仅维护阶段提示音等系统资产及旧数据引用，不保存新的比赛发言音频，也不影响比赛数据质量。</p></div><span className="badge">默认只读盘点</span></div>
      {media ? (
        <>
          <div className="stats-grid">
      ...
      {selectedIssue && (
        <div className="dialog-overlay" role="presentation" onMouseDown={(event) => event.target === event.currentTarget && setSelectedIssue(null)}>
          <form className="dialog" role="dialog" aria-modal="true" aria-label="记录比赛数据异常处置" onSubmit={submitDisposition}>
            ...
            <div className="notice-box">该操作不会伪造或覆盖发言文字和分段数据，只记录后续补采或人工核验结论。</div>
            <div className="field">
