...
describe("admin lazy modules", () => {
  ...
  it("shows actionable production data coverage and records a durable disposition", async () => {
    ...
    expect(screen.queryByText("真人录音覆盖")).not.toBeInTheDocument();
    expect(screen.queryByText(/缺录音/)).not.toBeInTheDocument();
    expect(screen.queryByRole("button", { name: /缺少真人录音/ })).not.toBeInTheDocument();
    expect(screen.getByRole("link", { name: "查看记录" })).toHaveAttribute("href", "/rooms/381526/result");
    fireEvent.click(screen.getByRole("button", { name: "缺少逐字稿 · 未处置" }));
