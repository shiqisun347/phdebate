    ...
    <>
      <div className="panel-title admin-review-title admin-review-history-title">
        <div>
          ...
          <p className="admin-section-copy">逐条对照原始文字、识别分段、比赛上下文与参赛者建议。批准只更新规范化逐字稿，原始文字和审计记录都会保留。</p>
        </div>
        ...
        <div className="dialog-overlay" role="presentation" onMouseDown={(event) => event.target === event.currentTarget && setTarget(null)}>
          <form className="dialog" role="dialog" aria-modal="true" aria-labelledby="speech-correction-review-title" onSubmit={submit}>
            ...
            <div className="field">
              ...
              <textarea id="speech-correction-review-reason" className="textarea" minLength={2} maxLength={1000} required autoFocus value={reason} onChange={(event) => setReason(event.target.value)} placeholder={decision === "approve" ? "说明核对依据，例如上下文、识别分段与相邻发言。" : "说明拒绝原因，例如建议文字与上下文或识别分段不符。"} />
            </div>
