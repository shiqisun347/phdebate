...
export function ParticipateDialog({ competition, onClose }: { competition: Competition; onClose: () => void }) {
  ...
  const { user, activeRoom, loading: sessionLoading } = useSession();
  ...
  const activeRoomHref = activeRoom
    ? `/rooms/${activeRoom.code}/${activeRoom.status === "lobby" ? "lobby" : "debate"}`
    : "";
  ...
  useEffect(() => {
    if (activeRoom && mode === "create") setMode("join");
  }, [activeRoom, mode]);
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      if (mode === "join") {
        ...
        if (activeRoom) throw new Error(`你已在房间 #${activeRoom.code} 参赛，请先返回该比赛。`);
    ...
    <div className="dialog-overlay" role="presentation" onMouseDown={(event) => event.target === event.currentTarget && onClose()}>
      <div ref={dialogRef} className="dialog participate-dialog" role="dialog" aria-modal="true" aria-labelledby="participate-title" aria-describedby="participate-description">
        ...
        <div className="choice-grid">
          <button type="button" aria-pressed={mode === "create"} disabled={busy || Boolean(creationBlockedReason) || Boolean(activeRoom)} className={`choice-card ${mode === "create" ? "active" : ""}`} onClick={() => setMode("create")}><Plus size={20} /><strong>{isPrimaryCompetition(detail) ? "创建 4v4 比赛" : "创建比赛"}</strong><small>{activeRoom ? `先返回房间 #${activeRoom.code}` : creationBlockedReason ? "当前赛季不可创建" : "选择席位并邀请队友"}</small></button>
        ...
        {activeRoom && <div className="notice-box active-room-return" role="status" style={{ marginTop: 16 }}><span><strong>你正在房间 #{activeRoom.code} 参赛</strong><small>{activeRoom.topic}</small></span><Link className="button button-small" href={activeRoomHref} onClick={onClose}>返回当前比赛</Link></div>}
