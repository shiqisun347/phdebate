...
export function AuthForm({ mode }: { mode: "login" | "register" }) {
  ...
  return (
    <form className="form-stack" onSubmit={submit}>
      ...
      <div className="field"><label htmlFor="auth-password">密码</label><input id="auth-password" name="password" className="input" type="password" value={password} onChange={(e) => { setPassword(e.target.value); setConfirmError(""); }} autoComplete={mode === "login" ? "current-password" : "new-password"} autoCapitalize="none" spellCheck={false} minLength={8} required /></div>
