#!/usr/bin/env python3
"""Atomically publish a non-secret database-backup status record."""
...
def read_previous(path: Path) -> dict[str, object]:
    try:
...
def main() -> None:
    ...
    previous = read_previous(args.status_file)
    now = datetime.now(timezone.utc).isoformat()
    payload: dict[str, object] = {
        key: previous[key]
        for key in ("last_success_at", "file", "bytes", "sha256", "retention_days")
        if key in previous
    }
    ...
    atomic_write(args.status_file, payload)
