...
assert manifest["runtime_parameters"]["decode_chunk_frames"] == 12
assert manifest["runtime_parameters"]["initial_chunk_frames"] == 6
assert manifest["runtime_parameters"]["dynamo_cache_size_limit"] == 64
assert manifest["runtime_parameters"]["local_compile_mode"] == "disabled"
assert manifest["runtime_parameters"]["local_compile_dynamic"] is False
...
assert 'MOSS_GATEWAY_DECODE_CHUNK_FRAMES="${MOSS_GATEWAY_DECODE_CHUNK_FRAMES:-12}"' in runner
...
assert 'MOSS_GATEWAY_LOCAL_COMPILE_MODE="${MOSS_GATEWAY_LOCAL_COMPILE_MODE:-reduce-overhead}"' in runner
assert 'MOSS_GATEWAY_LOCAL_COMPILE_DYNAMIC="${MOSS_GATEWAY_LOCAL_COMPILE_DYNAMIC:-false}"' in runner
assert 'MOSS_GATEWAY_ASYNC_DECODER_ENABLED="${MOSS_GATEWAY_ASYNC_DECODER_ENABLED:-false}"' in runner
assert 'MOSS_GATEWAY_ASYNC_DECODER_QUEUE_SIZE="${MOSS_GATEWAY_ASYNC_DECODER_QUEUE_SIZE:-2}"' in runner
assert 'PYTHONPATH="$UPSTREAM_CHECKOUT/moss_tts_realtime:$GATEWAY_DIR' in runner
assert 'MOSS_GATEWAY_ATTN_IMPL="${MOSS_GATEWAY_ATTN_IMPL:-sdpa}"' in runner
