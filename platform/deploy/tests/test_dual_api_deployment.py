...
def test_supervisor_workers_have_unique_ports_and_safe_process_roles() -> None:
    config = read("phdebate.supervisor.conf")
    primary = config.split("[program:jixia-api]", 1)[1].split("[program:jixia-api-secondary]", 1)[0]
    secondary = config.split("[program:jixia-api-secondary]", 1)[1].split("[program:jixia-engine]", 1)[0]
    engine = config.split("[program:jixia-engine]", 1)[1].split("[program:jixia-worker]", 1)[0]
    ...
    for worker in (primary, secondary):
        ...
        assert "MEDIA_ROOT=/home/ubuntu/sunsq/phdebate/storage/audio" in worker
        assert "ARCHIVE_ROOT=/home/ubuntu/sunsq/phdebate/storage/archives" in worker
...
def test_nginx_balances_rest_and_state_ws_but_not_audio_or_asr() -> None:
    config = read("jixia-nginx-root.conf")
...
def test_nginx_supervisor_uses_the_tracked_production_config() -> None:
    ...
    assert "deploy/jixia-nginx-root.conf" in config
...
def test_production_moss_gateway_recovers_after_a_host_reboot() -> None:
    ...
    assert "MOSS_GATEWAY_DECODE_CHUNK_FRAMES=\"12\"" in config
    assert "MOSS_GATEWAY_INITIAL_CHUNK_FRAMES=\"6\"" in config
    assert "MOSS_GATEWAY_LOCAL_COMPILE_MODE=\"reduce-overhead\"" in config
    assert "MOSS_GATEWAY_LOCAL_COMPILE_DYNAMIC=\"false\"" in config
