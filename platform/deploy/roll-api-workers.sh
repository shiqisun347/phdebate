...
PRIMARY_SERVICE="jixia-api"
SECONDARY_SERVICE="jixia-api-secondary"
PRIMARY_URL="http://127.0.0.1:12340"
SECONDARY_URL="http://127.0.0.1:12342"
HEALTH_RETRIES="${PHDEBATE_API_HEALTH_RETRIES:-30}"
...
wait_public_healthy() {
  local attempt
  local url="${PHDEBATE_PUBLIC_ORIGIN:-https://117.50.192.216}/api/health"
  for ((attempt = 1; attempt <= HEALTH_RETRIES; attempt++)); do
