...
- Supervisor 中 平台、Agent、PostgreSQL、Redis、MOSS、FunASR、LiveKit 和 Nginx 均为 RUNNING。
...
- 最大当前业务日志为 MOSS stderr，约 11.7 MB；其他 平台 日志均小于 200 KB。
- Supervisor 配置没有对多数 平台 日志显式写出 rotation 参数，虽然 Supervisor 有默认值，建议显式
...
- 平台 最近备份：`2026-07-20T11:31:29Z`，readiness 显示约 0.7 小时，dump 结构可读。
- 平台 health 会在最后成功备份超过 36 小时时失败，并检查文件状态 JSON。
