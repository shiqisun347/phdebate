...
- 独立 `jixia-engine` 是唯一比赛引擎；两个 API 均强制
...
| 进程 | 端口 | instance | 职责 |
| --- | ---: | --- | --- |
| `jixia-api` | 12340 | `api-primary` | REST、房间状态 WS、可靠语音 WS |
| `jixia-api-secondary` | 12342 | `api-secondary` | REST、房间状态 WS |
| `jixia-engine` | 无 HTTP 端口 | — | 唯一状态机和 Presence 过期回收 |
...
1. 构建不可变 API release：
   ...
   ```bash
   PHDEBATE_API_RELEASE=round4-dual-api \
     /home/ubuntu/sunsq/phdebate/deploy/build-api-release.sh
   ln -sfn /home/ubuntu/sunsq/phdebate/runtime/api-releases/round4-dual-api \
     /home/ubuntu/sunsq/phdebate/.api-secondary
   ```

2. 只安装 `phdebate-api-secondary.supervisor.conf`，执行
   `supervisorctl reread && supervisorctl update`。原 `12340` 不动。
3. 直连验证 `12342`：

   ```bash
   curl --fail http://127.0.0.1:12342/api/health
   PYTHONPATH=/home/ubuntu/sunsq/phdebate/apps/api \
     /home/ubuntu/sunsq/phdebate/.venv/bin/python \
     /home/ubuntu/sunsq/phdebate/deploy/verify_dual_api_workers.py \
     --room-code <一个公开运行中房间号>
   ```

4. 安装包含 `jixia_api` upstream 的 Nginx 配置，先运行 `nginx -t`，再 reload。
   连续请求 `/api/health` 应能看到两个不同的 `instance`。
5. 将 `.api-primary` 指到同一 release。移除临时 secondary 配置，安装完整
   `phdebate.supervisor.conf`，再执行 Supervisor reread/update。此时 secondary 已经承接
