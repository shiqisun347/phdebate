...
An isolated, single-active gateway for the debate platform realtime voice
protocol. Production uses one persistent bidirectional WebSocket per utterance:
LLM text deltas enter the same MOSS context while mono PCM leaves that context.
...
The API service (not the browser) connects to `/tts/session/ws` with
...
export MOSS_GATEWAY_DECODE_CHUNK_FRAMES=12
export MOSS_GATEWAY_INITIAL_CHUNK_FRAMES=6
export MOSS_GATEWAY_LOCAL_COMPILE_MODE=reduce-overhead
export MOSS_GATEWAY_LOCAL_COMPILE_DYNAMIC=false
export MOSS_GATEWAY_ASYNC_DECODER_ENABLED=false
```

For the production RTX 3090 / PyTorch `2.9.1+cu128` image, install the
matching prebuilt wheel instead of letting `pip` select the source-only
`2.8.3.post1` sdist:
...
If a wheel is unavailable for a future image, set `CUDA_HOME` to the CUDA
toolkit matching the PyTorch wheel and restrict `FLASH_ATTN_CUDA_ARCHS` to the
actual GPU (the 3090 uses `86`) before compiling.

These are the current SDPA production settings. OpenMOSS/MOSS-TTS#52 makes
`flash_attention_2` a correctness-compatible experimental option, but the
production default remains SDPA because the 2026-07-22 RTX 3090 canary measured
active RTF 2.72–2.77 and multi-second delivery gaps with FlashAttention 2. The
metrics below are the SDPA release baseline. OpenMOSS/MOSS-TTS#52
fixes the DynamicCache path required by `flash_attention_2`; in this mode the
upstream local transformer intentionally does not use the fixed CUDA-graph
compile. The mode must therefore be compared against the SDPA release path on
the same GPU before claiming a speedup. On the RTX
3090, eight fixed voices completed with RTF P50/P95/max
`0.845/0.883/0.889`; a 66.88-second Chinese utterance completed at RTF `0.805`.
First non-silent PCM P95 was 558 ms and a 1200 ms sender buffer produced zero
simulated underruns. A 12-frame codec batch contains about 960 ms of PCM, but
the LiveKit publisher splits it into one continuous, paced 20 ms WebRTC track.
Therefore raw gateway packet arrival intervals are diagnostic only; queued
audio lead and audible underrun are the release criteria.

When using SDPA, `reduce-overhead` uses fixed-shape CUDA graphs. If a PyTorch
runtime cannot safely reuse those graphs across the gateway inference thread,
set `MOSS_GATEWAY_LOCAL_COMPILE_MODE=disabled` to bypass local `torch.compile`
and restore correctness before attempting further optimization. Dynamic
compilation stays disabled. The codec decoder remains synchronous because an asynchronous decoder
on the same GPU contended with the talker and measured worse RTF. One process
admits only one active synthesis task, avoiding the distortion and CPU/GPU
contention reported for same-model concurrency.
...
and recovery gates pass. This override is intended only for controlled hardware
diagnostics and is not a production fallback.
