...
def test_websocket_canary_metadata_stays_off_without_session_opt_in(tmp_path: Path) -> None:
    ...
    with TestClient(application) as client:
        with client.websocket_connect("/tts/session/ws") as websocket:
            ...
            assert not any(
                isinstance(frame, dict) and str(frame.get("type", "")).startswith("canary.") for frame in frames
            )
            websocket.send_json({"type": "abort", "reason": "done"})
            receive_until_json(websocket, "released")
...
def test_websocket_ping_and_abort_are_not_blocked_by_slow_delta(tmp_path: Path) -> None:
    ...
    with TestClient(application) as client:
        with client.websocket_connect("/tts/session/ws") as websocket:
            ...
            assert any(isinstance(frame, dict) and frame.get("type") == "audio_reset" for frame in release_frames)
            reset_index = next(
                index
...
def test_websocket_final_abort_race_has_one_released_ack_and_no_orphan(tmp_path: Path) -> None:
    ...
    with TestClient(application) as client:
        with client.websocket_connect("/tts/session/ws") as websocket:
            ...
            assert sum(isinstance(frame, dict) and frame.get("type") == "released" for frame in frames) == 1
            assert released["released"] is True and released["status"] == "aborted"
...
def test_websocket_disconnect_aborts_worker_and_restores_capacity(tmp_path: Path) -> None:
    ...
    with TestClient(application) as client:
        ...
        # Codec-context teardown and session bookkeeping are separate
        # synchronization points now that all model work is serialized on the
        # persistent inference thread. Wait for the authoritative release ACK.
        assert session.worker_exited.wait(1.0)
...
def test_openmoss_configuration_fails_closed_without_fixed_security_and_revisions(tmp_path: Path) -> None:
    ...
    GatewaySettings(**{**base, "attention_implementation": "flash_attention_2"}).validate()
    with pytest.raises(ValueError, match="SDPA or flash_attention_2"):
...
def test_openmoss_diagnostic_places_model_on_cuda_and_codec_on_cpu(
    ...
    assert backend.placement["attention_implementation"] == "sdpa"
    assert backend.placement["local_compile_effective"] is True
...
def test_local_compile_canary_reads_environment_and_rejects_unknown_mode(monkeypatch) -> None:
    monkeypatch.setenv("MOSS_GATEWAY_LOCAL_COMPILE_MODE", "reduce-overhead")
    monkeypatch.setenv("MOSS_GATEWAY_LOCAL_COMPILE_DYNAMIC", "false")
    configured = GatewaySettings.from_env()
    assert configured.local_compile_mode == "reduce-overhead"
    assert configured.local_compile_dynamic is False
    with pytest.raises(ValueError, match="LOCAL_COMPILE_MODE"):
        GatewaySettings(local_compile_mode="max-autotune").validate()
    with pytest.raises(ValueError, match="LOCAL_COMPILE_DYNAMIC"):
        GatewaySettings(
            local_compile_mode="reduce-overhead",
            local_compile_dynamic=True,
        ).validate()


def test_local_compile_canary_uses_fixed_reduce_overhead_options(tmp_path, monkeypatch) -> None:
    calls: list[dict[str, object]] = []
    fake_torch = ModuleType("torch")

    def compile_runner(runner, **options):
        calls.append(options)
        return runner

    fake_torch.compile = compile_runner
    monkeypatch.setitem(sys.modules, "torch", fake_torch)
    ...
    class Inference:
        ...
        def _generate_local_transformer_impl(self):
            return "ok"

    backend = OpenMossCudaBackend(
        settings(
            tmp_path,
            local_compile_mode="reduce-overhead",
            local_compile_dynamic=False,
        )
    )
    inference_class = backend._get_cancellable_inference_class(Inference)
    inference = inference_class(cancel_event=threading.Event())

    runner = inference._get_local_transformer_runner()
    assert runner() == "ok"
    assert inference._get_local_transformer_runner() is runner
    assert calls == [{"fullgraph": True, "mode": "reduce-overhead", "dynamic": False}]


def test_reduce_overhead_snapshots_prefill_graph_output_before_incremental_steps(tmp_path) -> None:
    class Token:
    ...
    class Inference:
        ...
        def prefill(self, *_args, **_kwargs):
            self._generated_tokens = [raw]
            self._last_audio_tokens = raw
            return raw

    backend = OpenMossCudaBackend(settings(tmp_path, local_compile_mode="reduce-overhead"))
    inference_class = backend._get_cancellable_inference_class(Inference)
    inference = inference_class(cancel_event=threading.Event())

    captured = inference.prefill()

    assert captured is not raw
    assert captured.value == raw.value
    assert inference._generated_tokens == [captured]
    assert inference._last_audio_tokens is captured
...
def test_diagnostic_codec_encoder_offload_keeps_decode_graph_on_cuda(
    ...
    backend._prompt_tokens = {tmp_path / f"voice-{index}.wav": object() for index in range(1, 9)}
    ...
    assert backend.placement == {
        ...
        "warmup_user_text_tokens": 0,
        "dynamo_cache_size_limit": None,
        "attention_implementation": "sdpa",
        "local_compile_effective": True,
        "local_compile_mode": "reduce-overhead",
        "local_compile_dynamic": False,
...
def test_diagnostic_codec_encoder_offload_orders_prompt_encode_before_model_load(
    ...
    backend = OpenMossCudaBackend(configured)
    events: list[str] = []
    prompts = {f"debate_voice_{index}": tmp_path / f"voice-{index}.wav" for index in range(1, 9)}
...
def test_openmoss_warmup_primes_full_repetition_window_for_every_voice(
    ...
    def open_turn(*, prompt_path: Path, user_text: str, initial_text: str) -> Turn:
        del prompt_path
        observed_user_text.append(user_text)
    ...
    assert observed_user_text == [configured.warmup_user_text] * 8
    assert backend._warmup_user_text_tokens == len(
        backend.tokenizer.encode(configured.warmup_user_text, add_special_tokens=False)
    )
...
def _install_fake_tensor_torch(monkeypatch):
    ...
    class FakeTensor:
        ...
        def clone(self):
            return FakeTensor(self.values.copy(), device=self.device)
...
def test_cancellable_inference_uses_fixed_repetition_history_and_trims_ring(
    ...
    # Exercise the CUDA-graph canary path too: every generated token must be
    # snapshotted outside the compiled runner before entering repetition
    # history, otherwise the next graph replay overwrites it.
    backend = OpenMossCudaBackend(settings(tmp_path, local_compile_mode="reduce-overhead"))
    inference_class = backend._get_cancellable_inference_class(Inference)
...
def test_cancellable_inference_penalty_ignores_pad_and_penalizes_unique_valid_tokens(
    ...
    history = Tensor(np.asarray([[*[1024] * 45, 0, 3, 3, 5, 1024]], dtype=np.int64))
...
@pytest.mark.asyncio
async def test_backend_lifecycle_and_lazy_turn_output_share_one_persistent_thread(
    tmp_path: Path,
) -> None:
    operations: list[tuple[str, int]] = []

    class RecordingTurn(FakeTurn):
        def initial_audio(self):
            operations.append(("initial_audio", threading.get_ident()))
            chunks = super().initial_audio()

            def iterate():
                operations.append(("initial_iter", threading.get_ident()))
                yield from chunks

            return iterate()

        def push_text(self, text: str):
            operations.append(("push_text", threading.get_ident()))
            chunks = super().push_text(text)

            def iterate():
                operations.append(("push_iter", threading.get_ident()))
                yield from chunks

            return iterate()

        def finish(self):
            operations.append(("finish", threading.get_ident()))
            chunks = super().finish()

            def iterate():
                operations.append(("finish_iter", threading.get_ident()))
                yield from chunks
    ...
    class RecordingBackend(FakeBackend):
        def startup(self) -> None:
            operations.append(("startup", threading.get_ident()))
            super().startup()

        def warmup(self, prompts: dict[str, Path], text: str) -> None:
            operations.append(("warmup", threading.get_ident()))
            super().warmup(prompts, text)

        def open_turn(self, *, prompt_path: Path, user_text: str, initial_text: str):
            del user_text
            operations.append(("open_turn", threading.get_ident()))
            self.context_exited.clear()
            return RecordingTurn(self, initial_text)

        def shutdown(self) -> None:
            operations.append(("shutdown", threading.get_ident()))
            super().shutdown()

    runtime = GatewayRuntime(settings(tmp_path), RecordingBackend())
    await runtime.startup()
    assert runtime.health()["backend_execution_thread_alive"] is True
    await runtime.start_session(
    ...
    await runtime.push_text("persistent-thread", "中段", is_final=False)
    await runtime.push_text("persistent-thread", "尾段", is_final=True)
    await runtime.shutdown()
    ...
    assert len({thread_ident for _, thread_ident in operations}) == 1
    assert operations[0][1] != threading.get_ident()
    assert runtime.health()["backend_execution_thread_alive"] is False
