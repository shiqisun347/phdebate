...
def test_async_decoder_cancellation_is_bound_to_the_original_turn_event() -> None:
    events: list[str] = []
    decoder_started = threading.Event()
    decoder_release = threading.Event()
    ...
    bridge = OpenMossLowLatencyTextStreamBridge(
        ...
        decoder_poll_slice_seconds=0.001,
        decoder_join_timeout_seconds=0.5,
        externally_cancelled=turn_cancelled.is_set,
    )

    assert list(bridge.push_text_tokens([1])) == []
    assert decoder_started.wait(0.2)
    # The backend reuses one inferencer object. Replacing its mutable event must
    # not detach an older decoder worker from the event owned by its turn.
    inferencer.cancel_event = threading.Event()
    turn_cancelled.set()
    threading.Timer(0.02, decoder_release.set).start()
