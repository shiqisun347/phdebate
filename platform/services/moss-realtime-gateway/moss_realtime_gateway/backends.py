...
class FakeTurn:
    ...
    def _pcm_for(self, text: str) -> Iterable[bytes]:
        ...
        tone = array(
            "h",
            (round(5_000 * math.sin(2 * math.pi * 220 * (start + offset) / sample_rate)) for offset in range(samples)),
        )
        return [tone.tobytes()]
...
class OpenMossCudaBackend:
    ...
    def __init__(self, settings: GatewaySettings) -> None:
        ...
        self._warmup_user_text_tokens = 0
    ...
    def placement(self) -> dict[str, Any]:
        ...
        return {
            ...
            "prompt_token_frames": {
                path.name: int(getattr(tokens, "shape", (0, 0))[-1]) for path, tokens in self._prompt_tokens.items()
            },
            "warmup_text_tokens": self._warmup_text_tokens,
            "warmup_user_text_tokens": self._warmup_user_text_tokens,
            ...
            "local_compile_effective": bool(
                self.settings.local_compile_mode != "disabled"
                and self.settings.attention_implementation != "flash_attention_2"
            ),
            "local_compile_mode": self.settings.local_compile_mode,
            "local_compile_dynamic": self.settings.local_compile_dynamic,
    ...
    def _load_realtime_model(self) -> None:
        ...
        self.model = self._realtime_model_class.from_pretrained(
            ...
            # Transformers 5 treats ``dtype`` as the canonical loader argument.
            # The legacy ``torch_dtype`` alias is applied too late for the
            # FlashAttention 2 capability check and emits a misleading warning.
    ...
    def _get_cancellable_inference_class(self, inference_base: type[Any]) -> type[Any]:
        ...
        compile_mode = self.settings.local_compile_mode
        compile_dynamic = self.settings.local_compile_dynamic

        class CancellableInference(inference_base):
            fixed_repetition_window = 50
            audio_channel_pad = 1024

            def _get_local_transformer_runner(self) -> Any:
                """Keep the upstream production compile path and expose one canary mode.

                The class itself remains stable for the backend lifetime, so Dynamo
                guards do not recompile merely because a new turn starts.  The
                canary is intentionally limited to ``reduce-overhead`` and fixed
                shapes; arbitrary compile backends are not accepted from env.
                """

                if not self._should_compile_local_transformer or compile_mode == "disabled":
                    return self._generate_local_transformer_impl
                if self._compiled_local_transformer is None:
                    import torch

                    options: dict[str, Any] = {"fullgraph": True}
                    if compile_mode != "default":
                        options["mode"] = compile_mode
                        options["dynamic"] = compile_dynamic
                    self._compiled_local_transformer = torch.compile(
                        self._generate_local_transformer_impl,
                        **options,
                    )
            ...
            def prefill(self, *args: Any, **kwargs: Any) -> Any:
                audio_tokens = super().prefill(*args, **kwargs)
                if compile_mode == "reduce-overhead":
                    # ``prefill`` also calls the compiled local transformer and
                    # stores its graph-owned output in both history and
                    # ``_last_audio_tokens``.  Snapshot both references before
                    # the first incremental replay can invalidate that buffer.
            ...
            def step(
                ...
                if repetition_window != self.fixed_repetition_window:
                    raise ValueError(
                        "OpenMOSS realtime inference requires repetition_window=50 for its fixed-shape history"
                    )
                ...
                with torch.inference_mode():
                    ...
                    if len(text_tokens) != batch_size:
                        raise ValueError(
                            f"text_token batch size mismatch: got {len(text_tokens)}, expected {batch_size}."
                        )
                    ...
                    if compile_mode == "reduce-overhead":
                        # CUDA Graphs intentionally reuse their static output
                        # buffers.  Repetition history must own an immutable
                        # snapshot of this step; otherwise the next replay
                        # overwrites the previous token tensor and PyTorch
                        # aborts (or, on older versions, silently corrupts the
                        # history).  Clone outside the compiled local runner so
                        # each turn keeps the exact generated token values.
                        audio_tokens = audio_tokens.clone()
    ...
    def _codec_streaming_context(self) -> Any:
        if not self.settings.diagnostic_codec_encoder_offload:
            return self.codec.streaming(batch_size=1) if hasattr(self.codec, "streaming") else contextlib.nullcontext()
        decoder = getattr(self.codec, "decoder", None)
        if decoder is None:
    ...
    def startup(self) -> None:
        ...
        self.codec_device = torch.device(self.settings.diagnostic_codec_device or self.settings.device)
    ...
    def warmup(self, prompts: dict[str, Path], text: str) -> None:
        ...
        self._warmup_user_text_tokens = len(
            self.tokenizer.encode(self.settings.warmup_user_text, add_special_tokens=False)
        )

        for voice_id, path in prompts.items():
            voice_started = time.monotonic()
            logger.info("MOSS warmup voice started: voice_id=%s", voice_id)
            turn = self.open_turn(
                prompt_path=path,
                # The user instruction is part of the transformer prefix. It
                # must match the production speech-style instruction exactly;
                # warming only assistant text leaves a new fixed-shape CUDA
                # graph compile on the first live turn.
                user_text=self.settings.warmup_user_text,
            ...
            try:
                ...
                turn.close()
            logger.info(
                "MOSS warmup voice completed: voice_id=%s elapsed_seconds=%.3f",
                voice_id,
                time.monotonic() - voice_started,
            )
        ...
        logger.info(
            "MOSS warmup completed: voices=%d elapsed_seconds=%.3f",
            len(prompts),
    ...
    def open_turn(self, *, prompt_path: Path, user_text: str, initial_text: str) -> BackendTurn:
        ...
        try:
            ...
            bridge = OpenMossLowLatencyTextStreamBridge(
                ...
                decoder_join_timeout_seconds=(self.settings.async_decoder_join_timeout_seconds),
                decoder_execution_context=decoder_execution_context,
                prepare_decode=prepare_decode,
                externally_cancelled=cancel_event.is_set,
            )
