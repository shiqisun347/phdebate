...
class GatewaySettings:
    ...
    device: str = "cuda:0"
    # The pinned OpenMOSS revision includes PR #52. Its FlashAttention 2 path
    # uses DynamicCache and disables the local CUDA-graph compile; opt in only
    # after measuring RTF and TTFB on the production GPU.
    ...
    # A frame represents about 80 ms of audio.  On the production RTX 3090 the
    # 12-frame steady decoder batch is the smallest measured configuration that
    # leaves enough real-time headroom for 1.1x playout (8-voice RTF P95 0.883,
    # long-form RTF 0.805).  The publisher still emits paced 20 ms WebRTC frames,
    # so this internal codec batch does not become a browser-side audio burst.
    ...
    # ``disabled`` is the fail-safe path when PyTorch's thread-local CUDA Graph
    # state is unavailable. It preserves correctness at the cost of throughput.
    # Dynamic capture remains fail-closed because it recompiles and grows memory
    # for the session's evolving token shapes.
    local_compile_mode: str = "reduce-overhead"
    local_compile_dynamic: bool = False
    ...
    def from_env(cls) -> GatewaySettings:
        return cls(
            ...
            warmup_user_text=os.getenv(
                "MOSS_GATEWAY_WARMUP_USER_TEXT",
                "请用自信、坚定、有说服力的中文辩论语气，语速比自然朗读快约百分之十，"
                "重音清晰，停顿简短，句尾有力，但不要喊叫或夸张。",
            ),
            ...
            decode_chunk_frames=_int_env("MOSS_GATEWAY_DECODE_CHUNK_FRAMES", 12),
            ...
            local_compile_mode=os.getenv(
                "MOSS_GATEWAY_LOCAL_COMPILE_MODE", "reduce-overhead"
            ).strip().lower(),
            local_compile_dynamic=_bool_env("MOSS_GATEWAY_LOCAL_COMPILE_DYNAMIC", False),
    ...
    def validate(self) -> None:
        ...
        if self.local_compile_mode not in {"disabled", "default", "reduce-overhead"}:
            raise ValueError(
                "MOSS_GATEWAY_LOCAL_COMPILE_MODE must be disabled, default, or reduce-overhead"
            )
        if self.local_compile_dynamic:
            raise ValueError(
                "MOSS_GATEWAY_LOCAL_COMPILE_DYNAMIC must remain false; the fixed-shape "
                "realtime canary is not approved for dynamic CUDA-graph capture"
            )
        ...
        if self.backend == "openmoss":
            ...
            if self.attention_implementation not in {"sdpa", "flash_attention_2"}:
                raise ValueError(
                    "openmoss backend supports only the fixed SDPA or flash_attention_2 acceleration path"
                )
