...
class OpenMossLowLatencyTextStreamBridge:
    ...
    def __init__(
        ...
        *,
        ...
        externally_cancelled: Callable[[], bool] | None = None,
    ) -> None:
        ...
        # Bind cancellation to the immutable per-turn event when the backend
        # supplies one.  The inferencer object is intentionally reused between
        # turns, so reading its mutable ``cancel_event`` attribute from a late
        # decoder worker could otherwise observe the next turn's event.
        self._external_cancelled_callback = externally_cancelled
    ...
    def _externally_cancelled(self) -> bool:
        if self._external_cancelled_callback is not None:
            return bool(self._external_cancelled_callback())
