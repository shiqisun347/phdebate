...
class PersistentBackendExecutor:
    """Own one long-lived thread for every model/CUDA operation.

    PyTorch's reduce-overhead mode captures CUDA Graph state in thread-local
    storage.  Running model startup/warmup through ``asyncio.to_thread`` and
    then opening every live turn on a fresh session thread therefore captures
    and replays the graph in different TLS contexts.  Keep the complete backend
    lifecycle on one thread; the per-session worker remains only the command
    coordinator and audio-queue producer.

    Cancellation is deliberately signalled out of band by ``BackendTurn.abort``
    so a blocked generation task can observe its event.  The corresponding
    model cleanup still runs here after generation yields.
    """

    def __init__(self) -> None:
        self._pool = concurrent.futures.ThreadPoolExecutor(
            max_workers=1,
            thread_name_prefix="moss-inference",
        )
    ...
    def alive(self) -> bool:
        with self._lock:
            return self._thread_ident is not None and not self._closed

    def call(self, function: Any, /, *args: Any, **kwargs: Any) -> Any:
        with self._lock:
            if self._closed:
                raise RuntimeError("backend executor is closed")
            thread_ident = self._thread_ident
        if thread_ident == threading.get_ident():
            return function(*args, **kwargs)
        future = self._pool.submit(self._invoke, function, args, kwargs)
        return future.result()

    def _invoke(self, function: Any, args: tuple[Any, ...], kwargs: dict[str, Any]) -> Any:
        current_ident = threading.get_ident()
        with self._lock:
            if self._thread_ident is None:
                self._thread_ident = current_ident
            elif self._thread_ident != current_ident:
                raise RuntimeError("backend executor changed its persistent thread")
        return function(*args, **kwargs)
...
class GatewayRuntime:
    ...
    def __init__(
        ...
        *,
        ...
        self._backend_executor = PersistentBackendExecutor()
    ...
    async def startup(self) -> None:
        try:
            ...
            await asyncio.wait_for(
                asyncio.to_thread(self._backend_executor.call, self.backend.startup),
                self.settings.startup_timeout_seconds,
            )
            await asyncio.wait_for(
                asyncio.to_thread(
                    self._backend_executor.call,
                    self.backend.warmup,
                    prompt_paths,
        ...
        for session in sessions:
            if not session.worker_exited.is_set():
                try:
                    ...
                    pass
        try:
            await asyncio.to_thread(self._backend_executor.call, self.backend.shutdown)
        finally:
            await asyncio.to_thread(self._backend_executor.shutdown)
    ...
    def health(self) -> dict[str, Any]:
        ...
        return {
            ...
            "diagnostic_codec_encoder_offload": bool(self.settings.diagnostic_codec_encoder_offload),
            "placement": getattr(self.backend, "placement", None),
            "canary_stage_observability": bool(self.settings.canary_stage_observability),
            "backend_execution_thread_alive": self._backend_executor.alive,
        }
    ...
    async def start_session(
        ...
        *,
        ...
        resolved_canary_observe = bool(canary_observe and self.settings.canary_stage_observability)
    ...
    async def push_text(self, session_id: str, text: str, *, is_final: bool) -> dict[str, Any]:
        ...
        if is_final:
            ...
            with session.lock:
                ...
                session.commands.put(command)
        timeout = (
            self.settings.final_generation_timeout_seconds if is_final else self.settings.control_ack_timeout_seconds
        )
        if is_final:
    ...
    def _worker_main(self, session: Session) -> None:
        ...
        aborted = False
        try:
            turn = self._backend_executor.call(
                self.backend.open_turn,
            ...
            if session.abort_requested.is_set() or session.status == "orphaned":
                self._backend_executor.call(turn.abort)
                session.started.set()
                return
            session.status = "active"
            self._backend_executor.call(
                self._emit_turn_output,
                session,
                getattr(turn, "initial_audio", lambda: []),
            )
            session.started.set()
            while True:
                ...
                try:
                    ...
                    command.error = exc
                    logger.exception(
                        "MOSS gateway command failed session=%s command=%s",
                        session.session_id,
                        command.kind,
                    )
                    terminal_command = command
            ...
            logger.exception("MOSS gateway turn failed session=%s", session.session_id)
            session.started.set()
        finally:
            session.started.set()
            if turn is not None:
                ...
                try:
                    self._backend_executor.call(turn.close)
                except Exception as exc:
    ...
    def _emit_turn_output(
        ...
        *args: Any,
        ...
        """Create and consume lazy CUDA-backed output on the inference thread."""

        self._emit(session, producer(*args))
