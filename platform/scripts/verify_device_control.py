...
def main() -> None:
    ...
    try:
        ...
        # A rotated login session in the same browser retains the persistent
        # device lease. It must rebind silently instead of being presented as
        # another physical device, then the original tab can do the same.
        same_browser = secondary.post(
            f"/api/rooms/{code}/control-lease",
            headers=csrf(secondary) | {"X-Control-Lease": "live-device-a"},
            json={},
        )
        same_browser.raise_for_status()
        assert same_browser.json()["same_device_recovery"] is True
        primary_rebound = primary.post(
            f"/api/rooms/{code}/control-lease",
            headers=csrf(primary) | {"X-Control-Lease": "live-device-a"},
            json={},
        )
        primary_rebound.raise_for_status()
        assert primary_rebound.json()["same_device_recovery"] is True
        ...
        print(
            "device_control_verified missing=409 rotated_session_same_device=200 "
