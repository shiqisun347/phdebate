...
def pcm16k(path) -> bytes:
    with wave.open(str(path), "rb") as audio:
        ...
        if audio.getnchannels() != 1 or audio.getsampwidth() != 2:
            raise RuntimeError("generated LightTTS WAV must be mono 16-bit PCM")
...
async def main_async(base_url: str) -> None:
    ...
    try:
        ...
        with SessionLocal() as db:
            ...
            tts_config = match.service_snapshot["lighttts"]
            db.commit()
        ...
        expected = "人工智能时代仍然需要学习编程。"
        await lighttts.synthesize(
            expected,
            room_code=voice_room,
            speech_id="sample",
            voice="debate_voice_1",
            provider_config=tts_config,
        )
        frames = pcm16k(settings.media_path / voice_room / "sample.wav")
        ...
        async with websockets.connect(ws_url, ssl=ssl_context, additional_headers={"Cookie": cookie_header}) as socket:
            await socket.send(json.dumps({"type": "authenticate", "lease": lease}))
        ...
        print(f"asr_bridge_verified room={code} final={final['text']}")
    finally:
        await client.aclose()
        if room_id:
            with SessionLocal() as db:
