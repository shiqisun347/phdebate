    ...
    async with httpx.AsyncClient(base_url=base_url, verify=not insecure, timeout=30) as client:
        ...
        try:
            ...
            async with websockets.connect(ws_url, ssl=ssl_context, additional_headers={"Cookie": cookie_header}) as socket:
                ...
                (await client.post(f"/api/rooms/{code}/control-lease", headers=headers | {"X-Control-Lease": "e2e-device"}, json={})).raise_for_status()
                ...
                first_finish = await client.post(f"/api/rooms/{code}/speech/finish", headers=finish_headers, json={"speech_id": speech_id, "content": "真人发言端到端测试内容。"})
                first_finish.raise_for_status()
                second_finish = await client.post(f"/api/rooms/{code}/speech/finish", headers=finish_headers, json={"speech_id": speech_id, "content": "不应覆盖的内容"})
                assert second_finish.status_code == 409
                invalid_audio = await client.post(f"/api/rooms/{code}/speech/{speech_id}/audio", headers=headers, files={"audio": ("fake.wav", b"invalid", "audio/wav")})
                valid_audio = await client.post(f"/api/rooms/{code}/speech/{speech_id}/audio", headers=headers, files={"audio": ("recording.bin", wav_bytes(), "application/octet-stream")})
                valid_audio.raise_for_status()
                repeated_audio = await client.post(f"/api/rooms/{code}/speech/{speech_id}/audio", headers=headers, files={"audio": ("replacement.mp3", b"ID3replacement", "audio/mpeg")})
