...
def evaluate_gates(document: dict[str, Any], args: argparse.Namespace) -> dict[str, Any]:
    ...
    return {
        ...
        "release_ready": automatic_passed and all(item["passed"] for item in manual.values()),
    }
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    if any(value not in {1, 2, 3} for value in args.concurrency) or 1 not in args.concurrency or 3 not in args.concurrency:
        raise SystemExit("--concurrency values must be 1, 2, or 3 and include both 1 and 3")
