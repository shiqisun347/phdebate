...
formal transport; the HTTP lifecycle is benchmark-only compatibility coverage
and is never a runtime player or provider fallback. Each request records handshake latency, first PCM after the first
...
def delivery_excess_gaps_ms(arrivals: list[float], durations: list[float]) -> list[float]:
    """Return transport lateness after crediting audio already in each chunk.

    A codec may emit a 960 ms PCM block every ~800 ms.  The raw arrival gap is
    large, but it cannot be an audible gap because the previous block still has
    audio left.  Treat only ``arrival gap - previous chunk duration`` as network
    starvation; the continuous-playback simulation remains the authoritative
    playout gate.
    """
    ...
    if len(arrivals) != len(durations):
        raise ValueError("playback arrivals and durations must have equal length")
    return [
        max(0.0, (arrival - previous_arrival - previous_duration) * 1000)
        for previous_arrival, arrival, previous_duration in zip(
            arrivals,
            arrivals[1:],
            durations,
        )
    ...
    async with httpx.AsyncClient(timeout=timeout, headers=headers) as client:
        ...
        try:
            ...
            result = {
                ...
                "delivery_excess_gap_p99_ms": round(
                    percentile(delivery_excess_gaps_ms(chunk_arrivals, chunk_durations), 0.99)
                    or 0.0,
                    3,
                ),
    ...
    async def receive_control(websocket: Any) -> dict[str, Any]:
        ...
        while True:
            ...
            if payload.get("type") == "error":
                ...
                raise RuntimeError(f"candidate WS error: {payload.get('code') or 'unknown'}{suffix}")
...
def summarize(records: list[dict[str, Any]], concurrency: int) -> dict[str, Any]:
    ...
    excess_gaps = [float(record.get("delivery_excess_gap_p99_ms", 0)) for record in succeeded]
    playback_1200 = [
    ...
    summary = {
        ...
        "per_request_delivery_excess_gap_p99_ms": distribution(excess_gaps),
        "playback_1200ms": {
...
def markdown(document: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        "RTF/active P95 | Excess gap P99 max ms | 1200ms underruns/max ms | Lifecycle | Release gate |",
        "|---:|---:|---:|---:|---:|---:|---:|---|---|",
    ]
    for scenario in document["scenarios"]:
        ...
        rtf = summary["rtf"]
        gap = summary["per_request_delivery_excess_gap_p99_ms"]
    ...
    lines.extend(
        [
            ...
            "final, the 1200ms continuous playout model has zero underruns, delivery excess gap is reported "
            "after crediting PCM already queued, active RTF P95 is below 1.0, "
...
async def run(args: argparse.Namespace) -> int:
    ...
    document = {
        ...
        "user_text_configured": bool(args.user_text),
        "user_text_sha256": hashlib.sha256(args.user_text.encode()).hexdigest() if args.user_text else None,
        "chunk_characters": args.chunk_characters,
        "delta_delay_seconds": args.delta_delay_seconds,
        "finish_delay_seconds": args.finish_delay_seconds,
        "rounds": args.rounds,
    ...
    # A one-GPU optimization canary intentionally exercises only c1 because
    # the gateway admits a single active turn. Keep the default c1/c2/c3
    # release suite unchanged, but let an explicitly requested subset return a
    # meaningful process status instead of forcing callers to ignore exit 1.
    expected = set(args.concurrency)
    summaries = {scenario["concurrency"]: scenario["summary"] for scenario in scenarios}
    return (
        0
        if expected
        and expected.issubset(summaries)
        and all(summaries[item]["release_gate_passed"] for item in expected)
        else 1
    )
...
def argument_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--user-text",
        default=None,
        help="server-side speech instruction sent in the MOSS start frame; omitted by default",
    )
