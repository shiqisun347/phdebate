...
_SECRET_LIKE = re.compile(r"(?i)(?:bearer\s+|api[_-]?key[=:]\s*)[^\s,;]+")


def _safe_provider_error(provider: LLMProvider, exc: Exception) -> str:
    """Return actionable provider diagnostics without copying upstream secrets.

    Provider failures used to collapse into the unhelpful ``RuntimeError``
    label.  Keep the provider name, HTTP status/error code and a short message
    so an operator can identify the broken fallback, while redacting bearer
    tokens and bounding arbitrary upstream response text.
    """

    raw = re.sub(r"\s+", " ", str(exc)).strip()
    raw = _SECRET_LIKE.sub("<redacted>", raw)
    if len(raw) > 240:
        raw = f"{raw[:237]}..."
    return f"{provider.name} ({provider.provider}/{provider.model_id}): {raw or type(exc).__name__}"
    ...
    async def stream(self, task: AgentTask, payload: DebateRequest, prepared: PreparedState) -> AsyncIterator[dict[str, Any]]:
        ...
        async with self._semaphore:
            ...
            for provider in providers:
                ...
                if not await self._within_provider_limit(provider):
                    last_error = RuntimeError("provider rate limit reached")
                    provider_failures.append(_safe_provider_error(provider, last_error))
                    continue
                try:
                    ...
                    if exc.code in {"interrupted", "partial_stream_failed", "output_too_large", "output_truncated"}:
                        raise
                    last_error = exc
                    provider_failures.append(_safe_provider_error(provider, exc))
                    continue
                except Exception as exc:
                    last_error = exc
                    provider_failures.append(_safe_provider_error(provider, exc))
        ...
        if provider_failures:
            message = "所有 LLM Provider 均调用失败：" + "；".join(provider_failures)
        else:
