...
- 根 Git 仍记录大量旧 V1 文件，而完整 `v2/` 和 `debate-agent/` 目前未纳入根仓库版本历史。
...
- `v2/assets/moss-prompts` 的 8 个固定音色、逐字稿、来源、许可证和 SHA-256 清单。
...
- MOSS 模型、Tokenizer 和 Codec 作为独立只读模型卷迁移，不放入 Git；体积较小且许可证清晰的 8 个固定音色与来源清单保留在 `v2/assets/moss-prompts`。
...
8. [ ] 建立干净、可审计的 Git 历史，把当前未跟踪的 `v2/` 和 `debate-agent/` 正式纳入。
9. [ ] 在下一次音频发布窗口删除受保护的 LightTTS 兼容代码并生成新可靠基线。
10. [ ] 对历史 QA 大文件建立独立归档索引，生产源码包继续排除 `docs/qa`。
