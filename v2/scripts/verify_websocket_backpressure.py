...
async def main_async(args: argparse.Namespace) -> None:
    ...
    try:
        ...
        print(
            ...
            f"catchup_max_ms={max(ordered_catchup):.1f} errors=0"
        )
    finally:
        await client.aclose()
...
def main() -> None:
    if os.geteuid() == 0:
        raise SystemExit("请使用 V2 服务账号运行 WebSocket 压力验收。")
    parser = argparse.ArgumentParser()
    parser.add_argument("--base-url", default="https://117.50.192.216")
    parser.add_argument("--clients", type=int, default=500)
    parser.add_argument("--slow-clients", type=int, default=50)
    parser.add_argument("--events", type=int, default=64)
    args = parser.parse_args()
    if args.clients < 1 or not 0 <= args.slow_clients <= args.clients or args.events < 1:
        raise SystemExit("clients/events 必须为正数，slow-clients 必须位于 0..clients。")
