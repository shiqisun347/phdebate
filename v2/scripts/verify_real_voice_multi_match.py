#!/usr/bin/env python3
"""Run four concurrent matches with mocked Agents and real LightTTS, without external Agent calls."""
...
async def main_async(base_url: str) -> None:
    ...
    pause_code = ""
    paused_once = False
    playback_speech_ids: set[str] = set()
    generated_calls = 0
    ...
    try:
        ...
        created = await asyncio.gather(
            create_room(
                clients[0],
                account=f"real_voice_daily_a_{suffix}",
                real_name="多场语音验收甲",
            ...
            create_room(
                clients[1],
                account=f"real_voice_daily_b_{suffix}",
                real_name="多场语音验收乙",
                competition_slug="daily-4v4",
                topic=topics[1],
                topic_id=daily["topics"][1]["id"],
            ),
            create_room(
                clients[2],
                account=f"real_voice_training_a_{suffix}",
                real_name="多场语音验收丙",
            ...
            create_room(
                clients[3],
                account=f"real_voice_training_b_{suffix}",
                real_name="多场语音验收丁",
                competition_slug="training-1v1",
                topic=topics[3],
            ),
        ...
        async def generated(payload, *_args, **_kwargs):
            ...
            generated_calls += 1
            return (
                f"{payload['debater_name']}就“{payload['debate_topic']}”指出："
                "判断争议必须统一标准，并同时考察现实效果、长期风险和人的主体性。"
            )
        ...
        while time.monotonic() < deadline:
            ...
            with SessionLocal() as db:
                ...
                playing = list(
                    db.scalars(
                        select(Speech).where(
                            Speech.room_id.in_(room_ids),
                            Speech.status == "playing",
                        )
                    ).all()
                )
                for speech in playing:
                    if speech.id in playback_speech_ids:
                        continue
                    audio_path = settings.media_path / next(item.code for item in rooms if item.id == speech.room_id) / f"{speech.id}.wav"
                    assert audio_path.is_file() and speech.duration_seconds > 1
                    playback_speech_ids.add(speech.id)
                    speech.playback_ends_at = now()
                if playing:
                    db.commit()
            ...
            for client_index, failed_code in failed_rooms:
                ...
                retried = await clients[client_index].post(
                    ...
                    json={"reason": "真实 LightTTS 瞬时故障恢复验收"},
                )
        ...
        with SessionLocal() as db:
            ...
            for room in rooms:
                ...
                assert all(item.audio_url.startswith(f"/media/{room.code}/") for item in room_speeches)
        ...
        print(
            ...
            f"rooms=4 formats=4v4x2+1v1x2 speeches=8 agent_calls={generated_calls} "
            f"pause_resume=1 provider_retries={sum(provider_retries.values())} "
            f"real_lighttts=1 cross_room_audio_events=0 elapsed={elapsed:.2f}s"
        )
    finally:
        ...
        if room_ids:
            with SessionLocal() as db:
                speech_ids = select(Speech.id).where(Speech.room_id.in_(room_ids))
