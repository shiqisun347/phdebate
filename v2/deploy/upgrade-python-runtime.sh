...
PYTHON_BIN="${PHDEBATE_V2_PYTHON_BIN:-/usr/bin/python3}"
SERVICE_USER="${PHDEBATE_V2_SERVICE_USER:-ubuntu}"
SERVICE_GROUP="${PHDEBATE_V2_SERVICE_GROUP:-ubuntu}"
SUPERVISORCTL="${PHDEBATE_V2_SUPERVISORCTL:-supervisorctl}"
SHADOW_PORT="${PHDEBATE_V2_SHADOW_PORT:-12342}"
API_PORT="${PHDEBATE_V2_API_PORT:-12340}"
RELEASE="${PHDEBATE_V2_RELEASE:-$(date -u +%Y%m%dT%H%M%SZ)}"
RELEASES="$ROOT/.python-venvs"
RELEASE_DIR="$RELEASES/$RELEASE"
LOCK_FILE="$RELEASES/.upgrade.lock"
SERVICES=(jixia-v2-api jixia-v2-engine jixia-v2-worker)
...
assert_runtime_access() {
  local postgres_data_dir="${PHDEBATE_V2_POSTGRES_DATA_DIR:-$ROOT/runtime/postgres}"
  local postgres_user
...
if [[ "${PHDEBATE_V2_SKIP_SWITCH:-false}" == "true" ]]; then
