...
manifest_is_complete() {
  local manifest="$1" role filename bytes checksum
  local roles="v2_database agent_database data_volumes private_config reliable_voice moss_offline"

  [[ "$(value_for "$manifest" schema_version 2>/dev/null || true)" == "2" ]] || return 1
