...
def run() -> dict:
    ...
    def owner_post(path: str, payload: dict) -> httpx.Response:
        return owner_client.post(path, headers={"X-CSRF-Token": owner_client.cookies["jixia_v2_csrf"]}, json=payload)

    def admin_post(path: str) -> httpx.Response:
        return admin_client.post(path, headers={"X-CSRF-Token": admin_client.cookies["jixia_v2_csrf"]}, json={})
