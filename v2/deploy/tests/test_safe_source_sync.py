...
def test_source_sync_replaces_code_but_preserves_server_runtime(tmp_path: Path) -> None:
    ...
    (source_v2 / "apps").mkdir(parents=True)
    (source_v2 / "README.md").write_text("v2\n")
    (source_v2 / "apps" / "new.py").write_text("new\n")
    destination.mkdir()
    ...
    subprocess.run(["git", "-C", str(source), "add", "v2"], check=True)
    subprocess.run(["git", "-C", str(source), "commit", "-qm", "fixture"], check=True)

    env = os.environ | {"PHDEBATE_V2_ROOT": str(destination)}
