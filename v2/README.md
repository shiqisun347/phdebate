...
当前服务器的 FunASR 是本地轻量适配器，使用 `START` / PCM / `STOP` 命令协议；浏览器不能直接假设标准 FunASR JSON 握手，必须经过 V2 的 `/ws/rooms/:code/asr` 鉴权桥接。
...
```bash
PHDEBATE_V2_PYTHON_BIN_DIR=/path/to/python-3.10-venv/bin ./scripts/quality.sh
```

一次性浏览器环境会使用临时 SQLite 数据库，验证桌面与手机端的双用户房间大厅，以及系统管理员全部后台栏目；结束后自动删除测试用户和房间数据：

```bash
PHDEBATE_V2_TEST_PYTHON=/path/to/python-3.10-venv/bin ./deploy/verify-browser-lobby.sh
```

多房间状态机稳定性可用重复压力门禁验证；该流程使用测试数据库和模拟服务，不会调用外部 Agent：

```bash
PHDEBATE_V2_TEST_PYTHON=/path/to/python-3.10-venv/bin \
PHDEBATE_V2_SOAK_CYCLES=10 \
...
生产前端使用 Node.js 24 LTS 独立运行时。房间计时和状态扫描彼此独立，不会因为单个外部服务调用变慢而停止其他房间；Agent 与裁判调用默认全局最多并发 8 个。真人录音在应用层限制为 50 MiB，Nginx V2 API 入口限制为 52 MiB（包含 multipart 开销）。
...
PYTHONPATH=apps/api .venv/bin/python scripts/verify_parallel_audio_uploads.py \
  --base-url https://127.0.0.1 \
  --rooms 20
...
PYTHONPATH=apps/api .venv/bin/python scripts/verify_return_after_substitution.py \
  --base-url https://117.50.192.216
...
并行验收使用默认的 `parallel` 模式和 `/v2`。正式环境已切换为根路径；新版本应先生成独立 release 并影子验收，再切换 `.web-current`：

```bash
sudo ./deploy/prepare-runtime.sh
PHDEBATE_V2_WEB_DEPLOYMENT_MODE=root ./deploy/build-web-release.sh
```

根路径 Nginx 位置模板位于 `deploy/nginx-v2-root.locations.conf`；旧服务继续只绑定本机端口，回滚时恢复切换前的 Nginx 配置和前端构建即可。
...
```bash
sudo PHDEBATE_V2_SKIP_SWITCH=true ./deploy/upgrade-python-runtime.sh
```
...
生产 PostgreSQL 由 Supervisor 中的 `jixia-v2-backup` 每天北京时间 03:17 自动备份。备份使用自定义归档格式、原子写入、SHA-256 校验和与 14 天保留策略。手工备份及恢复演练：
