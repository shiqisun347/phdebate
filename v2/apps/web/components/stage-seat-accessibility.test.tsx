...
function renderStage(targetRoom: Room) {
  return render(<><FrozenStage /><StageSeatAccessibility room={targetRoom} /><StageScrollAccessibility /></>);
}
...
describe("StageSeatAccessibility", () => {
  ...
  it("keeps the unique current-speaker wording during fixed stages and active free-debate speeches", () => {
    ...
    rerender(<><FrozenStage /><StageSeatAccessibility room={room({ active_speech: { id: "speech", seat_key: "aff_1", speaker_type: "human", status: "speaking", content: "" } })} /><StageScrollAccessibility /></>);
    expect(queryByText(/本轮可发言席位/)).not.toBeInTheDocument();
    expect(queryByText(/当前可发言阵营/)).not.toBeInTheDocument();
  });
  ...
  it("cleans managed text on stage transition and unmount while preserving keyboard access to subtitles", async () => {
    const view = renderStage(room());
    ...
    view.rerender(<><FrozenStage /><StageSeatAccessibility room={room({ current_stage: { key: "aff_1", name: "正方一辩立论", kind: "speech", duration: 180, seat: "aff_1" } })} /><StageScrollAccessibility /></>);
    expect(firstSeatSource).toHaveTextContent("当前发言席位");
    ...
    expect(view.container.querySelector(".subtitle-stage p")).toHaveAttribute("tabindex", "0");
