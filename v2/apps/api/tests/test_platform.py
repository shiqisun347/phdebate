...
def test_seat_control_lease_is_explicit_and_takeover_is_safe(client: TestClient, register_user) -> None:
    ...
    assert public_control_events and all("lease_fingerprint" not in item["payload"] for item in public_control_events)
...
def test_public_projection_redacts_internal_failures(client: TestClient, register_user) -> None:
    ...
    assert outsider_failed["payload"] == {"provider": "lighttts", "message": "服务暂时异常"}
    public_result = client.get(f"/api/rooms/{room_data['code']}/result")
    assert public_result.status_code == 200
    result_failed = next(item for item in public_result.json()["events"] if item["type"] == "provider.failed")
    assert result_failed["payload"] == {"message": "服务暂时异常"}
    result_rtc = next(item for item in public_result.json()["events"] if item["type"] == "audio.rtc.started")
    assert result_rtc["payload"] == {"speech_id": streamed.id, "generation": "a" * 32}
