...
async def test_disconnected_human_is_replaced_then_explicitly_restored_without_cross_room_effects(
    register_user,
    monkeypatch,
) -> None:
    ...
    monkeypatch.setattr(debate_agent, "generate", AsyncMock(return_value="AI 接替断线辩手完成本轮必要发言。"))
    monkeypatch.setattr(lighttts, "synthesize", AsyncMock(return_value="/media/mock/non-audio.wav"))
    ...
    with SessionLocal() as db:
        ...
        # The substitute completed the disconnected participant's turn and the
        # authoritative flow advanced exactly once to the next human stage.
    ...
    assert restored["display_name"] == participant.get("/api/auth/session").json()["user"]["real_name"]
    with SessionLocal() as db:
        room = load_room(db, code)
        restored_row = next(item for item in room.seats if item.seat_key == "neg_1")
        assert restored_row.user_id == participant.get("/api/auth/session").json()["user"]["id"]
