...
async def test_round5_realtime_agent_body_gate_blocks_thinking_and_invalid_prefix_before_session(
    ...
    async def valid_stream(*_args, **_kwargs):
        ...
        yield {"type": "delta", "delta": "我"}
        yield {"type": "delta", "delta": "方"}
        await release_final.wait()
        yield {"type": "delta", "delta": "认为应当以长期影响作为判断标准。"}
        yield {"type": "final", "content": "我方认为应当以长期影响作为判断标准。"}
    ...
    assert "".join(sessions[1].pushes) == "我方"
    ...
    assert "".join(sessions[1].pushes) == "我方认为应当以长期影响作为判断标准。"
