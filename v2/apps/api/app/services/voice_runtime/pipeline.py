...
class IncrementalVoicePipeline:
    ...
    async def run(
        ...
        *,
        ...
        async def push_chunks(chunks: list[str]) -> None:
            nonlocal synthesis_error, submitted_text
            if synthesis_error is not None:
                return
            try:
                for chunk in chunks:
                    await session.push_text(chunk)
                    submitted_text += chunk
            ...
            while True:
                ...
                if not done:
                    ...
                    if soft_deadline is not None and now_monotonic >= soft_deadline:
                        ...
                        soft_deadline = (
                            now_monotonic + self.maximum_wait_seconds
                            if self.assembler.buffered_text
                            else None
                        )
                    continue
                try:
                ...
                if event_type == "delta":
                    ...
                    if chunks:
                        soft_deadline = (
                            asyncio.get_running_loop().time() + self.maximum_wait_seconds
                            if self.assembler.buffered_text
                            else None
                        )
