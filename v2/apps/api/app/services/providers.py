...
class DebateAgentProvider:
    ...
    async def interrupt(
        ...
        *,
        ...
        interrupt_url = urlunsplit(
            (parts.scheme, parts.netloc, f"{path}/tasks/{quote(task_id, safe='')}/interrupt", "", "")
        )
        ...
        *,
        ...
        try:
            ...
            async with client.stream("POST", url, json=request_payload, headers=headers, timeout=timeout) as response:
                ...
                if "text/event-stream" in content_type or request_payload.get("output", {}).get("stream"):
                    content = ""
                    final_emitted = False
    ...
    def _remove_stale_parts(cls, target_dir: Path, speech_id: str) -> None:
        ...
        for stale in entries:
            if (
                ...
                or not stale.name.startswith(stale_prefix)
                or not stale.name.endswith(".part")
            ):
                continue
            try:
        ...
        *,
        ...
        for attempt in range(1, self.MAX_REQUEST_ATTEMPTS + 1):
            ...
            while asyncio.get_running_loop().time() < min(deadline, job_deadline):
                ...
                await asyncio.sleep(
                    min(0.1, max(0.0, min(deadline, job_deadline) - asyncio.get_running_loop().time()))
                )
            if asyncio.get_running_loop().time() >= job_deadline:
                raise ProviderCancelled("LightTTS 全任务已超时。")
...
class MossTTSRealtimeProvider:
    ...
    def _ensure_pool(self) -> tuple[asyncio.Semaphore, list[tuple[str, asyncio.Semaphore]]]:
        ...
        if self._pool_loop is not loop or self._pool_key != key:
            ...
            self._endpoint_semaphores = [
                (endpoint, asyncio.Semaphore(settings.moss_tts_realtime_max_active_per_endpoint))
    ...
    async def _create_and_store_idle_websocket(self, endpoint: str) -> None:
        ...
        try:
            ...
            if websocket is not None and idle is None:
                await self._close_idle_websocket(
                    _MossIdleWebSocket(endpoint, connection, websocket, loop, loop.time())
                )
            raise
    ...
    async def _checkout_idle_websocket(self, endpoint: str) -> _MossIdleWebSocket | None:
        ...
        expired = (
            ...
            or asyncio.get_running_loop().time() - idle.created_at
            >= settings.moss_tts_realtime_idle_ws_ttl_seconds
        )
    ...
    async def readiness_snapshot(self) -> dict[str, Any]:
        ...
        required = math.ceil(
            settings.moss_tts_realtime_max_active / settings.moss_tts_realtime_max_active_per_endpoint
        )
        return {
            "ok": ready >= required,
    ...
    async def _acquire_ready_endpoint(
        ...
        while candidates:
            ...
            try:
                await self._probe_readiness(client, endpoint, deadline=deadline)
                return endpoint, semaphore
            except ProviderError as exc:
                semaphore.release()
    ...
    async def _synthesize_incremental_ws_job(
        ...
        *,
        ...
        loop = asyncio.get_running_loop()
        provider_deadline = loop.time() + settings.moss_tts_realtime_job_timeout_seconds
        deadline = min(provider_deadline, deadline_monotonic) if deadline_monotonic is not None else provider_deadline
            ...
            try:
                while True:
                    ...
                    try:
                        ...
                        if (
                            first_pcm_deadline is not None
                            and not upstream_pcm_received
                            and loop.time() >= first_pcm_deadline
                        ):
                            raise ProviderError(
                                "MOSS-TTS-Realtime 首个 PCM 超时。",
                                code="moss_tts_first_pcm_timeout",
                    ...
                    if event_type == "ready":
                        ...
                        if (
                            payload.get("voice") != voice
                            or payload.get("next_seq") != 1
        ...
        async def send_with_ack(
            ...
            *,
            ...
            await wait_for_future(
                pending_ack_future,
                timeout_seconds=(
                    settings.moss_tts_realtime_ack_timeout_seconds
        ...
        try:
            ...
            if (
                endpoint_semaphore is not None
                and selected_endpoint is not None
                and not endpoint_isolated
                and not upstream_release_acknowledged
            ):
                ...
                # Reconcile that cancellation-only ambiguity before returning
                # the slot; protocol/timeout isolation still remains strict.
                ...
                try:
                    ...
                    recovered_release = False
            if endpoint_semaphore is not None and (
                (upstream_release_acknowledged and not endpoint_isolated) or recovered_release
            ):
                ...
                logger.error("MOSS WS endpoint isolated speech=%s released=%s", speech_id, upstream_release_acknowledged)
            if global_acquired:
            ...
            async with self._client_context() as client:
                ...
                try:
                    ...
                    try:
                        ...
                        if (
                            not isinstance(close_body, dict)
                            or close_body.get("ok") is not True
                            or close_body.get("released") is not True
                        ):
                        ...
                        try:
                            if await LightTTSProvider._cancel_requested(should_cancel):
                                ...
                                upstream_release_acknowledged = bool(
                                    isinstance(abort_body, dict)
                                    and abort_body.get("ok") is True
                                    and abort_body.get("released") is True
                                )
                            ...
                            if upstream_release_acknowledged:
                                ...
                                close_body = close_response.json()
                                upstream_release_acknowledged = bool(
                                    isinstance(close_body, dict)
                                    and close_body.get("ok") is True
                                    and close_body.get("released") is True
                                )
                                if upstream_release_acknowledged:
                                    release_deadline = min(
                                        deadline,
...
class RealtimeTTSProviderRouter:
    ...
    def enabled(self) -> bool:
        if settings.realtime_voice_backend == "moss_realtime":
            return bool(
                settings.moss_tts_realtime_enabled
                and (settings.moss_tts_realtime_url.strip() or settings.moss_tts_realtime_urls)
            )
