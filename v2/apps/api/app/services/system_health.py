...
def _storage_check(*, production: bool) -> dict[str, Any]:
    ...
    return {
        "ok": not production or free_percent >= settings.health_min_disk_free_percent,
        "skipped": not production,
        "free_percent": round(free_percent, 2),
        "free_bytes": usage.free,
        "minimum_percent": settings.health_min_disk_free_percent,
    }


def _backup_check(*, production: bool) -> dict[str, Any]:
    if not production:
        return {"ok": True, "skipped": True}
    try:
        payload = json.loads(settings.backup_status_path.read_text(encoding="utf-8"))
...
async def system_readiness(db: Session) -> dict[str, Any]:
    ...
    checks["backup"] = _backup_check(production=production)
    try:
