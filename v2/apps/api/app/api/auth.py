...
from app.core.deps import CSRF_COOKIE, SESSION_COOKIE, current_user, optional_user, verify_csrf
...
@router.get("/session")
def session(user: User = Depends(current_user), db: Session = Depends(get_db)) -> dict:
    return {"user": _serialized_user(db, user)}


@router.get("/session-state")
def session_state(user: User | None = Depends(optional_user), db: Session = Depends(get_db)) -> dict:
