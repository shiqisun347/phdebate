...
def load_models(args):
    ...
    if _vllm_engine is None:
        ...
        if getattr(args, "disable_spk", False):
            logger.info("SPK disabled: phdebate already knows the speaker identity")
            _spk_model = None
        else:
...
async def handle_client(websocket, args):
    ...
    spk_tracker = HybridSpeakerTracker(spk_model, args.device) if spk_model is not None else None
...
async def main(args):
    load_models(args)
    logger.info(f"Server on ws://{args.host}:{args.port}")
    async with websockets.serve(
        lambda ws: handle_client(ws, args), args.host, args.port,
...
if __name__ == "__main__":
    ...
    parser.add_argument("--disable-spk", action="store_true", default=False)
